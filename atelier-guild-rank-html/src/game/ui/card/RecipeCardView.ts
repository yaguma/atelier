 ...
 * レシピカードビュー
 *
 * レシピカード（RecipeCard）の視覚的表現を担当する。
 * 設計文書: docs/design/atelier-guild-rank-phaser/ui-design/overview.md
 */

import Phaser from 'phaser';
import { IRecipeCard, Card } from '@domain/card/Card';
 ...
 * レシピカードビュー固有のオプション
 */
export interface RecipeCardViewOptions extends Omit<CardViewOptions, 'card'> {
  /** レシピカードデータ */
  card: IRecipeCard;
  /** 素材名解決関数（オプション） */
...
export class RecipeCardView implements ICardView {
  ...
  /** カードデータ */
  public readonly card: IRecipeCard;
   ...
   */
  constructor(scene: Phaser.Scene, options: RecipeCardViewOptions) {
    this.scene = scene;
    ...
    this.getItemName = options.getItemName;
    ...
    // カード種別の表示オプションを取得
    const typeOption = getCardTypeDisplayOption(CardType.RECIPE);
    ...
    // カード名（出力アイテム名）（中央上部）
    const outputItemName = this.formatOutputItem();
    ...
    // 必要素材ラベル
    const materialsLabel = scene.add.text(
      -width / 2 + CardLayout.PADDING,
      -height / 2 + 60 * scale,
      '必要素材:',
      mergeTextStyle('bodySmall', {
    ...
    this.container.add(materialsLabel);

    // 必要素材リスト
    const materialsStr = this.formatMaterials();
    this.materialsText = scene.add.text(
      -width / 2 + CardLayout.PADDING + 4,
      -height / 2 + 75 * scale,
      materialsStr,
      mergeTextStyle('cardDescription', {
        fontSize: `${Math.round(10 * scale)}px`,
        wordWrap: { width: width - CardLayout.PADDING * 2 - 4 },
      })
    );
  ...
  update(card: Card): void {
    if (card.type !== CardType.RECIPE) {
      console.warn('RecipeCardView: Card type mismatch');
      return;
    }

    const recipeCard = card as IRecipeCard;
    this.nameText.setText(this.formatOutputItem());
    this.costText.setText(`${recipeCard.cost}`);
    this.materialsText.setText(this.formatMaterials());
    this.outputText.setText(this.formatCategory());
  }
   ...
   * 出力アイテム名をフォーマット
   */
  private formatOutputItem(): string {
    // アイテム名解決関数があれば使用、なければIDを表示
    if (this.getItemName) {
      return this.getItemName(this.card.outputItemId);
    }
    return this.card.outputItemId;
  }
  ...
  private formatMaterials(): string {
    const maxDisplay = 3;
    const materials = this.card.requiredMaterials.slice(0, maxDisplay);
    ...
    if (this.card.requiredMaterials.length > maxDisplay) {
      lines.push('...');
    }
  ...
  private formatCategory(): string {
    return `カテゴリ: ${this.card.category}`;
  }

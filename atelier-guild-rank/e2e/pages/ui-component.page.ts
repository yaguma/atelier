...
export class UIComponentPage extends BasePage {
  ...
  async isSidebarCollapsed(sectionName?: string): Promise<boolean> {
    const selector = sectionName ? `.sidebar-section[data-section="${sectionName}"]` : '.sidebar';
  ...
  async hasParticleEffect(): Promise<boolean> {
    return await this.page.evaluate(() => {
      ...
      return (
        scene?.children?.list?.some((child: any) => child.type === 'ParticleEmitterManager') ??
        false
      );
    });
  }

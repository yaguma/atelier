...
export class DeliveryPage extends BasePage {
	 ...
	 * @returns 要求アイテムの情報
	 */
	async getQuestRequirement(
		questId: string
	): Promise<{ itemType: string; minQuality: string }> {
		const quest = this.getDeliveryQuest(questId);
		const itemType = (await quest.getAttribute('data-required-item')) ?? '';
		const minQuality =
			(await quest.getAttribute('data-min-quality')) ?? 'D';
	...
	async getReward(): Promise<{ gold: number; contribution: number }> {
		...
		const goldText =
			(await rewardDisplay.locator('.reward-gold').textContent()) ?? '0';
		const gold = Number.parseInt(goldText.replace(/[^\d]/g, ''), 10);

		const contributionText =
			(await rewardDisplay.locator('.reward-contribution').textContent()) ??
			'0';
		const contribution = Number.parseInt(
			contributionText.replace(/[^\d]/g, ''),
			10
		);
	...
	async endDay(): Promise<void> {
		...
		await this.page.waitForFunction(
			() => {
				const state = (window as any).gameState?.();
				return (
					state?.currentPhase === 'QuestAccept' ||
					state?.currentScene !== 'MainScene'
				);
			},
			{ timeout: 5000 }
		);
	...
	private getDeliveryQuest(questId: string): Locator {
		return this.page.locator(
			`.delivery-quest-item[data-quest-id="${questId}"]`
		);
	}

...
test.describe('Phase 5 UI強化: フェードアニメーション (TASK-0038)', () => {
	test('UI-0038-01: TitleSceneのフェードイン', async ({ gamePage }) => {
	...
	test('UI-0038-02: シーン遷移時のフェードアウト', async ({ gamePage }) => {
...
test.describe('Phase 5 UI強化: ボタンホバーエフェクト (TASK-0039)', () => {
	test('UI-0039-01: ボタンホバー時の拡大エフェクト', async ({ gamePage }) => {
	...
	test('UI-0039-02: ボタンホバー時の色変化（ビジュアルリグレッション）', async ({ gamePage }) => {
...
// 合計: 14ケース
// - 実装済み（有効）: 2ケース（TASK-0038）
// - 未実装（skip）: 12ケース（MainScene実装後に有効化）
//
// 優先度:

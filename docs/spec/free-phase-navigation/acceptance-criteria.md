...
- [ ] **TC-003-05**: 複数日分のAP超過 🟡
  - **入力**: AP残量3で合計AP消費7
  - **期待結果**: 2日分消費（7/3 = ceil 3日分使用、日をまたぐ2回）、翌々日AP = 3 - (7 - 6) = 2
  - **信頼性**: 🟡 *ヒアリングQ5から妥当な推測*
...
- [ ] **TC-003-B03**: AP消費上限×2（6） 🟡
  - **入力**: AP消費6
  - **期待結果**: 1日進行、翌日AP = 3（超過分0）
  - **信頼性**: 🟡 *計算式から推測: 6 mod 3 = 0*

- [ ] **TC-003-B04**: AP消費上限×2+1（7） 🟡
  - **入力**: AP消費7
  - **期待結果**: 2日進行、翌々日AP = 2
  - **信頼性**: 🟡 *計算式から推測*
...
- [ ] **TC-EDGE-002-01**: 2日分の日終了処理順次実行
  - **条件**: AP7消費（2日分消費）
  - **期待結果**: 各日の日終了処理（依頼期限更新、手札補充等）が順次実行される
  - **信頼性**: 🟡 *推測*

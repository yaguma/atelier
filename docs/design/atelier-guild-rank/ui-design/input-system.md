...
採取地カードを使用してドラフト採取UIが表示された状態でのキーバインド。

| キー | アクション | 説明 |
|------|-----------|------|
| `1`-`3` | 素材選択 | 提示された3つの素材から選択 |
| `0` / `S` | スキップ | 今回の提示をスキップ |
| `E` | 採取終了 | 採取を終了してコスト計算 |
| `Escape` | キャンセル | 採取をキャンセル（未選択時のみ） |
| `←` / `→` | 素材間移動 | 3つの素材間のフォーカス移動 |
| `Enter` / `Space` | 決定 | フォーカス中の素材を選択 |
...
| **ドラフト採取中** | 素材選択操作 | 1-3選択、0スキップ、E終了 |
...
| 2026-01-01 | 1.1.0 | ドラフト採取フェーズのキーバインドを追加（セクション2.4）。入力コンテキストに「ドラフト採取中」を追加。 |

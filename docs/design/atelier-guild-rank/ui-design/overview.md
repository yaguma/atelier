...
stateDiagram-v2
    ...
    SCR_002_Main --> SCR_004_RankUp: 昇格ゲージ満タン（昇格試験解放）
...
| コンポーネント | 責務 | 表示内容 |
|--------------|------|----------|
| **RoundIndicator** | 現在のラウンド表示 | 「2/3回目」のような進捗表示 |
| **MaterialOptionView** | 素材オプション表示 | 3つの素材カードを横並び表示 |
| **MaterialCard** | 個別素材表示 | 素材名、品質、アイコン |
| **SelectedMaterialsView** | 選択済み素材一覧 | これまでに選んだ素材のリスト |
| **GatheringCostView** | コスト計算表示 | 基本+追加=合計のリアルタイム表示 |
| **ExtraDayWarning** | 翌日持越し警告 | 7個以上選択時の警告表示 |
...
| 2026-01-01 | 1.1.0 | ドラフト採取UI仕様を追加。採取フェーズのUI説明を更新。RoundIndicator、MaterialOptionView、GatheringCostView等のコンポーネントを追加。 |
| 2026-01-02 | 1.2.0 | 「ランクHP」から「昇格ゲージ」への用語変更に対応。画面遷移図・プログレスバーの表記を修正。 |

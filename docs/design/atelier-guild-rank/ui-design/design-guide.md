...
#### ターシャリボタン（設定・控えめなアクション）

タイトル画面の設定ボタン等、画面上で最も控えめに見せたいアクションに使う第3スタイル。薄い枠線とミューテッド文字で構成し、フォントは小さめ（13px）にする。

| 状態 | 背景 | 枠線 | テキスト |
|------|------|------|---------|
| 通常 | `surface.card` | `border.subtle` 1.5px | `text.muted` |
| ホバー | `surface.sidebar` | `border.subtle` 1.5px | `text.muted` |

- フォントサイズ: 13px（他ボタンは16px）
- `ButtonType.TERTIARY` で利用する
...
- **セカンダリ**: キャンセル・戻る・補助
- **デンジャー**: 日終了・破棄・不可逆操作
- **ターシャリ**: 設定など最も控えめなアクション（薄い枠線＋ミューテッド文字・13px）

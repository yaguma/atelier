...
| レイヤー | 技術 | 用途 |
|---------|------|------|
| **Canvas UI** | Phaser + rexUI | ゲーム本体のUI（カード、フェーズ表示等） |
| **HTML Overlay UI** | HTML + Tailwind CSS | オーバーレイUI（メニュー、ダイアログ、設定画面等） |
...
- 外部依存を必要最小限に抑える
- Biome による一貫したコードスタイルの維持
- Lefthook によるコミット前の品質チェック自動化
...
| 2026-01-16 | 2.2.0 | 技術スタック更新：Tailwind CSS, pnpm, Biome, Lefthook, Vitest, Playwright, TypeDoc追加 |

...
| テストID | テスト内容 | 期待結果 |
|---------|----------|----------|
| T-0015-01 | 素材追加 | 追加成功、カウント増加 |
| T-0015-02 | 素材削除 | 削除成功、インスタンス返却 |
| T-0015-03 | 容量超過 | エラー発生 |
| T-0015-04 | カテゴリフィルタ | 該当素材のみ返却 |
| T-0015-05 | アーティファクト追加 | 追加成功 |
| T-0015-06 | クリア | 全アイテム削除 |
...
| 成果物 | パス |
|--------|------|
| インターフェース | `/src/domain/interfaces/inventory-service.interface.ts` |
| 実装 | `/src/application/services/inventory-service.ts` |
| テスト | `/tests/unit/application/services/inventory-service.test.ts` |

---

## 関連文書

- **セーブデータスキーマ**: [../../design/atelier-guild-rank/data-schema-save.md](../../design/atelier-guild-rank/data-schema-save.md)

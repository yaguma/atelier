# TASK-0014: ContributionCalculator・RankService実装

**バージョン**: 1.0.0
**作成日**: 2026-01-16
**フェーズ**: 2 - ドメイン層・コアサービス
**見積時間**: 4時間（半日）
**依存タスク**: TASK-0005, TASK-0013

---

## 1. タスク概要

| 項目 | 値 |
|------|-----|
| **タスクID** | TASK-0014 |
| **タスク名** | ContributionCalculator・RankService実装 |
| **カテゴリ** | ドメイン層 |
| **優先度** | 高 |
| **担当レイヤー** | Domain, Application |

### 信頼性レベル

- 🔵 **青信号**: 設計文書に記載
- 🟡 **黄信号**: 設計文書から妥当な推測
- 🔴 **赤信号**: 設計文書にない推測

---

## 2. 実装内容

### 2.1 貢献度計算式 🔵

```
最終貢献度 = 基礎貢献度 × 品質補正 × 依頼者補正 × コンボ補正

品質補正:
- D: 0.5
- C: 0.75
- B: 1.0
- A: 1.5
- S: 2.0

依頼者補正:
- villager: 1.0
- adventurer: 1.1
- merchant: 1.2
- noble: 1.3
- guild: 1.5

コンボ補正:
- 同日複数納品: 1 + 0.1 × (納品数 - 1)
```

### 2.2 ContributionCalculator 🔵

`src/domain/services/contribution-calculator.ts`:

```typescript
export class ContributionCalculator {
  calculate(delivery: DeliveryContext): number {
    const base = delivery.quest.reward.contribution;
    const qualityMod = this.getQualityModifier(delivery.itemQuality);
    const clientMod = this.getClientModifier(delivery.quest.clientType);
    const comboMod = this.getComboModifier(delivery.deliveryCount);

    return Math.floor(base * qualityMod * clientMod * comboMod);
  }

  private getQualityModifier(quality: Quality): number {
    const modifiers: Record<Quality, number> = {
      'D': 0.5,
      'C': 0.75,
      'B': 1.0,
      'A': 1.5,
      'S': 2.0,
    };
    return modifiers[quality];
  }

  private getClientModifier(clientType: ClientType): number {
    const modifiers: Record<ClientType, number> = {
      'villager': 1.0,
      'adventurer': 1.1,
      'merchant': 1.2,
      'noble': 1.3,
      'guild': 1.5,
    };
    return modifiers[clientType];
  }

  private getComboModifier(deliveryCount: number): number {
    return 1 + 0.1 * (deliveryCount - 1);
  }
}
```

### 2.3 RankServiceインターフェース 🔵

`src/domain/interfaces/rank-service.interface.ts`:

```typescript
export interface IRankService {
  // ランク操作
  getCurrentRank(): GuildRank;
  getPromotionGauge(): number;
  addContribution(amount: number): void;

  // 昇格判定
  canPromote(): boolean;
  promote(): PromotionResult;

  // 昇格試験
  startPromotionTest(): PromotionTest;
  completePromotionTest(success: boolean): void;

  // ランク情報取得
  getRankRequirements(rank: GuildRank): GuildRankMaster;
  getNextRank(): GuildRank | null;
}

export interface PromotionResult {
  previousRank: GuildRank;
  newRank: GuildRank;
  bonusReward: number;
}
```

### 2.4 RankService実装 🔵

`src/application/services/rank-service.ts`:

- 昇格ゲージ管理
- 昇格条件チェック
- 昇格処理とイベント発火

---

## 3. 受け入れ基準

### 3.1 必須条件 🔵

- [ ] 貢献度計算が正しく動作する
- [ ] 品質・依頼者・コンボ補正が適用される
- [ ] 昇格ゲージが100到達で昇格可能
- [ ] G→F→E...→Sの順で昇格する

### 3.2 推奨条件 🟡

- [ ] 昇格時のボーナス報酬
- [ ] 単体テストカバレッジ80%以上

---

## 4. テストケース

### 4.1 単体テスト

| テストID | テスト内容 | 期待結果 |
|---------|----------|----------|
| T-0014-01 | 貢献度計算（基本） | 基礎値のみ |
| T-0014-02 | 品質補正適用 | S品質で2倍 |
| T-0014-03 | 依頼者補正適用 | guild依頼で1.5倍 |
| T-0014-04 | コンボ補正適用 | 3回納品で1.2倍 |
| T-0014-05 | 昇格可能判定 | ゲージ100以上でtrue |
| T-0014-06 | 昇格処理 | ランクアップ、ゲージリセット |

---

## 5. 成果物

| 成果物 | パス |
|--------|------|
| 貢献度計算 | `/src/domain/services/contribution-calculator.ts` |
| インターフェース | `/src/domain/interfaces/rank-service.interface.ts` |
| 実装 | `/src/application/services/rank-service.ts` |
| テスト | `/tests/unit/domain/services/contribution-calculator.test.ts` |
| テスト | `/tests/unit/application/services/rank-service.test.ts` |

---

## 関連文書

- **ゲームメカニクス設計**: [../../design/atelier-guild-rank/game-mechanics.md](../../design/atelier-guild-rank/game-mechanics.md)
- **コアサービス設計**: [../../design/atelier-guild-rank/core-systems-core-services.md](../../design/atelier-guild-rank/core-systems-core-services.md)

---

## 変更履歴

| 日付 | バージョン | 変更内容 |
|------|----------|---------|
| 2026-01-16 | 1.0.0 | 初版作成 |

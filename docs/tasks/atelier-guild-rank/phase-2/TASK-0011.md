# TASK-0011: GatheringService実装（ドラフト採取） ✅ **完了**

**バージョン**: 1.0.0
**作成日**: 2026-01-16
**完了日**: 2026-01-16
**フェーズ**: 2 - ドメイン層・コアサービス
**見積時間**: 4時間（半日）
**依存タスク**: TASK-0009, TASK-0010
**完了理由**: TDD開発完了 - 16テストケース全通過（正常系6件、異常系4件、境界値6件）

---

## 1. タスク概要

| 項目 | 値 |
|------|-----|
| **タスクID** | TASK-0011 |
| **タスク名** | GatheringService実装（ドラフト採取） |
| **カテゴリ** | ドメイン層 |
| **優先度** | 最高 |
| **担当レイヤー** | Application |

### 信頼性レベル

- 🔵 **青信号**: 設計文書に記載
- 🟡 **黄信号**: 設計文書から妥当な推測
- 🔴 **赤信号**: 設計文書にない推測

---

## 2. 実装内容

### 2.1 ドラフト採取システム概要 🔵

1. プレイヤーはカードをプレイしてAP消費
2. 素材プール（6つ）から1つ選択
3. 選択した素材を獲得、プールは1つ減少
4. 残りプールから再度選択を繰り返す
5. プール枯渇または採取終了で完了

### 2.2 GatheringServiceインターフェース 🔵

`src/domain/interfaces/gathering-service.interface.ts`:

```typescript
export interface IGatheringService {
  // ドラフト採取開始
  startDraftGathering(card: Card): MaterialPool;

  // 素材選択
  selectMaterial(poolIndex: number): MaterialInstance;

  // 状態取得
  getCurrentPool(): MaterialPool;
  getRemainingSelections(): number;
  isGatheringActive(): boolean;

  // 採取コスト計算
  calculateGatheringCost(card: Card): number;

  // 採取終了
  endGathering(): MaterialInstance[];
}

export interface MaterialPool {
  materials: MaterialInstance[];
  maxSelections: number;
  currentSelections: number;
}
```

### 2.3 GatheringService実装 🔵

`src/application/services/gathering-service.ts`:

```typescript
export class GatheringService implements IGatheringService {
  private currentPool: MaterialPool | null = null;
  private gatheredMaterials: MaterialInstance[] = [];

  constructor(
    private materialService: IMaterialService,
    private masterDataRepo: IMasterDataRepository,
    private eventBus: IEventBus,
  ) {}

  startDraftGathering(card: Card): MaterialPool {
    // カード効果に基づいてプール生成
    const poolSize = 6;
    const maxSelections = card.effect.gatheringCount ?? 3;

    const materials = this.generateMaterialPool(poolSize, card);

    this.currentPool = {
      materials,
      maxSelections,
      currentSelections: 0,
    };

    this.eventBus.emit(GameEventType.GATHERING_STARTED, {
      pool: this.currentPool,
    });

    return this.currentPool;
  }

  selectMaterial(poolIndex: number): MaterialInstance {
    if (!this.currentPool) {
      throw new Error('No active gathering session');
    }

    const material = this.currentPool.materials[poolIndex];
    this.gatheredMaterials.push(material);

    // プールから削除
    this.currentPool.materials.splice(poolIndex, 1);
    this.currentPool.currentSelections++;

    this.eventBus.emit(GameEventType.MATERIAL_SELECTED, { material });

    return material;
  }

  // ... 他のメソッド実装
}
```

### 2.4 素材プール生成ロジック 🔵

- ランクに応じた素材候補フィルタリング
- カードタイプによる素材カテゴリ重み付け
- 品質のランダム変動

---

## 3. 受け入れ基準

### 3.1 必須条件 🔵

- [x] ドラフト採取が開始できる ✅
- [x] 素材プールから選択できる ✅
- [x] 選択した素材がインベントリに追加される ✅
- [x] 選択回数制限が機能する ✅

### 3.2 推奨条件 🟡

- [x] カード効果による採取数変動 ✅
- [x] 単体テストカバレッジ80%以上 ✅ (100%達成)

---

## 4. テストケース

### 4.1 単体テスト

| テストID | テスト内容 | 期待結果 |
|---------|----------|----------|
| T-0011-01 | ドラフト採取開始 | プール6個生成 |
| T-0011-02 | 素材選択 | プール-1、獲得+1 |
| T-0011-03 | 選択回数上限 | 上限到達で選択不可 |
| T-0011-04 | 採取終了 | 獲得素材リスト返却 |
| T-0011-05 | カード効果適用 | 採取数が効果通り |

---

## 5. 成果物

| 成果物 | パス |
|--------|------|
| インターフェース | `/src/domain/interfaces/gathering-service.interface.ts` |
| 実装 | `/src/application/services/gathering-service.ts` |
| テスト | `/tests/unit/application/services/gathering-service.test.ts` |

---

## 関連文書

- **ゲームメカニクス設計**: [../../design/atelier-guild-rank/game-mechanics.md](../../design/atelier-guild-rank/game-mechanics.md)
- **コアサービス設計**: [../../design/atelier-guild-rank/core-systems-core-services.md](../../design/atelier-guild-rank/core-systems-core-services.md)

---

## 変更履歴

| 日付 | バージョン | 変更内容 |
|------|----------|---------|
| 2026-01-16 | 1.0.0 | 初版作成 |

# TASK-0009: カードエンティティ・DeckService実装

**バージョン**: 1.0.0
**作成日**: 2026-01-16
**フェーズ**: 2 - ドメイン層・コアサービス
**見積時間**: 4時間（半日）
**依存タスク**: TASK-0003, TASK-0004

---

## 1. タスク概要

| 項目 | 値 |
|------|-----|
| **タスクID** | TASK-0009 |
| **タスク名** | カードエンティティ・DeckService実装 |
| **カテゴリ** | ドメイン層 |
| **優先度** | 最高 |
| **担当レイヤー** | Domain, Application |

### 信頼性レベル

- 🔵 **青信号**: 設計文書に記載
- 🟡 **黄信号**: 設計文書から妥当な推測
- 🔴 **赤信号**: 設計文書にない推測

---

## 2. 実装内容

### 2.1 カードエンティティ 🔵

`src/domain/entities/Card.ts`:

```typescript
export class Card {
  constructor(
    public readonly id: CardId,
    public readonly master: CardMaster,
  ) {}

  get name(): string {
    return this.master.name;
  }

  get type(): CardType {
    return this.master.type;
  }

  get cost(): number {
    return this.master.cost;
  }
}
```

### 2.2 DeckServiceインターフェース 🔵

`src/domain/interfaces/deck-service.interface.ts`:

```typescript
export interface IDeckService {
  // デッキ操作
  shuffle(): void;
  draw(count: number): Card[];
  playCard(card: Card): void;
  discardHand(): void;
  refillHand(): void;

  // 状態取得
  getDeck(): Card[];
  getHand(): Card[];
  getDiscard(): Card[];
  getHandSize(): number;

  // デッキ構築
  addCard(cardId: CardId): void;
  removeCard(cardId: CardId): void;

  // 初期化
  initialize(cardIds: CardId[]): void;
  reset(): void;
}
```

### 2.3 DeckService実装 🔵

`src/application/services/deck-service.ts`:

```typescript
export class DeckService implements IDeckService {
  private deck: Card[] = [];
  private hand: Card[] = [];
  private discard: Card[] = [];
  private readonly HAND_SIZE = 5;

  constructor(
    private masterDataRepo: IMasterDataRepository,
    private eventBus: IEventBus,
  ) {}

  shuffle(): void {
    // Fisher-Yatesシャッフル
    for (let i = this.deck.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [this.deck[i], this.deck[j]] = [this.deck[j], this.deck[i]];
    }
  }

  draw(count: number): Card[] {
    const drawn: Card[] = [];
    for (let i = 0; i < count && this.deck.length > 0; i++) {
      const card = this.deck.pop()!;
      drawn.push(card);
      this.hand.push(card);
    }
    this.eventBus.emit(GameEventType.CARD_DRAWN, { cards: drawn });
    return drawn;
  }

  // ... 他のメソッド実装
}
```

### 2.4 シャッフルロジック 🔵

- Fisher-Yatesアルゴリズム
- デッキが空になったら捨て札をシャッフルしてデッキに戻す

---

## 3. 受け入れ基準

### 3.1 必須条件 🔵

- [ ] シャッフルが正しく動作する
- [ ] ドローで手札にカードが追加される
- [ ] プレイでカードが捨て札に移動する
- [ ] 手札補充が正しく動作する

### 3.2 推奨条件 🟡

- [ ] 各操作でイベントが発火する
- [ ] 単体テストカバレッジ80%以上

---

## 4. テストケース

### 4.1 単体テスト

| テストID | テスト内容 | 期待結果 |
|---------|----------|----------|
| T-0009-01 | シャッフル | 順序がランダム化 |
| T-0009-02 | ドロー | 手札に追加、デッキから減少 |
| T-0009-03 | カードプレイ | 手札から削除、捨て札に追加 |
| T-0009-04 | 手札補充 | 5枚になるまでドロー |
| T-0009-05 | デッキ枯渇時のドロー | 捨て札をシャッフルしてデッキに |

---

## 5. 成果物

| 成果物 | パス |
|--------|------|
| カードエンティティ | `/src/domain/entities/Card.ts` |
| インターフェース | `/src/domain/interfaces/deck-service.interface.ts` |
| 実装 | `/src/application/services/deck-service.ts` |
| テスト | `/tests/unit/application/services/deck-service.test.ts` |

---

## 関連文書

- **コアサービス設計**: [../../design/atelier-guild-rank/core-systems-core-services.md](../../design/atelier-guild-rank/core-systems-core-services.md)
- **インターフェース定義**: [../../design/atelier-guild-rank/interfaces.ts](../../design/atelier-guild-rank/interfaces.ts)

---

## 変更履歴

| 日付 | バージョン | 変更内容 |
|------|----------|---------|
| 2026-01-16 | 1.0.0 | 初版作成 |

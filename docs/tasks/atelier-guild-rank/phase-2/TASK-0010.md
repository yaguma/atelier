# TASK-0010: 素材エンティティ・MaterialService実装

**バージョン**: 1.0.0
**作成日**: 2026-01-16
**フェーズ**: 2 - ドメイン層・コアサービス
**見積時間**: 4時間（半日）
**依存タスク**: TASK-0003, TASK-0004

---

## 1. タスク概要

| 項目 | 値 |
|------|-----|
| **タスクID** | TASK-0010 |
| **タスク名** | 素材エンティティ・MaterialService実装 |
| **カテゴリ** | ドメイン層 |
| **優先度** | 高 |
| **担当レイヤー** | Domain, Application |

### 信頼性レベル

- 🔵 **青信号**: 設計文書に記載
- 🟡 **黄信号**: 設計文書から妥当な推測
- 🔴 **赤信号**: 設計文書にない推測

---

## 2. 実装内容

### 2.1 素材インスタンスエンティティ 🔵

`src/domain/entities/MaterialInstance.ts`:

```typescript
export class MaterialInstance {
  constructor(
    public readonly instanceId: string,
    public readonly master: MaterialMaster,
    public readonly quality: Quality,
  ) {}

  get materialId(): MaterialId {
    return this.master.id;
  }

  get name(): string {
    return this.master.name;
  }

  get category(): MaterialCategory {
    return this.master.category;
  }
}
```

### 2.2 品質値オブジェクト 🔵

`src/domain/value-objects/Quality.ts`:

```typescript
export const QUALITY_ORDER: Record<Quality, number> = {
  'D': 1,
  'C': 2,
  'B': 3,
  'A': 4,
  'S': 5,
};

export function compareQuality(a: Quality, b: Quality): number {
  return QUALITY_ORDER[a] - QUALITY_ORDER[b];
}

export function getHigherQuality(a: Quality, b: Quality): Quality {
  return compareQuality(a, b) >= 0 ? a : b;
}
```

### 2.3 MaterialServiceインターフェース 🔵

`src/domain/interfaces/material-service.interface.ts`:

```typescript
export interface IMaterialService {
  // 素材生成
  createInstance(materialId: MaterialId, quality: Quality): MaterialInstance;
  generateRandomQuality(baseQuality: Quality): Quality;

  // 品質計算
  calculateAverageQuality(materials: MaterialInstance[]): Quality;

  // 検索
  getMaterialsByCategory(category: MaterialCategory): MaterialMaster[];
  getMaterialsByRank(rank: GuildRank): MaterialMaster[];
}
```

### 2.4 MaterialService実装 🔵

`src/application/services/material-service.ts`:

- 素材インスタンス生成（ユニークID付与）
- 品質のランダム生成（±1段階の変動）
- 平均品質計算

---

## 3. 受け入れ基準

### 3.1 必須条件 🔵

- [ ] 素材インスタンスが生成できる
- [ ] 品質比較が正しく動作する
- [ ] 平均品質計算が正しい

### 3.2 推奨条件 🟡

- [ ] 品質変動ロジックのカスタマイズ
- [ ] 単体テストカバレッジ80%以上

---

## 4. テストケース

### 4.1 単体テスト

| テストID | テスト内容 | 期待結果 |
|---------|----------|----------|
| T-0010-01 | 素材インスタンス生成 | 正しいプロパティ |
| T-0010-02 | 品質比較 | S > A > B > C > D |
| T-0010-03 | 平均品質（同一品質） | 入力と同じ品質 |
| T-0010-04 | 平均品質（混合） | 正しい平均 |
| T-0010-05 | ランダム品質生成 | 基準±1以内 |

---

## 5. 成果物

| 成果物 | パス |
|--------|------|
| 素材インスタンス | `/src/domain/entities/MaterialInstance.ts` |
| 品質値オブジェクト | `/src/domain/value-objects/Quality.ts` |
| インターフェース | `/src/domain/interfaces/material-service.interface.ts` |
| 実装 | `/src/application/services/material-service.ts` |
| テスト | `/tests/unit/application/services/material-service.test.ts` |

---

## 関連文書

- **データスキーマ**: [../../design/atelier-guild-rank/data-schema-master-game.md](../../design/atelier-guild-rank/data-schema-master-game.md)

---

## 変更履歴

| 日付 | バージョン | 変更内容 |
|------|----------|---------|
| 2026-01-16 | 1.0.0 | 初版作成 |

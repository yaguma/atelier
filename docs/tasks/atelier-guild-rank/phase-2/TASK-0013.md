# TASK-0013: 依頼エンティティ・QuestService実装

**バージョン**: 1.0.0
**作成日**: 2026-01-16
**フェーズ**: 2 - ドメイン層・コアサービス
**見積時間**: 4時間（半日）
**依存タスク**: TASK-0003, TASK-0004

---

## 1. タスク概要

| 項目 | 値 |
|------|-----|
| **タスクID** | TASK-0013 |
| **タスク名** | 依頼エンティティ・QuestService実装 |
| **カテゴリ** | ドメイン層 |
| **優先度** | 最高 |
| **担当レイヤー** | Domain, Application |

### 信頼性レベル

- 🔵 **青信号**: 設計文書に記載
- 🟡 **黄信号**: 設計文書から妥当な推測
- 🔴 **赤信号**: 設計文書にない推測

---

## 2. 実装内容

### 2.1 依頼エンティティ 🔵

`src/domain/entities/Quest.ts`:

```typescript
export class Quest {
  constructor(
    public readonly id: QuestId,
    public readonly client: ClientMaster,
    public readonly requirements: QuestRequirement[],
    public readonly reward: QuestReward,
    public readonly deadline: number, // 残り日数
    public status: QuestStatus = 'available',
  ) {}

  get clientType(): ClientType {
    return this.client.type;
  }

  // 納品可能チェック
  canDeliver(items: ItemInstance[]): boolean {
    return this.requirements.every(req =>
      this.checkRequirement(req, items)
    );
  }

  private checkRequirement(
    req: QuestRequirement,
    items: ItemInstance[],
  ): boolean {
    const matching = items.filter(item =>
      item.itemId === req.itemId &&
      compareQuality(item.quality, req.minQuality) >= 0
    );
    return matching.length >= req.quantity;
  }
}

export type QuestStatus = 'available' | 'accepted' | 'completed' | 'failed';

export interface QuestRequirement {
  itemId: ItemId;
  quantity: number;
  minQuality: Quality;
}

export interface QuestReward {
  gold: number;
  contribution: number;
}
```

### 2.2 QuestServiceインターフェース 🔵

`src/domain/interfaces/quest-service.interface.ts`:

```typescript
export interface IQuestService {
  // 依頼生成
  generateDailyQuests(rank: GuildRank): Quest[];

  // 依頼受注
  acceptQuest(questId: QuestId): void;

  // 納品
  deliver(questId: QuestId, items: ItemInstance[]): DeliveryResult;

  // 状態取得
  getAvailableQuests(): Quest[];
  getAcceptedQuests(): Quest[];

  // 条件チェック
  checkDeliveryCondition(questId: QuestId, items: ItemInstance[]): boolean;

  // 日終了処理
  processEndOfDay(): FailedQuest[];
}

export interface DeliveryResult {
  success: boolean;
  reward: QuestReward;
  consumedItems: ItemInstance[];
  contribution: number;
}
```

### 2.3 QuestService実装 🔵

`src/application/services/quest-service.ts`:

- 日次依頼生成（ランクに応じた難易度）
- 依頼受注管理
- 納品処理と報酬計算
- 期限切れ処理

### 2.4 依頼生成ロジック 🔵

- ランクに応じた依頼者プール
- 要求アイテムの難易度調整
- 報酬の自動計算

---

## 3. 受け入れ基準

### 3.1 必須条件 🔵

- [x] 日次依頼が生成される
- [x] 依頼を受注できる
- [x] 条件を満たすアイテムで納品できる
- [x] 報酬が正しく計算される

### 3.2 推奨条件 🟡

- [x] 期限切れ依頼の自動失敗処理
- [x] 単体テストカバレッジ80%以上

---

## 4. テストケース

### 4.1 単体テスト

| テストID | テスト内容 | 期待結果 |
|---------|----------|----------|
| T-0013-01 | 日次依頼生成 | ランクに応じた依頼 |
| T-0013-02 | 依頼受注 | ステータス変更 |
| T-0013-03 | 納品成功 | 報酬獲得、依頼完了 |
| T-0013-04 | 納品失敗（条件未達） | エラー発生 |
| T-0013-05 | 期限切れ処理 | 依頼失敗 |

---

## 5. 成果物

| 成果物 | パス |
|--------|------|
| 依頼エンティティ | `/src/domain/entities/Quest.ts` |
| インターフェース | `/src/domain/interfaces/quest-service.interface.ts` |
| 実装 | `/src/application/services/quest-service.ts` |
| テスト | `/tests/unit/application/services/quest-service.test.ts` |

---

## 関連文書

- **ゲームメカニクス設計**: [../../design/atelier-guild-rank/game-mechanics.md](../../design/atelier-guild-rank/game-mechanics.md)
- **コアサービス設計**: [../../design/atelier-guild-rank/core-systems-core-services.md](../../design/atelier-guild-rank/core-systems-core-services.md)

---

## 変更履歴

| 日付 | バージョン | 変更内容 |
|------|----------|---------|
| 2026-01-16 | 1.0.0 | 初版作成 |
| 2026-01-18 | 1.0.1 | 受け入れ基準更新（実装完了確認） |

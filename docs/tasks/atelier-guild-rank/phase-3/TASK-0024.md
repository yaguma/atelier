# TASK-0024: 調合フェーズUI

**バージョン**: 1.0.0
**作成日**: 2026-01-16
**フェーズ**: 3 - UI層
**見積時間**: 4時間（半日）
**依存タスク**: TASK-0020, TASK-0021, TASK-0012

---

## 1. タスク概要

| 項目 | 値 |
|------|-----|
| **タスクID** | TASK-0024 |
| **タスク名** | 調合フェーズUI |
| **カテゴリ** | UI層 |
| **優先度** | 最高 |
| **担当レイヤー** | Presentation |

### 信頼性レベル

- 🔵 **青信号**: 設計文書に記載
- 🟡 **黄信号**: 設計文書から妥当な推測
- 🔴 **赤信号**: 設計文書にない推測

---

## 2. 実装内容

### 2.1 調合画面レイアウト 🔵

```
┌─────────────────────────────────────────────────────────────────┐
│                    🧪 調合フェーズ                              │
│                                                                 │
│  ┌─────────────────────┐    ┌─────────────────────────────────┐│
│  │    レシピ一覧       │    │       調合エリア               ││
│  │                     │    │                                 ││
│  │  ▶ 回復薬          │    │   素材スロット:                 ││
│  │    植物×1          │    │   [   ] + [   ] + [   ]         ││
│  │                     │    │                                 ││
│  │  ▷ 爆弾            │    │   ─────────────────────         ││
│  │    鉱物×1, 火薬×1   │    │                                 ││
│  │                     │    │   完成予測: 回復薬 (B品質)      ││
│  │  ▷ 鋼の剣          │    │                                 ││
│  │    鉱物×2          │    │   [調合する]                    ││
│  │                     │    │                                 ││
│  └─────────────────────┘    └─────────────────────────────────┘│
│                                                                 │
│  所持素材: [薬草C] [薬草B] [鉄鉱A] [花D]                         │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 2.2 AlchemyPhaseUI 🔵

`src/presentation/ui/phases/AlchemyPhaseUI.ts`:

```typescript
export class AlchemyPhaseUI extends BaseComponent {
  private recipeList!: ScrollablePanel;
  private materialSlots: MaterialSlotUI[] = [];
  private previewText!: Phaser.GameObjects.Text;
  private craftButton!: Button;

  private selectedRecipe: ItemMaster | null = null;
  private selectedMaterials: MaterialInstance[] = [];

  constructor(
    scene: Phaser.Scene,
    private alchemyService: IAlchemyService,
    private inventoryService: IInventoryService,
  ) {
    super(scene, 160, 80);
  }

  create(): void {
    this.createTitle();
    this.createRecipeList();
    this.createAlchemyArea();
    this.createMaterialInventory();
    this.createCraftButton();
  }

  private createRecipeList(): void {
    // 調合可能なレシピをリスト表示
    const recipes = this.alchemyService.getAvailableRecipes(
      this.inventoryService.getMaterials()
    );

    // rexUIのScrollablePanelで表示
  }

  private onRecipeSelect(recipe: ItemMaster): void {
    this.selectedRecipe = recipe;
    this.selectedMaterials = [];
    this.updateMaterialSlots();
    this.updatePreview();
  }

  private onMaterialDrop(slot: number, material: MaterialInstance): void {
    this.selectedMaterials[slot] = material;
    this.updatePreview();
  }

  private updatePreview(): void {
    if (!this.selectedRecipe || !this.canCraft()) {
      this.previewText.setText('素材を選択してください');
      this.craftButton.setEnabled(false);
      return;
    }

    const quality = this.alchemyService.previewQuality(
      this.selectedRecipe.id,
      this.selectedMaterials
    );
    this.previewText.setText(
      `完成予測: ${this.selectedRecipe.name} (${quality}品質)`
    );
    this.craftButton.setEnabled(true);
  }

  private onCraft(): void {
    const item = this.alchemyService.craft(
      this.selectedRecipe!.id,
      this.selectedMaterials
    );
    // 素材消費、アイテム追加
    this.showCraftResult(item);
    this.reset();
  }
}
```

### 2.3 レシピ選択リスト 🔵

- 調合可能レシピのフィルタリング
- 必要素材の表示
- 選択状態の強調

### 2.4 素材ドラッグ＆ドロップ 🟡

- インベントリから調合スロットへドラッグ
- スロット間の素材入れ替え
- ダブルクリックで自動配置

---

## 3. 受け入れ基準

### 3.1 必須条件 🔵

- [x] レシピ一覧が表示される
- [x] 素材を配置できる
- [x] 完成品質がプレビューされる
- [x] 調合が実行できる

### 3.2 推奨条件 🟡

- [x] ドラッグ＆ドロップ
- [x] 調合エフェクト

---

## 4. テストケース

### 4.1 統合テスト

| テストID | テスト内容 | 期待結果 |
|---------|----------|----------|
| T-0024-01 | レシピ表示 | 調合可能レシピ表示 |
| T-0024-02 | 素材配置 | スロットに配置 |
| T-0024-03 | 品質プレビュー | 正しい品質表示 |
| T-0024-04 | 調合実行 | アイテム生成、素材消費 |

---

## 5. 成果物

| 成果物 | パス |
|--------|------|
| フェーズUI | `/src/presentation/ui/phases/AlchemyPhaseUI.ts` |
| レシピリスト | `/src/presentation/ui/components/RecipeListUI.ts` |

---

## 関連文書

- **調合フェーズ設計**: [../../design/atelier-guild-rank/ui-design/screens/alchemy.md](../../design/atelier-guild-rank/ui-design/screens/alchemy.md)

---

## 変更履歴

| 日付 | バージョン | 変更内容 |
|------|----------|---------|
| 2026-01-16 | 1.0.0 | 初版作成 |
| 2026-01-18 | 1.0.1 | 受け入れ基準更新（実装完了確認） |

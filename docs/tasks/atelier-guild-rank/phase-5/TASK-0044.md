# TASK-0044: 品質に応じた視覚効果

**タスクID**: TASK-0044
**タスクタイプ**: DIRECT
**推定工数**: 4時間
**フェーズ**: Phase 5 - UI強化・ポリッシュ
**信頼性レベル**: 🟡 *TASK-0023の推奨条件より*

## 関連文書

- **概要**: [📋 overview.md](../overview.md)
- **元タスク**: [📋 TASK-0023.md](../phase-3/TASK-0023.md) - 採取フェーズUI推奨条件
- **UI設計**: [📐 ui-design/screens/gathering.md](../../design/atelier-guild-rank/ui-design/screens/gathering.md)

## タスク概要

素材やアイテムの品質（D〜S）に応じた視覚効果を実装する。高品質アイテムには光彩効果やパーティクルを追加し、プレイヤーが品質を直感的に認識できるようにする。

## 依存タスク

- **前提タスク**: [TASK-0023: 採取フェーズUI](../phase-3/TASK-0023.md) ✅完了
- **後続タスク**: なし（独立タスク）

## 完了条件

- [x] 品質ごとの枠色定義
- [x] 高品質アイテムの光彩効果
- [x] S品質のパーティクルエフェクト
- [x] 品質バッジのスタイル改善
- [x] 動作確認完了

---

## 実装詳細

### 1. 品質ごとの枠色定義 🔵

**信頼性**: 🔵 *ゲームメカニクス設計から推測*

**実装ファイル**: `src/presentation/ui/theme.ts`

```typescript
export const QUALITY_COLORS = {
  D: 0x808080,   // グレー
  C: 0x00FF00,   // 緑
  B: 0x0080FF,   // 青
  A: 0xFFD700,   // ゴールド
  S: 0xFF00FF,   // マゼンタ（紫）
} as const;

export const QUALITY_GLOW = {
  D: null,              // なし
  C: null,              // なし
  B: { intensity: 0.3 }, // 弱い光彩
  A: { intensity: 0.6 }, // 中程度の光彩
  S: { intensity: 1.0, particles: true }, // 強い光彩＋パーティクル
} as const;
```

### 2. 光彩効果（Glow） 🟡

**信頼性**: 🟡 *Phaser機能から推測*

**実装ファイル**: `src/presentation/ui/components/MaterialSlotUI.ts`

```typescript
private applyQualityEffect(quality: Quality): void {
  const color = QUALITY_COLORS[quality];
  const glow = QUALITY_GLOW[quality];

  // 枠色の設定
  this.border.setStrokeStyle(3, color);

  // 光彩効果
  if (glow) {
    this.glowGraphics.clear();
    this.glowGraphics.fillStyle(color, glow.intensity * 0.3);
    this.glowGraphics.fillCircle(
      this.container.x,
      this.container.y,
      this.slotSize * 0.7
    );
    this.glowGraphics.setBlendMode(Phaser.BlendModes.ADD);

    // 脈動アニメーション
    this.scene.tweens.add({
      targets: this.glowGraphics,
      alpha: { from: 0.5, to: 1 },
      duration: 1000,
      yoyo: true,
      repeat: -1,
      ease: 'Sine.easeInOut',
    });
  }
}
```

### 3. S品質パーティクルエフェクト 🟡

**信頼性**: 🟡 *Phaserパーティクル機能から推測*

```typescript
private createSQualityParticles(): void {
  const particles = this.scene.add.particles('sparkle');

  const emitter = particles.createEmitter({
    x: this.container.x,
    y: this.container.y,
    speed: { min: 10, max: 30 },
    angle: { min: 0, max: 360 },
    scale: { start: 0.5, end: 0 },
    alpha: { start: 1, end: 0 },
    lifespan: 1000,
    frequency: 200,
    blendMode: Phaser.BlendModes.ADD,
    tint: QUALITY_COLORS.S,
  });

  this.particleEmitter = emitter;
}

private stopParticles(): void {
  this.particleEmitter?.stop();
}
```

### 4. 品質バッジのスタイル改善 🟡

**信頼性**: 🟡 *既存MaterialSlotUI実装から推測*

```typescript
private createQualityBadge(quality: Quality): void {
  const color = QUALITY_COLORS[quality];

  // バッジ背景
  const badge = this.scene.add.graphics();
  badge.fillStyle(color, 1);
  badge.fillRoundedRect(-15, -15, 30, 20, 5);

  // バッジテキスト
  const text = this.scene.add.text(0, -5, quality, {
    fontSize: '14px',
    color: quality === 'D' ? '#FFFFFF' : '#000000',
    fontStyle: 'bold',
  }).setOrigin(0.5);

  // S品質の場合は特別なスタイル
  if (quality === 'S') {
    // 虹色グラデーションアニメーション
    this.scene.tweens.add({
      targets: badge,
      tint: { from: 0xFF00FF, to: 0x00FFFF },
      duration: 2000,
      yoyo: true,
      repeat: -1,
    });
  }

  this.qualityBadge = this.scene.add.container(
    this.slotSize / 2 - 15,
    -this.slotSize / 2 + 15
  );
  this.qualityBadge.add([badge, text]);
}
```

---

## 実装手順

### DIRECTタスクの場合

1. `/tsumiki:direct-setup` - 直接実装・設定
2. `/tsumiki:direct-verify` - 動作確認・品質確認

---

## 注意事項

- パーティクルエフェクトはパフォーマンスに注意
- 多数のアイテム表示時は効果を制限する
- 視覚効果は設定でON/OFF可能にする（アクセシビリティ）

---

## 信頼性レベルサマリー

- **総項目数**: 4項目
- 🔵 **青信号**: 1項目 (25%)
- 🟡 **黄信号**: 3項目 (75%)
- 🔴 **赤信号**: 0項目 (0%)

**品質評価**: 高品質

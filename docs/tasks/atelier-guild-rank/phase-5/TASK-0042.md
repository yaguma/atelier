...
**信頼性レベル**: 🟡 *TASK-0021の推奨条件より*

## 関連文書

- **概要**: [📋 overview.md](../overview.md)
- **元タスク**: [📋 TASK-0021.md](../phase-3/TASK-0021.md) - カードUIコンポーネント推奨条件
- **UI設計**: [📐 ui-design/overview.md](../../design/atelier-guild-rank/ui-design/overview.md)

## タスク概要

カードUIコンポーネントにドラッグ＆ドロップ機能を追加する。手札からプレイエリアへのカード移動、調合画面での素材配置などで使用する。

## 依存タスク

- **前提タスク**: [TASK-0021: カードUIコンポーネント](../phase-3/TASK-0021.md) ✅完了
- **後続タスク**: なし（独立タスク）
...
**実装ファイル**: `src/presentation/ui/components/DraggableCardUI.ts`
...
**実装ファイル**: `src/presentation/ui/components/DropZone.ts`
...
**Given**: ドラッグ可能なカードが存在
**When**: カードをドラッグ開始
**Then**: onDragStartコールバックが呼ばれ、視覚効果が適用される

### テストケース2: ドロップ成功 🟡

**信頼性**: 🟡 *標準的なテストパターン*

**Given**: 有効なドロップゾーン上でドラッグ終了
**When**: ドロップ
**Then**: onDropコールバックがゾーン情報と共に呼ばれる

### テストケース3: ドロップ失敗（元の位置に戻る） 🟡

**信頼性**: 🟡 *標準的なテストパターン*

**Given**: ドロップゾーン外でドラッグ終了
**When**: ドロップ

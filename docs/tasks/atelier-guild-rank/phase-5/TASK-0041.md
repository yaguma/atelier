# TASK-0041: ツールチップ表示システム

**タスクID**: TASK-0041
**タスクタイプ**: TDD
**推定工数**: 4時間
**フェーズ**: Phase 5 - UI強化・ポリッシュ
**信頼性レベル**: 🟡 *TASK-0020の推奨条件より*

## 関連文書

- **概要**: [📋 overview.md](../overview.md)
- **元タスク**: [📋 TASK-0020.md](../phase-3/TASK-0020.md) - MainScene共通レイアウト推奨条件
- **UI設計**: [📐 ui-design/overview.md](../../design/atelier-guild-rank/ui-design/overview.md)

## タスク概要

ゲーム全体で使用可能なツールチップ表示システムを実装する。ホバー時にアイテムや要素の詳細情報を表示し、ユーザーの理解を助ける。

## 依存タスク

- **前提タスク**: [TASK-0018: 共通UIコンポーネント基盤](../phase-3/TASK-0018.md) ✅完了
- **後続タスク**: なし（独立タスク）

## 完了条件

- [ ] TooltipManagerシングルトン実装
- [ ] ツールチップ表示/非表示機能
- [ ] 表示位置の自動調整（画面端対応）
- [ ] 表示遅延設定機能
- [ ] 単体テスト実装

---

## 実装詳細

### 1. TooltipManagerクラス 🟡

**信頼性**: 🟡 *標準的なUIパターンから推測*

**実装ファイル**: `src/presentation/ui/components/TooltipManager.ts`

```typescript
export interface TooltipConfig {
  content: string;
  x: number;
  y: number;
  delay?: number;
  maxWidth?: number;
}

export class TooltipManager {
  private static instance: TooltipManager;
  private scene: Phaser.Scene | null = null;
  private tooltip: Phaser.GameObjects.Container | null = null;
  private showTimeout: number | null = null;

  static getInstance(): TooltipManager {
    if (!TooltipManager.instance) {
      TooltipManager.instance = new TooltipManager();
    }
    return TooltipManager.instance;
  }

  initialize(scene: Phaser.Scene): void {
    this.scene = scene;
    this.createTooltipContainer();
  }

  show(config: TooltipConfig): void {
    if (this.showTimeout) {
      clearTimeout(this.showTimeout);
    }

    const delay = config.delay ?? 500;
    this.showTimeout = setTimeout(() => {
      this.displayTooltip(config);
    }, delay);
  }

  hide(): void {
    if (this.showTimeout) {
      clearTimeout(this.showTimeout);
      this.showTimeout = null;
    }
    this.tooltip?.setVisible(false);
  }

  private displayTooltip(config: TooltipConfig): void {
    if (!this.tooltip || !this.scene) return;

    this.updateContent(config.content, config.maxWidth);
    this.updatePosition(config.x, config.y);
    this.tooltip.setVisible(true);
  }
}
```

### 2. 表示位置の自動調整 🟡

**信頼性**: 🟡 *標準的なUIパターンから推測*

```typescript
private updatePosition(x: number, y: number): void {
  if (!this.tooltip || !this.scene) return;

  const bounds = this.tooltip.getBounds();
  const camera = this.scene.cameras.main;

  // 右端からはみ出す場合は左に表示
  if (x + bounds.width > camera.width) {
    x = x - bounds.width - 10;
  }

  // 下端からはみ出す場合は上に表示
  if (y + bounds.height > camera.height) {
    y = y - bounds.height - 10;
  }

  // 上端・左端のチェック
  x = Math.max(10, x);
  y = Math.max(10, y);

  this.tooltip.setPosition(x, y);
}
```

### 3. ツールチップコンテナ作成 🟡

**信頼性**: 🟡 *既存UIコンポーネントから推測*

```typescript
private createTooltipContainer(): void {
  if (!this.scene) return;

  this.tooltip = this.scene.add.container(0, 0);

  // 背景
  const background = this.scene.add.rectangle(
    0, 0, 100, 30,
    0x333333, 0.9
  ).setOrigin(0);

  // テキスト
  const text = this.scene.add.text(8, 8, '', {
    fontSize: '12px',
    color: '#FFFFFF',
    wordWrap: { width: 200 },
  });

  this.tooltip.add([background, text]);
  this.tooltip.setDepth(1000); // 最前面
  this.tooltip.setVisible(false);
}
```

---

## 単体テスト要件

### テストケース1: ツールチップ表示 🟡

**信頼性**: 🟡 *標準的なテストパターン*

**Given**: TooltipManagerが初期化されている
**When**: show()を呼び出す
**Then**: 指定位置にツールチップが表示される

### テストケース2: 表示位置自動調整 🟡

**信頼性**: 🟡 *標準的なテストパターン*

**Given**: 画面右端付近にツールチップを表示
**When**: show()を呼び出す
**Then**: ツールチップが画面内に収まるよう位置調整される

### テストケース3: 表示遅延 🟡

**信頼性**: 🟡 *標準的なテストパターン*

**Given**: delay: 500msで設定
**When**: show()を呼び出し、300ms後にhide()
**Then**: ツールチップは表示されない

---

## 実装手順

### TDDタスクの場合

1. `/tsumiki:tdd-requirements TASK-0041` - 詳細要件定義
2. `/tsumiki:tdd-testcases` - テストケース作成
3. `/tsumiki:tdd-red` - テスト実装（失敗）
4. `/tsumiki:tdd-green` - 最小実装
5. `/tsumiki:tdd-refactor` - リファクタリング
6. `/tsumiki:tdd-verify-complete` - 品質確認

---

## 注意事項

- シングルトンパターンでシーン間で再利用可能にする
- Zインデックスを最大値に設定して最前面表示
- パフォーマンスを考慮し、不要なDOM操作を避ける

---

## 信頼性レベルサマリー

### 項目別信頼性

| カテゴリ | 🔵 青 | 🟡 黄 | 🔴 赤 | 合計 |
|---------|-------|-------|-------|------|
| 実装詳細 | 0 | 3 | 0 | 3 |
| 単体テスト | 0 | 3 | 0 | 3 |

### 全体評価

- **総項目数**: 6項目
- 🔵 **青信号**: 0項目 (0%)
- 🟡 **黄信号**: 6項目 (100%)
- 🔴 **赤信号**: 0項目 (0%)

**品質評価**: 高品質

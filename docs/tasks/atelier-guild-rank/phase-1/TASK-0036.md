...
- [x] Vitest 2.xがインストールされている（v4.0にアップグレード）
- [x] vitest.config.tsが設定済み
- [x] カバレッジ計測が設定済み（@vitest/coverage-v8）
- [x] `pnpm test` でテストが実行可能
- [x] `pnpm test:coverage` でカバレッジレポートが生成される
- [x] サンプルテストが正常に動作する

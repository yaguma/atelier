# TASK-0004: イベントバス実装

**バージョン**: 1.0.0
**作成日**: 2026-01-16
**フェーズ**: 1 - 基盤構築
**見積時間**: 4時間（半日）
**依存タスク**: TASK-0003

---

## 1. タスク概要

| 項目 | 値 |
|------|-----|
| **タスクID** | TASK-0004 |
| **タスク名** | イベントバス実装 |
| **カテゴリ** | 基盤構築 |
| **優先度** | 最高 |
| **担当レイヤー** | Application |

### 信頼性レベル

- 🔵 **青信号**: 設計文書に記載
- 🟡 **黄信号**: 設計文書から妥当な推測
- 🔴 **赤信号**: 設計文書にない推測

---

## 2. 実装内容

### 2.1 イベント型定義 🔵

`src/application/events/event-types.ts`:

```typescript
export enum GameEventType {
  // 状態変更イベント
  STATE_CHANGED = 'state:changed',
  PHASE_CHANGED = 'phase:changed',
  DAY_STARTED = 'day:started',
  DAY_ENDED = 'day:ended',

  // カードイベント
  CARD_DRAWN = 'card:drawn',
  CARD_PLAYED = 'card:played',
  HAND_REFILLED = 'hand:refilled',

  // 採取イベント
  GATHERING_STARTED = 'gathering:started',
  MATERIAL_SELECTED = 'material:selected',
  GATHERING_COMPLETED = 'gathering:completed',

  // 調合イベント
  ALCHEMY_STARTED = 'alchemy:started',
  ITEM_CRAFTED = 'item:crafted',

  // 依頼イベント
  QUEST_ACCEPTED = 'quest:accepted',
  QUEST_DELIVERED = 'quest:delivered',
  QUEST_FAILED = 'quest:failed',

  // ランクイベント
  RANK_PROMOTED = 'rank:promoted',
  PROMOTION_TEST_STARTED = 'promotion:test:started',

  // UI イベント
  UI_DIALOG_OPENED = 'ui:dialog:opened',
  UI_DIALOG_CLOSED = 'ui:dialog:closed',
}

export interface GameEvent<T = unknown> {
  type: GameEventType;
  payload: T;
  timestamp: number;
}
```

### 2.2 イベントバスインターフェース 🔵

`src/application/events/event-bus.interface.ts`:

```typescript
export interface IEventBus {
  emit<T>(type: GameEventType, payload: T): void;
  on<T>(type: GameEventType, handler: (event: GameEvent<T>) => void): () => void;
  once<T>(type: GameEventType, handler: (event: GameEvent<T>) => void): void;
  off(type: GameEventType, handler: Function): void;
}
```

### 2.3 イベントバス実装 🔵

`src/application/events/event-bus.ts`:

```typescript
export class EventBus implements IEventBus {
  private handlers: Map<GameEventType, Set<Function>> = new Map();

  emit<T>(type: GameEventType, payload: T): void {
    const event: GameEvent<T> = {
      type,
      payload,
      timestamp: Date.now(),
    };

    const typeHandlers = this.handlers.get(type);
    if (typeHandlers) {
      typeHandlers.forEach(handler => handler(event));
    }
  }

  on<T>(type: GameEventType, handler: (event: GameEvent<T>) => void): () => void {
    if (!this.handlers.has(type)) {
      this.handlers.set(type, new Set());
    }
    this.handlers.get(type)!.add(handler);

    return () => this.off(type, handler);
  }

  once<T>(type: GameEventType, handler: (event: GameEvent<T>) => void): void {
    const onceHandler = (event: GameEvent<T>) => {
      handler(event);
      this.off(type, onceHandler);
    };
    this.on(type, onceHandler);
  }

  off(type: GameEventType, handler: Function): void {
    this.handlers.get(type)?.delete(handler);
  }
}
```

---

## 3. 受け入れ基準

### 3.1 必須条件 🔵

- [x] emit/on/once/offメソッドが動作する
- [x] 型安全なイベントハンドリングができる
- [x] unsubscribe関数で購読解除できる

### 3.2 推奨条件 🟡

- [x] イベント履歴のデバッグログ出力機能
- [x] 単体テストカバレッジ80%以上

---

## 4. テストケース

### 4.1 単体テスト

| テストID | テスト内容 | 期待結果 |
|---------|----------|----------|
| T-0004-01 | emitでハンドラーが呼ばれる | ハンドラー実行される |
| T-0004-02 | onで複数ハンドラー登録 | 全て実行される |
| T-0004-03 | onceは1回のみ実行 | 2回目は実行されない |
| T-0004-04 | offで購読解除 | 解除後は実行されない |
| T-0004-05 | 戻り値のunsubscribeで解除 | 解除後は実行されない |

---

## 5. 成果物

| 成果物 | パス |
|--------|------|
| イベント型定義 | `/src/application/events/event-types.ts` |
| インターフェース | `/src/application/events/event-bus.interface.ts` |
| 実装 | `/src/application/events/event-bus.ts` |
| テスト | `/tests/unit/application/events/event-bus.test.ts` |

---

## 関連文書

- **アーキテクチャ設計書**: [../../design/atelier-guild-rank/architecture-overview.md](../../design/atelier-guild-rank/architecture-overview.md)
- **データフロー設計書**: [../../design/atelier-guild-rank/dataflow.md](../../design/atelier-guild-rank/dataflow.md)

---

## 変更履歴

| 日付 | バージョン | 変更内容 |
|------|----------|---------|
| 2026-01-16 | 1.0.0 | 初版作成 |
| 2026-01-18 | 1.0.1 | 受け入れ基準更新（実装完了確認） |

# TASK-0037: Playwright E2E設定

**タスクID**: TASK-0037
**タスクタイプ**: DIRECT
**推定工数**: 4時間
**フェーズ**: Phase 1 - 基盤構築
**信頼性レベル**: 🔵 *architecture-overview.md v2.2.0より*

## 関連文書

- **概要**: [📋 overview.md](../overview.md)
- **設計文書**: [📐 architecture-overview.md](../../../design/atelier-guild-rank/architecture-overview.md)

## タスク概要

Playwrightを設定し、E2E（End-to-End）テスト環境を構築する。ブラウザでの実際のゲーム動作を検証し、ユーザー体験の品質を保証する。

## 依存タスク

- **前提タスク**: [TASK-0033: pnpm移行とBiome設定](TASK-0033.md), [TASK-0008: Phaser基本設定とBootScene](TASK-0008.md)
- **後続タスク**: [TASK-0030: E2Eテスト・デバッグ](../phase-4/TASK-0030.md)

## 完了条件

- [ ] Playwrightがインストールされている
- [ ] playwright.config.tsが設定済み
- [ ] e2e/ディレクトリが作成されている
- [ ] `pnpm test:e2e` でE2Eテストが実行可能
- [ ] サンプルE2Eテストが正常に動作する
- [ ] Chromiumブラウザでテストが実行される

---

## 実装詳細

### 1. Playwrightインストール 🔵

**信頼性**: 🔵 *architecture-overview.md セクション1.3より*

```bash
pnpm add -D @playwright/test
pnpm exec playwright install chromium
```

### 2. playwright.config.ts作成 🔵

**信頼性**: 🔵 *architecture-overview.md セクション1.3より*

```typescript
// playwright.config.ts
import { defineConfig, devices } from '@playwright/test';

export default defineConfig({
  testDir: './e2e',
  fullyParallel: true,
  forbidOnly: !!process.env.CI,
  retries: process.env.CI ? 2 : 0,
  workers: process.env.CI ? 1 : undefined,
  reporter: [
    ['html', { outputFolder: 'playwright-report' }],
    ['list'],
  ],
  use: {
    baseURL: 'http://localhost:5173',
    trace: 'on-first-retry',
    screenshot: 'only-on-failure',
    video: 'on-first-retry',
  },
  projects: [
    {
      name: 'chromium',
      use: { ...devices['Desktop Chrome'] },
    },
  ],
  webServer: {
    command: 'pnpm dev',
    url: 'http://localhost:5173',
    reuseExistingServer: !process.env.CI,
    timeout: 120 * 1000,
  },
});
```

**実装ファイル**: `playwright.config.ts`

### 3. E2Eテストディレクトリ作成 🔵

**信頼性**: 🔵 *標準的なディレクトリ構成*

```
e2e/
├── fixtures/           # テスト用フィクスチャ
│   └── game.fixture.ts
├── pages/              # ページオブジェクト
│   └── game.page.ts
└── specs/              # テストスペック
    └── boot.spec.ts
```

### 4. テストフィクスチャ作成 🔵

**信頼性**: 🔵 *Playwright標準パターン*

```typescript
// e2e/fixtures/game.fixture.ts
import { test as base, Page } from '@playwright/test';

export interface GameFixtures {
  gamePage: Page;
}

export const test = base.extend<GameFixtures>({
  gamePage: async ({ page }, use) => {
    // ゲームページに移動
    await page.goto('/');
    // Phaserキャンバスが読み込まれるまで待機
    await page.waitForSelector('#game-container canvas', { timeout: 10000 });
    await use(page);
  },
});

export { expect } from '@playwright/test';
```

**実装ファイル**: `e2e/fixtures/game.fixture.ts`

### 5. ページオブジェクト作成 🔵

**信頼性**: 🔵 *Page Object Model パターン*

```typescript
// e2e/pages/game.page.ts
import { Page, Locator } from '@playwright/test';

export class GamePage {
  readonly page: Page;
  readonly canvas: Locator;
  readonly gameContainer: Locator;

  constructor(page: Page) {
    this.page = page;
    this.gameContainer = page.locator('#game-container');
    this.canvas = page.locator('#game-container canvas');
  }

  async waitForGameLoad(): Promise<void> {
    await this.canvas.waitFor({ state: 'visible', timeout: 10000 });
  }

  async getCanvasSize(): Promise<{ width: number; height: number }> {
    const boundingBox = await this.canvas.boundingBox();
    return {
      width: boundingBox?.width ?? 0,
      height: boundingBox?.height ?? 0,
    };
  }

  async clickCanvas(x: number, y: number): Promise<void> {
    await this.canvas.click({ position: { x, y } });
  }

  async takeScreenshot(name: string): Promise<void> {
    await this.page.screenshot({ path: `e2e/screenshots/${name}.png` });
  }
}
```

**実装ファイル**: `e2e/pages/game.page.ts`

### 6. サンプルE2Eテスト作成 🔵

**信頼性**: 🔵 *動作確認用*

```typescript
// e2e/specs/boot.spec.ts
import { test, expect } from '../fixtures/game.fixture';
import { GamePage } from '../pages/game.page';

test.describe('Game Boot', () => {
  test('should load the game canvas', async ({ gamePage }) => {
    const game = new GamePage(gamePage);
    await game.waitForGameLoad();

    // キャンバスが表示されていることを確認
    await expect(game.canvas).toBeVisible();
  });

  test('should have correct canvas size', async ({ gamePage }) => {
    const game = new GamePage(gamePage);
    await game.waitForGameLoad();

    const size = await game.getCanvasSize();
    // 設計書の解像度に基づく（1280x720または適切なサイズ）
    expect(size.width).toBeGreaterThan(0);
    expect(size.height).toBeGreaterThan(0);
  });

  test('should not have console errors on boot', async ({ gamePage }) => {
    const errors: string[] = [];
    gamePage.on('console', (msg) => {
      if (msg.type() === 'error') {
        errors.push(msg.text());
      }
    });

    const game = new GamePage(gamePage);
    await game.waitForGameLoad();

    // 致命的なエラーがないことを確認
    expect(errors.filter((e) => !e.includes('warning'))).toHaveLength(0);
  });
});
```

**実装ファイル**: `e2e/specs/boot.spec.ts`

### 7. package.jsonスクリプト追加 🔵

**信頼性**: 🔵 *標準的なスクリプト設定*

```json
{
  "scripts": {
    "test:e2e": "playwright test",
    "test:e2e:ui": "playwright test --ui",
    "test:e2e:headed": "playwright test --headed",
    "test:e2e:report": "playwright show-report"
  }
}
```

**実装ファイル**: `package.json`

### 8. .gitignore更新 🟡

**信頼性**: 🟡 *プロジェクト状況による*

```gitignore
# Playwright
playwright-report/
test-results/
e2e/screenshots/
```

---

## テスト要件

### 動作確認テスト 🔵

**信頼性**: 🔵 *標準的な動作確認*

1. **E2Eテスト実行確認**
   - `pnpm test:e2e` でテストが実行される
   - `pnpm test:e2e:ui` でUI付きで実行される
   - `pnpm test:e2e:headed` でブラウザが表示される

2. **レポート確認**
   - `pnpm test:e2e:report` でHTMLレポートが表示される
   - 失敗時にスクリーンショットが保存される

3. **開発サーバー連携確認**
   - webServerオプションで開発サーバーが自動起動される
   - テスト終了後にサーバーが適切に終了する

---

## 実装手順

### DIRECTタスク

1. `/tsumiki:direct-setup` - Playwright設定
2. `/tsumiki:direct-verify` - 動作確認

---

## 注意事項

- Phaserゲームはキャンバス内にUIを描画するため、通常のDOM操作でのテストが困難
- キャンバス座標をクリックする形でのテストが主体となる
- 本格的なE2Eテストはフェーズ4（TASK-0030）で実施

---

## 信頼性レベルサマリー

### 項目別信頼性

| カテゴリ | 🔵 青 | 🟡 黄 | 🔴 赤 | 合計 |
|---------|-------|-------|-------|------|
| 実装詳細 | 7 | 1 | 0 | 8 |
| テスト要件 | 1 | 0 | 0 | 1 |

### 全体評価

- **総項目数**: 9項目
- 🔵 **青信号**: 8項目 (89%)
- 🟡 **黄信号**: 1項目 (11%)
- 🔴 **赤信号**: 0項目 (0%)

**品質評価**: 高品質

# TASK-0005: StateManager実装

**バージョン**: 1.0.0
**作成日**: 2026-01-16
**フェーズ**: 1 - 基盤構築
**見積時間**: 4時間（半日）
**依存タスク**: TASK-0004

---

## 1. タスク概要

| 項目 | 値 |
|------|-----|
| **タスクID** | TASK-0005 |
| **タスク名** | StateManager実装 |
| **カテゴリ** | 基盤構築 |
| **優先度** | 最高 |
| **担当レイヤー** | Application |

### 信頼性レベル

- 🔵 **青信号**: 設計文書に記載
- 🟡 **黄信号**: 設計文書から妥当な推測
- 🔴 **赤信号**: 設計文書にない推測

---

## 2. 実装内容

### 2.1 StateManagerインターフェース 🔵

`src/application/services/state-manager.interface.ts`:

```typescript
export interface IStateManager {
  // 状態取得
  getState(): GameState;

  // 状態更新
  updateState(partial: Partial<GameState>): void;

  // フェーズ管理
  setPhase(phase: GamePhase): void;
  canTransitionTo(phase: GamePhase): boolean;

  // 日進行
  advanceDay(): void;

  // リソース管理
  spendActionPoints(amount: number): boolean;
  addGold(amount: number): void;
  spendGold(amount: number): boolean;

  // 昇格ゲージ
  addContribution(amount: number): void;

  // 初期化・リセット
  initialize(initialState?: Partial<GameState>): void;
  reset(): void;

  // セーブ/ロード用
  loadFromSaveData(saveData: SaveData): void;
  exportToSaveData(): SaveData;
}
```

### 2.2 初期状態定義 🔵

`src/application/services/initial-state.ts`:

```typescript
export const INITIAL_GAME_STATE: GameState = {
  currentRank: 'G',
  promotionGauge: 0,
  remainingDays: 30,
  gold: 100,
  actionPoints: 3,
  maxActionPoints: 3,
  currentPhase: 'quest_accept',
  currentDay: 1,
};
```

### 2.3 StateManager実装 🔵

`src/application/services/state-manager.ts`:

- 状態のイミュータブル管理
- フェーズ遷移の検証ロジック
- イベントバスとの連携（STATE_CHANGED, PHASE_CHANGED等）
- アクションポイント消費時のバリデーション

### 2.4 フェーズ遷移ルール 🔵

```typescript
const VALID_TRANSITIONS: Record<GamePhase, GamePhase[]> = {
  'quest_accept': ['gathering'],
  'gathering': ['alchemy'],
  'alchemy': ['delivery'],
  'delivery': ['quest_accept'], // 日終了後
};
```

---

## 3. 受け入れ基準

### 3.1 必須条件 🔵

- [x] 状態の取得・更新ができる
- [x] フェーズ遷移が正しく制御される
- [x] 不正なフェーズ遷移はエラーになる
- [x] リソース操作が正しく動作する

### 3.2 推奨条件 🟡

- [x] 状態変更時にイベントが発火する
- [x] 単体テストカバレッジ80%以上

---

## 4. テストケース

### 4.1 単体テスト

| テストID | テスト内容 | 期待結果 |
|---------|----------|----------|
| T-0005-01 | 初期状態の取得 | INITIAL_STATE返却 |
| T-0005-02 | 状態更新 | 部分更新が反映 |
| T-0005-03 | 有効なフェーズ遷移 | 遷移成功 |
| T-0005-04 | 無効なフェーズ遷移 | エラー発生 |
| T-0005-05 | AP消費（十分な場合） | true、AP減少 |
| T-0005-06 | AP消費（不足時） | false、AP変化なし |
| T-0005-07 | ゴールド追加 | 正しく加算 |
| T-0005-08 | ゴールド消費 | 正しく減算 |
| T-0005-09 | 日の進行 | day+1、AP回復 |

---

## 5. 成果物

| 成果物 | パス |
|--------|------|
| インターフェース | `/src/application/services/state-manager.interface.ts` |
| 初期状態 | `/src/application/services/initial-state.ts` |
| 実装 | `/src/application/services/state-manager.ts` |
| テスト | `/tests/unit/application/services/state-manager.test.ts` |

---

## 関連文書

- **インターフェース定義**: [../../design/atelier-guild-rank/interfaces.ts](../../design/atelier-guild-rank/interfaces.ts)
- **データフロー設計書**: [../../design/atelier-guild-rank/dataflow.md](../../design/atelier-guild-rank/dataflow.md)

---

## 変更履歴

| 日付 | バージョン | 変更内容 |
|------|----------|---------|
| 2026-01-16 | 1.0.0 | 初版作成 |
| 2026-01-18 | 1.0.1 | 受け入れ基準更新（実装完了確認） |

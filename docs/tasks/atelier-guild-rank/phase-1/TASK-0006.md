# TASK-0006: マスターデータローダー実装

**バージョン**: 1.0.0
**作成日**: 2026-01-16
**フェーズ**: 1 - 基盤構築
**見積時間**: 4時間（半日）
**依存タスク**: TASK-0003

---

## 1. タスク概要

| 項目 | 値 |
|------|-----|
| **タスクID** | TASK-0006 |
| **タスク名** | マスターデータローダー実装 |
| **カテゴリ** | 基盤構築 |
| **優先度** | 高 |
| **担当レイヤー** | Infrastructure |

### 信頼性レベル

- 🔵 **青信号**: 設計文書に記載
- 🟡 **黄信号**: 設計文書から妥当な推測
- 🔴 **赤信号**: 設計文書にない推測

---

## 2. 実装内容

### 2.1 マスターデータリポジトリインターフェース 🔵

`src/domain/interfaces/master-data-repository.interface.ts`:

```typescript
export interface IMasterDataRepository {
  // カード
  getAllCards(): CardMaster[];
  getCardById(id: CardId): CardMaster | undefined;
  getCardsByType(type: CardType): CardMaster[];

  // 素材
  getAllMaterials(): MaterialMaster[];
  getMaterialById(id: MaterialId): MaterialMaster | undefined;
  getMaterialsByCategory(category: MaterialCategory): MaterialMaster[];

  // アイテム
  getAllItems(): ItemMaster[];
  getItemById(id: ItemId): ItemMaster | undefined;

  // ランク
  getAllRanks(): GuildRankMaster[];
  getRankByValue(rank: GuildRank): GuildRankMaster | undefined;

  // 依頼者
  getAllClients(): ClientMaster[];
  getClientById(id: ClientId): ClientMaster | undefined;

  // アーティファクト
  getAllArtifacts(): ArtifactMaster[];
  getArtifactById(id: ArtifactId): ArtifactMaster | undefined;

  // 初期化
  load(): Promise<void>;
  isLoaded(): boolean;
}
```

### 2.2 JSONローダー 🔵

`src/infrastructure/loaders/json-loader.ts`:

```typescript
export class JsonLoader {
  async load<T>(path: string): Promise<T> {
    const response = await fetch(path);
    if (!response.ok) {
      throw new Error(`Failed to load: ${path}`);
    }
    return response.json();
  }
}
```

### 2.3 マスターデータリポジトリ実装 🔵

`src/infrastructure/repositories/master-data-repository.ts`:

- 各マスターデータのJSON読み込み
- インデックス構築（ID検索用Map）
- キャッシュ機構

### 2.4 マスターデータJSON配置 🔵

```
public/assets/data/master/
├── cards.json
├── materials.json
├── items.json
├── ranks.json
├── clients.json
└── artifacts.json
```

---

## 3. 受け入れ基準

### 3.1 必須条件 🔵

- [x] 全マスターデータが読み込める
- [x] ID検索が高速に動作する
- [x] 読み込みエラー時に適切なエラーを投げる

### 3.2 推奨条件 🟡

- [x] バリデーション機能
- [x] 単体テストカバレッジ80%以上

---

## 4. テストケース

### 4.1 単体テスト

| テストID | テスト内容 | 期待結果 |
|---------|----------|----------|
| T-0006-01 | カードマスター読み込み | 全カード取得可能 |
| T-0006-02 | IDによる検索 | 正しいデータ返却 |
| T-0006-03 | 存在しないID検索 | undefined返却 |
| T-0006-04 | 読み込みエラー | エラーをthrow |
| T-0006-05 | 複数回load呼び出し | 2回目以降はキャッシュ使用 |

---

## 5. 成果物

| 成果物 | パス |
|--------|------|
| インターフェース | `/src/domain/interfaces/master-data-repository.interface.ts` |
| JSONローダー | `/src/infrastructure/loaders/json-loader.ts` |
| リポジトリ実装 | `/src/infrastructure/repositories/master-data-repository.ts` |
| マスターデータJSON | `/public/assets/data/master/*.json` |
| テスト | `/tests/unit/infrastructure/repositories/master-data-repository.test.ts` |

---

## 関連文書

- **データスキーマ**: [../../design/atelier-guild-rank/data-schema-master-game.md](../../design/atelier-guild-rank/data-schema-master-game.md)
- **アーキテクチャ設計書**: [../../design/atelier-guild-rank/architecture-overview.md](../../design/atelier-guild-rank/architecture-overview.md)

---

## 変更履歴

| 日付 | バージョン | 変更内容 |
|------|----------|---------|
| 2026-01-16 | 1.0.0 | 初版作成 |
| 2026-01-18 | 1.0.1 | 受け入れ基準更新（実装完了確認） |

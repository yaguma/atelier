...
| Phase 3 | UI層 | 13 | 52時間 | シーン、UIコンポーネント実装 |
| Phase 4 | 統合・テスト | 5 | 20時間 | 統合、テスト、デプロイ |
| Phase 5 | UI強化・ポリッシュ | 7 | 28時間 | UIアニメーション、UX改善 |
| Phase 6 | バグ修正・未実装機能 | 5 | 25時間 | テスト修正、欠損シーン実装、統合完成 |
| Phase 7 | Presentation層リファクタリング | 8 | 32時間 | 大型ファイル分割、共通ユーティリティ、型定義追加 |
...
## Phase 6: バグ修正・未実装機能（25時間）

テスト失敗の修正と未実装シーンの実装、統合完成。

| タスクID | タスク名 | 見積 | 依存 | 状態 | タイプ |
|---------|---------|-----|------|------|-------|
| [TASK-0048](./phase-6/TASK-0048.md) | テストモック修正（fillRoundedRect） | 2h | - | 🔲 | DIRECT |
| [TASK-0049](./phase-6/TASK-0049.md) | テーマテスト修正（フォント名不整合） | 1h | - | 🔲 | DIRECT |
| [TASK-0050](./phase-6/TASK-0050.md) | ShopScene実装 | 8h | TASK-0048, TASK-0049 | 🔲 | TDD |
| [TASK-0051](./phase-6/TASK-0051.md) | RankUpScene実装 | 8h | TASK-0048, TASK-0049 | 🔲 | TDD |
| [TASK-0052](./phase-6/TASK-0052.md) | MainSceneフェーズUI統合完成 | 6h | TASK-0048, TASK-0049 | 🔲 | TDD |

### Phase 6 依存関係図

```mermaid
graph TD
    T0048[TASK-0048<br>fillRoundedRectモック修正]
    T0049[TASK-0049<br>フォント名テスト修正]
    T0050[TASK-0050<br>ShopScene実装]
    T0051[TASK-0051<br>RankUpScene実装]
    T0052[TASK-0052<br>MainSceneフェーズUI統合]
...
TASK-0047完了後のテスト実行で以下の問題が発覚:

1. **テスト失敗（70件）**
   - `header-ui-visual.test.ts`: 69件 - `fillRoundedRect is not a function`
   - `theme.spec.ts`: 1件 - フォント名不整合

2. **欠損ファイル**
   - `ShopScene.ts` - TASK-0026で計画されたが未作成
   - `RankUpScene.ts` - TASK-0026で計画されたが未作成

3. **統合不完全**
   - MainSceneのフェーズUI統合が不完全

---

## Phase 7: Presentation層リファクタリング（32時間）

コードベース調査に基づくPresentation層のリファクタリング。大型ファイルの分割、共通ユーティリティ作成、型定義追加を行う。

| タスクID | タスク名 | 見積 | 依存 | 状態 | タイプ |
|---------|---------|-----|------|------|-------|
| [TASK-0053](./phase-7/TASK-0053.md) | 共通UIユーティリティ基盤作成 | 4h | - | ✅ | TDD |
| [TASK-0054](./phase-7/TASK-0054.md) | テーマ定数統一（カラー・アニメーション） | 4h | TASK-0053 | 🔲 | DIRECT |
| [TASK-0055](./phase-7/TASK-0055.md) | RankUpSceneリファクタリング | 4h | TASK-0053, TASK-0054 | 🔲 | TDD |
| [TASK-0056](./phase-7/TASK-0056.md) | ShopSceneリファクタリング | 4h | TASK-0053, TASK-0054 | 🔲 | TDD |
| [TASK-0057](./phase-7/TASK-0057.md) | DeliveryPhaseUIリファクタリング | 4h | TASK-0053, TASK-0054 | 🔲 | TDD |
| [TASK-0058](./phase-7/TASK-0058.md) | TitleSceneリファクタリング | 4h | TASK-0053, TASK-0054 | 🔲 | TDD |
| [TASK-0059](./phase-7/TASK-0059.md) | rexUI型定義追加 | 4h | TASK-0055, TASK-0056 | 🔲 | DIRECT |
| [TASK-0060](./phase-7/TASK-0060.md) | ディレクトリ構造整理 | 4h | TASK-0055〜TASK-0058 | 🔲 | DIRECT |

### Phase 7 依存関係図

```mermaid
graph TD
    T0053[TASK-0053<br>共通UIユーティリティ基盤]
    T0054[TASK-0054<br>テーマ定数統一]
    T0055[TASK-0055<br>RankUpSceneリファクタリング]
    T0056[TASK-0056<br>ShopSceneリファクタリング]
    T0057[TASK-0057<br>DeliveryPhaseUIリファクタリング]
    T0058[TASK-0058<br>TitleSceneリファクタリング]
    T0059[TASK-0059<br>rexUI型定義追加]
    T0060[TASK-0060<br>ディレクトリ構造整理]
...
コードベース調査（2026-01-23）で以下の問題が特定された：

1. **大型ファイル（800行以上）**: 7ファイル
   - RankUpScene.ts（1,011行）
   - ShopScene.ts（890行）
   - DeliveryPhaseUI.ts（884行）
   - TitleScene.ts（819行）
...
3. **コード重複**
   - 背景パネル作成、ホバーエフェクト、カラー定数など

4. **ディレクトリ構造の重複**
   - `/scenes` と `/ui/scenes` の両方に同名ファイルが存在
...
### Phase 7
- TASK-0055, TASK-0056, TASK-0057, TASK-0058（共にTASK-0054完了後）
...
| 2026-01-21 | 2.0.0 | 未実装機能発見に伴うタスク追加: TASK-0020/0024のステータスを⬜に変更、TASK-0045（調合フェーズUI再実装）・TASK-0046（MainScene共通レイアウト再実装）を追加。総タスク数44→46 |
| 2026-01-22 | 2.1.0 | 共通UIコンポーネント視覚実装タスク追加: TASK-0047（HeaderUI/SidebarUI/FooterUIの実際のGameObject生成）を追加。総タスク数46→47 |
| 2026-01-22 | 2.2.0 | Phase 6追加: バグ修正・未実装機能（5タスク、25時間）- TASK-0048～0052作成。テストモック修正、フォントテスト修正、ShopScene/RankUpScene実装、MainSceneフェーズUI統合。総タスク数47→52 |
| 2026-01-23 | 2.3.0 | Phase 7追加: Presentation層リファクタリング（8タスク、32時間）- TASK-0053～0060作成。共通UIユーティリティ、テーマ定数統一、大型ファイル分割（RankUpScene/ShopScene/DeliveryPhaseUI/TitleScene）、rexUI型定義追加、ディレクトリ構造整理。総タスク数52→60 |

# TASK-0174: UIFactoryラベル生成実装

**タスクID**: TASK-0174
**タスクタイプ**: TDD
**推定工数**: 4時間
**フェーズ**: Phase 1 - Phaser基盤・インフラ構築
**信頼性レベル**: 🟡 *妥当な推測に基づく*

## 関連文書

- **概要**: [overview.md](./overview.md)
- **コアシステム**: [core-systems.md](../../design/atelier-guild-rank-phaser/core-systems.md)

## タスク概要

UIFactoryのcreateLabel()メソッドを実装し、アイコン付きラベルやテキストラベルを生成できるようにする。

## 依存タスク

- **前提タスク**: TASK-0173
- **後続タスク**: TASK-0175, TASK-0176, TASK-0177, TASK-0178, TASK-0180

## 完了条件

- [ ] createLabel()メソッドが実装されている
- [ ] テキストのみのラベルが生成できる
- [ ] アイコン付きラベルが生成できる
- [ ] 水平/垂直配置が選択できる
- [ ] テキストスタイルがカスタマイズできる

---

## 実装詳細

### 1. createLabel実装 🟡

**信頼性**: 🟡 *rexUI.Labelを使用*

**実装ファイル**: `src/presentation/phaser/ui/UIFactory.ts`に追加

```typescript
createLabel(options: LabelOptions): Label {
  const {
    x, y, text, icon, iconSize,
    textStyle, orientation, space
  } = options;

  const labelConfig: any = {
    x, y,
    orientation: orientation ?? 'horizontal',
    space: { icon: space ?? 8 },
  };

  // アイコン
  if (icon) {
    const iconObj = this.scene.add.image(0, 0, icon);
    if (iconSize) {
      iconObj.setDisplaySize(iconSize, iconSize);
    }
    labelConfig.icon = iconObj;
  }

  // テキスト
  labelConfig.text = this.scene.add.text(
    0, 0,
    text,
    textStyle ?? TextStyles.body
  );

  const label = this.rexUI.add.label(labelConfig).layout();

  return label;
}

// 値表示用ラベル（アイコン + 値）
createValueLabel(options: {
  x: number;
  y: number;
  icon: string;
  value: number | string;
  iconSize?: number;
  valueStyle?: Phaser.Types.GameObjects.Text.TextStyle;
}): Label {
  const { x, y, icon, value, iconSize, valueStyle } = options;

  return this.createLabel({
    x, y,
    icon,
    iconSize: iconSize ?? 24,
    text: String(value),
    textStyle: valueStyle ?? TextStyles.number,
    orientation: 'horizontal',
    space: 8,
  });
}

// タイトルラベル
createTitleLabel(options: {
  x: number;
  y: number;
  text: string;
  size?: 'large' | 'medium' | 'small';
}): Phaser.GameObjects.Text {
  const { x, y, text, size } = options;

  const styleKey = size === 'large' ? 'titleLarge' :
                   size === 'small' ? 'titleSmall' : 'titleMedium';

  return this.scene.add.text(x, y, text, TextStyles[styleKey])
    .setOrigin(0.5);
}

// 説明ラベル
createDescriptionLabel(options: {
  x: number;
  y: number;
  text: string;
  width?: number;
}): Phaser.GameObjects.Text {
  const { x, y, text, width } = options;

  const style = {
    ...TextStyles.body,
    wordWrap: width ? { width } : undefined,
  };

  return this.scene.add.text(x, y, text, style);
}
```

### 2. ラベル更新メソッド 🟡

**信頼性**: 🟡 *動的更新用*

```typescript
// ラベルのテキストを更新
updateLabelText(label: Label, text: string): void {
  const textObj = (label as any).getElement('text');
  if (textObj) {
    textObj.setText(text);
    label.layout();
  }
}

// ラベルのアイコンを更新
updateLabelIcon(label: Label, iconKey: string): void {
  const iconObj = (label as any).getElement('icon');
  if (iconObj && iconObj instanceof Phaser.GameObjects.Image) {
    iconObj.setTexture(iconKey);
    label.layout();
  }
}
```

---

## 単体テスト要件

### テストケース1: テキストラベル生成 🟡

**Given**: UIFactoryインスタンスがある
**When**: createLabel({ text: 'Hello' })を呼ぶ
**Then**: テキストラベルが生成される

### テストケース2: アイコン付きラベル生成 🟡

**Given**: UIFactoryインスタンスがある
**When**: createLabel({ text: 'Gold', icon: 'icon-gold' })を呼ぶ
**Then**: アイコン付きラベルが生成される

### テストケース3: ラベル更新 🟡

**Given**: ラベルが生成されている
**When**: updateLabelText()を呼ぶ
**Then**: テキストが更新される

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0174` - 詳細要件定義
2. `/tsumiki:tdd-testcases` - テストケース作成
3. `/tsumiki:tdd-red` - テスト実装（失敗）
4. `/tsumiki:tdd-green` - 最小実装
5. `/tsumiki:tdd-refactor` - リファクタリング
6. `/tsumiki:tdd-verify-complete` - 品質確認

---

## 注意事項

- アイコン画像はプリロードされている必要がある
- 長いテキストのワードラップ処理
- layout()を呼ばないとレイアウトが更新されない

---

## 信頼性レベルサマリー

- **総項目数**: 2項目
- 🔵 **青信号**: 0項目 (0%)
- 🟡 **黄信号**: 2項目 (100%)
- 🔴 **赤信号**: 0項目 (0%)

**品質評価**: 中品質

# TASK-0167: BaseGameScene基底クラス設計

**タスクID**: TASK-0167
**タスクタイプ**: TDD
**推定工数**: 4時間
**フェーズ**: Phase 1 - Phaser基盤・インフラ構築
**信頼性レベル**: 🔴 *Phaser固有の調査が必要*

## 関連文書

- **概要**: [overview.md](./overview.md)
- **設計文書**: [architecture.md](../../design/atelier-guild-rank-phaser/architecture.md)
- **コアシステム**: [core-systems.md](../../design/atelier-guild-rank-phaser/core-systems.md)

## タスク概要

すべてのゲームシーンの基底クラスとなるBaseGameSceneを設計する。共通機能（EventBus連携、UIFactory参照、ライフサイクル管理）を提供する。

## 依存タスク

- **前提タスク**: TASK-0165
- **後続タスク**: TASK-0168

## 完了条件

- [x] BaseGameSceneの設計が完了している
- [x] EventBus統合の仕組みが設計されている
- [x] UIFactory参照の仕組みが設計されている（rexUIプラグイン参照として実装）
- [x] シーンライフサイクルフックが設計されている
- [x] クリーンアップ処理が設計されている

---

## 実装詳細

### 1. BaseGameScene設計 🔴

**信頼性**: 🔴 *Phaser.Sceneの拡張方法要調査*

**実装ファイル**: `src/presentation/phaser/scenes/BaseGameScene.ts`

```typescript
import Phaser from 'phaser';
import RexUIPlugin from 'phaser3-rex-plugins/templates/ui/ui-plugin';
import { EventBus } from '../events/EventBus';
import { UIFactory } from '../ui/UIFactory';
import { SceneKey } from '../config/SceneKeys';

export interface SceneInitData {
  [key: string]: unknown;
}

export abstract class BaseGameScene extends Phaser.Scene {
  // rexUIプラグイン参照
  protected rexUI!: RexUIPlugin;

  // EventBus参照
  protected eventBus: EventBus;

  // UIFactory参照（遅延初期化）
  protected uiFactory!: UIFactory;

  // 購読解除関数リスト
  private unsubscribers: (() => void)[] = [];

  constructor(key: SceneKey) {
    super({ key });
    this.eventBus = EventBus.getInstance();
  }

  // Phaserライフサイクル: 初期化
  init(data?: SceneInitData): void {
    this.onInit(data);
  }

  // Phaserライフサイクル: プリロード
  preload(): void {
    this.onPreload();
  }

  // Phaserライフサイクル: 作成
  create(data?: SceneInitData): void {
    // rexUIプラグイン取得
    this.rexUI = this.plugins.get('rexUI') as RexUIPlugin;

    // UIFactory初期化
    this.uiFactory = new UIFactory(this, this.rexUI);

    // イベント購読セットアップ
    this.setupEventListeners();

    // サブクラスのcreate処理
    this.onCreate(data);
  }

  // Phaserライフサイクル: 更新
  update(time: number, delta: number): void {
    this.onUpdate(time, delta);
  }

  // シーン終了時のクリーンアップ
  shutdown(): void {
    this.cleanup();
    this.onShutdown();
  }

  // --- 抽象メソッド（サブクラスで実装） ---

  protected abstract onInit(data?: SceneInitData): void;
  protected abstract onPreload(): void;
  protected abstract onCreate(data?: SceneInitData): void;
  protected abstract setupEventListeners(): void;

  // --- オプショナルフック ---

  protected onUpdate(_time: number, _delta: number): void {
    // デフォルトは何もしない
  }

  protected onShutdown(): void {
    // デフォルトは何もしない
  }

  // --- ユーティリティメソッド ---

  /**
   * イベント購読を登録し、自動クリーンアップ対象に追加
   */
  protected subscribe(unsubscriber: () => void): void {
    this.unsubscribers.push(unsubscriber);
  }

  /**
   * すべての購読を解除
   */
  protected cleanup(): void {
    this.unsubscribers.forEach(unsub => unsub());
    this.unsubscribers = [];
  }

  /**
   * シーン遷移
   */
  protected goToScene(key: SceneKey, data?: SceneInitData): void {
    this.scene.start(key, data);
  }

  /**
   * オーバーレイシーンを開く
   */
  protected openOverlay(key: SceneKey, data?: SceneInitData): void {
    this.scene.launch(key, data);
    this.scene.pause();
  }

  /**
   * オーバーレイシーンを閉じる
   */
  protected closeOverlay(key: SceneKey): void {
    this.scene.stop(key);
    this.scene.resume();
  }
}
```

### 2. シーンライフサイクル図 🔴

**信頼性**: 🔴 *Phaserの正確なライフサイクル要確認*

```
┌─────────────────────────────────────────────────────────┐
│                    Scene Lifecycle                       │
├─────────────────────────────────────────────────────────┤
│  constructor() → シーンキー設定、EventBus取得            │
│       ↓                                                  │
│  init(data) → onInit() サブクラス初期化                  │
│       ↓                                                  │
│  preload() → onPreload() アセット読み込み                │
│       ↓                                                  │
│  create(data) → rexUI取得 → UIFactory作成               │
│              → setupEventListeners() イベント購読        │
│              → onCreate() サブクラスUI構築               │
│       ↓                                                  │
│  update(time, delta) → onUpdate() 毎フレーム処理        │
│       ↓                                                  │
│  shutdown() → cleanup() 購読解除                         │
│            → onShutdown() サブクラスクリーンアップ       │
└─────────────────────────────────────────────────────────┘
```

---

## 単体テスト要件

### テストケース1: シーン初期化 🔴

**Given**: BaseGameSceneを継承したシーンがある
**When**: シーンが初期化される
**Then**: EventBusが取得できる

### テストケース2: イベント購読のクリーンアップ 🔴

**Given**: イベントを購読している
**When**: shutdown()が呼ばれる
**Then**: すべての購読が解除される

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0167` - 詳細要件定義
2. `/tsumiki:tdd-testcases` - テストケース作成
3. `/tsumiki:tdd-red` - テスト実装（失敗）
4. `/tsumiki:tdd-green` - 最小実装
5. `/tsumiki:tdd-refactor` - リファクタリング
6. `/tsumiki:tdd-verify-complete` - 品質確認

---

## 注意事項

- Phaserの正確なライフサイクルをドキュメントで確認
- rexUIプラグインの取得タイミングに注意
- メモリリーク防止のためクリーンアップを徹底

---

## 信頼性レベルサマリー

- **総項目数**: 2項目
- 🔵 **青信号**: 0項目 (0%)
- 🟡 **黄信号**: 0項目 (0%)
- 🔴 **赤信号**: 2項目 (100%)

**品質評価**: 要調査

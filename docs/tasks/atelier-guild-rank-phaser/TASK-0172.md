# TASK-0172: UIFactory基盤設計

**タスクID**: TASK-0172
**タスクタイプ**: TDD
**推定工数**: 4時間
**フェーズ**: Phase 1 - Phaser基盤・インフラ構築
**信頼性レベル**: 🟡 *妥当な推測に基づく*

## 関連文書

- **概要**: [overview.md](./overview.md)
- **コアシステム**: [core-systems.md](../../design/atelier-guild-rank-phaser/core-systems.md)
- **UI設計**: [ui-design/overview.md](../../design/atelier-guild-rank-phaser/ui-design/overview.md)

## タスク概要

rexUIを使用したUI要素の生成を一元管理するUIFactoryの基盤を設計する。

## 依存タスク

- **前提タスク**: TASK-0160
- **後続タスク**: TASK-0173

## 完了条件

- [x] UIFactoryクラスの基盤が設計されている
- [x] 共通のUIオプションインターフェースが定義されている
- [x] rexUIプラグインの参照方法が確立されている
- [x] UI要素の基本スタイルが定義されている

---

## 実装詳細

### 1. 共通UIオプション定義 🟡

**信頼性**: 🟡 *標準的なUIパターン*

**実装ファイル**: `src/presentation/phaser/ui/UITypes.ts`

```typescript
import Phaser from 'phaser';

// 基本位置・サイズ
export interface UIPosition {
  x: number;
  y: number;
}

export interface UISize {
  width: number;
  height: number;
}

export interface UIBounds extends UIPosition, UISize {}

// 基本スタイル
export interface UIBaseStyle {
  backgroundColor?: number;
  borderColor?: number;
  borderWidth?: number;
  cornerRadius?: number;
  padding?: number | { top?: number; bottom?: number; left?: number; right?: number };
}

// ボタンオプション
export interface ButtonOptions extends UIPosition {
  text: string;
  width?: number;
  height?: number;
  style?: UIBaseStyle;
  textStyle?: Phaser.Types.GameObjects.Text.TextStyle;
  onClick?: () => void;
  disabled?: boolean;
}

// ラベルオプション
export interface LabelOptions extends UIPosition {
  text: string;
  icon?: string;
  iconSize?: number;
  textStyle?: Phaser.Types.GameObjects.Text.TextStyle;
  orientation?: 'horizontal' | 'vertical';
  space?: number;
}

// パネルオプション
export interface PanelOptions extends UIBounds {
  style?: UIBaseStyle;
  title?: string;
  titleStyle?: Phaser.Types.GameObjects.Text.TextStyle;
}

// ダイアログオプション
export interface DialogOptions {
  title: string;
  content: string | Phaser.GameObjects.GameObject;
  buttons?: Array<{
    text: string;
    onClick: () => void;
    primary?: boolean;
  }>;
  width?: number;
  modal?: boolean;
}

// プログレスバーオプション
export interface ProgressBarOptions extends UIPosition {
  width: number;
  height: number;
  value: number;
  maxValue: number;
  barColor?: number;
  backgroundColor?: number;
  showText?: boolean;
}

// スクロールパネルオプション
export interface ScrollPanelOptions extends UIBounds {
  content: Phaser.GameObjects.GameObject | Phaser.GameObjects.GameObject[];
  scrollMode?: 'vertical' | 'horizontal' | 'both';
  style?: UIBaseStyle;
}

// グリッドボタンオプション
export interface GridButtonsOptions extends UIPosition {
  items: Array<{
    id: string;
    content: Phaser.GameObjects.GameObject | string;
    data?: unknown;
  }>;
  columns: number;
  cellWidth: number;
  cellHeight: number;
  space?: { column?: number; row?: number };
  onItemClick?: (id: string, data?: unknown) => void;
}
```

### 2. UIFactory基盤クラス 🟡

**信頼性**: 🟡 *rexUIのAPIを参考*

**実装ファイル**: `src/presentation/phaser/ui/UIFactory.ts`

```typescript
import Phaser from 'phaser';
import RexUIPlugin from 'phaser3-rex-plugins/templates/ui/ui-plugin';
import { Colors } from '../config/ColorPalette';
import { TextStyles } from '../config/TextStyles';
import {
  ButtonOptions,
  LabelOptions,
  PanelOptions,
  DialogOptions,
  ProgressBarOptions,
  ScrollPanelOptions,
  GridButtonsOptions,
} from './UITypes';

export class UIFactory {
  constructor(
    private scene: Phaser.Scene,
    private rexUI: RexUIPlugin
  ) {}

  // デフォルトスタイル
  private getDefaultButtonStyle(): object {
    return {
      background: this.scene.add.rectangle(0, 0, 1, 1, Colors.primary),
      text: this.scene.add.text(0, 0, '', TextStyles.button),
      space: { left: 20, right: 20, top: 10, bottom: 10 },
    };
  }

  // 各createメソッドは後続タスクで実装
  createButton(options: ButtonOptions): any {
    throw new Error('Not implemented - see TASK-0173');
  }

  createLabel(options: LabelOptions): any {
    throw new Error('Not implemented - see TASK-0174');
  }

  createDialog(options: DialogOptions): any {
    throw new Error('Not implemented - see TASK-0175');
  }

  createProgressBar(options: ProgressBarOptions): any {
    throw new Error('Not implemented - see TASK-0176');
  }

  createScrollPanel(options: ScrollPanelOptions): any {
    throw new Error('Not implemented - see TASK-0177');
  }

  createGridButtons(options: GridButtonsOptions): any {
    throw new Error('Not implemented - see TASK-0178');
  }

  // ユーティリティメソッド
  protected createRoundedRect(
    x: number,
    y: number,
    width: number,
    height: number,
    color: number,
    radius: number = 8
  ): Phaser.GameObjects.Graphics {
    const graphics = this.scene.add.graphics();
    graphics.fillStyle(color);
    graphics.fillRoundedRect(x - width/2, y - height/2, width, height, radius);
    return graphics;
  }
}
```

### 3. UIFactoryエクスポート 🟡

**信頼性**: 🟡 *標準的なモジュール構成*

**実装ファイル**: `src/presentation/phaser/ui/index.ts`

```typescript
export { UIFactory } from './UIFactory';
export * from './UITypes';
```

---

## 単体テスト要件

### テストケース1: UIFactory生成 🟡

**Given**: シーンとrexUIプラグインがある
**When**: UIFactoryを生成する
**Then**: インスタンスが正常に生成される

### テストケース2: 型定義チェック 🟡

**Given**: UITypesが定義されている
**When**: 型チェックを実行する
**Then**: コンパイルエラーがない

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0172` - 詳細要件定義
2. `/tsumiki:tdd-testcases` - テストケース作成
3. `/tsumiki:tdd-red` - テスト実装（失敗）
4. `/tsumiki:tdd-green` - 最小実装
5. `/tsumiki:tdd-refactor` - リファクタリング
6. `/tsumiki:tdd-verify-complete` - 品質確認

---

## 注意事項

- rexUIの各コンポーネントAPIを確認
- 共通スタイルは一箇所で管理
- 拡張性を考慮した設計

---

## 信頼性レベルサマリー

- **総項目数**: 3項目
- 🔵 **青信号**: 0項目 (0%)
- 🟡 **黄信号**: 3項目 (100%)
- 🔴 **赤信号**: 0項目 (0%)

**品質評価**: 中品質

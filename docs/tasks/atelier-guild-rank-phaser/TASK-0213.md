...
BasePhaseContainerの具体的な実装を行う。共通機能の実装とテストを完了させる。

## 依存タスク

- **前提タスク**: TASK-0212
- **後続タスク**: TASK-0216, TASK-0222, TASK-0227, TASK-0232

## 完了条件

- [ ] BasePhaseContainerが完全に実装されている
- [ ] アニメーション（enter/exit）が動作する
- [ ] EventBus連携が動作する
- [ ] 有効/無効切り替えが動作する
- [ ] ユニットテストが通る
...
**実装ファイル**: `src/presentation/phaser/ui/phase/BasePhaseContainer.ts` (追加)

```typescript
// BasePhaseContainerに追加
...
**Given**: BasePhaseContainerがある
**When**: createTitle('テスト')を呼ぶ
**Then**: タイトルテキストが追加される

### テストケース2: ボタン有効/無効 🟡

**Given**: ボタンが生成されている
**When**: setButtonEnabled(button, false)を呼ぶ
**Then**: ボタンがグレーアウトされる

### テストケース3: ローディング表示 🟡

**Given**: フェーズコンテナがある
**When**: showLoading()を呼ぶ
**Then**: ローディングオーバーレイが表示される

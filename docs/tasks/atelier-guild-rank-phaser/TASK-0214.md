...
依頼の詳細を表示するパネルコンポーネントを設計・実装する。依頼名、期限、報酬、必要アイテムを表示する。
...
- [x] IQuestPanelインターフェースが定義されている
- [x] 依頼名が表示される
- [x] 期限と残り日数が表示される
- [x] 報酬（ゴールド、経験値、カード）が表示される
- [x] 必要アイテムリストが表示される
- [x] 進捗状況が表示される
...
**実装ファイル**: `src/presentation/phaser/ui/quest/QuestPanelConstants.ts`
...
### 2. IQuestPanelインターフェース 🔵

**信頼性**: 🔵 *コンポーネント設計*

**実装ファイル**: `src/presentation/phaser/ui/quest/IQuestPanel.ts`
...
**実装ファイル**: `src/presentation/phaser/ui/quest/QuestPanel.ts`
...
export class QuestPanel implements IQuestPanel {
  ...
  setQuest(quest: Quest | null): void {
    ...
    this.deadlineText.setText(`期限: 残り ${remaining} 日`);
  ...
  private updateRewardSection(quest: Quest): void {
    ...
    if (quest.reward.gold) rewards.push(`💰 ${quest.reward.gold} ゴールド`);
    if (quest.reward.exp) rewards.push(`✨ ${quest.reward.exp} EXP`);
    if (quest.reward.card) rewards.push(`🃏 カード報酬あり`);
  ...
  private updateRequirementSection(quest: Quest): void {
    ...
    quest.requiredItems.forEach((item, index) => {
      const text = this.scene.add.text(0, 25 + index * 20, `・${item.name} x${item.quantity}`, {
        ...TextStyles.body,
        fontSize: '13px',
      });
  ...
  // ボタン生成ヘルパー（BasePhaseContainerと同様）
  private createButton(...args: any[]): Phaser.GameObjects.Container {
    // 実装は省略（BasePhaseContainerと同様）
...
**Given**: QuestPanelがある
**When**: setQuest(quest)を呼ぶ
**Then**: 依頼情報が表示される

### テストケース2: 空状態表示 🔵

**Given**: QuestPanelに依頼が設定されている
**When**: setQuest(null)を呼ぶ
**Then**: 空状態が表示される

### テストケース3: 納品ボタン切り替え 🔵

**Given**: QuestPanelがある
**When**: showDeliverButton(true)を呼ぶ
**Then**: 納品ボタンが表示され、受注ボタンが非表示になる

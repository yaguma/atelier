# TASK-0168: BaseGameScene実装

**タスクID**: TASK-0168
**タスクタイプ**: TDD
**推定工数**: 4時間
**フェーズ**: Phase 1 - Phaser基盤・インフラ構築
**信頼性レベル**: 🔴 *Phaser固有の調査が必要*

## 関連文書

- **概要**: [overview.md](./overview.md)
- **設計文書**: [architecture.md](../../design/atelier-guild-rank-phaser/architecture.md)
- **コアシステム**: [core-systems.md](../../design/atelier-guild-rank-phaser/core-systems.md)

## タスク概要

TASK-0167で設計したBaseGameSceneを実装し、動作確認を行う。

## 依存タスク

- **前提タスク**: TASK-0167
- **後続タスク**: TASK-0169, TASK-0182, TASK-0212

## 完了条件

- [ ] BaseGameSceneクラスが実装されている
- [ ] テスト用の継承シーンが動作する
- [ ] EventBus連携が動作する
- [ ] UIFactory連携が動作する
- [ ] クリーンアップ処理が動作する

---

## 実装詳細

### 1. BaseGameScene実装 🔴

**信頼性**: 🔴 *設計に基づくが、Phaser固有の調整が必要*

**実装ファイル**: `src/presentation/phaser/scenes/BaseGameScene.ts`

TASK-0167の設計を実装する。

### 2. テスト用シーン実装 🔴

**信頼性**: 🔴 *動作確認用*

**実装ファイル**: `src/presentation/phaser/scenes/TestScene.ts`

```typescript
import { BaseGameScene, SceneInitData } from './BaseGameScene';
import { StateUpdateEvents } from '../events/EventTypes';

export class TestScene extends BaseGameScene {
  private testLabel!: Phaser.GameObjects.Text;

  constructor() {
    super('TestScene' as any);
  }

  protected onInit(data?: SceneInitData): void {
    console.log('TestScene: onInit', data);
  }

  protected onPreload(): void {
    console.log('TestScene: onPreload');
  }

  protected onCreate(data?: SceneInitData): void {
    console.log('TestScene: onCreate', data);

    // テスト用ラベル
    this.testLabel = this.add.text(640, 360, 'Test Scene', {
      fontSize: '32px',
      color: '#ffffff'
    }).setOrigin(0.5);

    // UIFactoryを使ったボタン
    const button = this.uiFactory.createButton({
      x: 640,
      y: 450,
      text: 'Click Me',
      onClick: () => {
        console.log('Button clicked!');
        this.eventBus.emitVoid('ui:newGame:clicked' as any);
      }
    });
  }

  protected setupEventListeners(): void {
    // 状態変更イベントを購読
    const unsub = this.eventBus.on('state:gold:changed', (payload) => {
      console.log('Gold changed:', payload);
      this.testLabel.setText(`Gold: ${payload.gold}`);
    });
    this.subscribe(unsub);
  }

  protected onUpdate(time: number, delta: number): void {
    // フレーム更新処理
  }

  protected onShutdown(): void {
    console.log('TestScene: onShutdown');
  }
}
```

### 3. シーンエクスポート 🔵

**信頼性**: 🔵 *標準的なモジュール構成*

**実装ファイル**: `src/presentation/phaser/scenes/index.ts`

```typescript
export { BaseGameScene, SceneInitData } from './BaseGameScene';
export { TestScene } from './TestScene';
```

---

## 単体テスト要件

### テストケース1: シーン作成 🔴

**Given**: TestSceneインスタンスがある
**When**: シーンをゲームに追加する
**Then**: create()が呼ばれUIが表示される

### テストケース2: イベント連携 🔴

**Given**: TestSceneがアクティブである
**When**: EventBusでstate:gold:changedを発行する
**Then**: ラベルが更新される

### テストケース3: シャットダウン 🔴

**Given**: TestSceneでイベントを購読している
**When**: シーンをシャットダウンする
**Then**: 購読が解除されリスナーカウントが0になる

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0168` - 詳細要件定義
2. `/tsumiki:tdd-testcases` - テストケース作成
3. `/tsumiki:tdd-red` - テスト実装（失敗）
4. `/tsumiki:tdd-green` - 最小実装
5. `/tsumiki:tdd-refactor` - リファクタリング
6. `/tsumiki:tdd-verify-complete` - 品質確認

---

## 注意事項

- Phaserのテスト環境構築が必要な場合がある
- headlessモードでのテストを検討
- 実際のゲーム動作での確認も必要

---

## 信頼性レベルサマリー

- **総項目数**: 3項目
- 🔵 **青信号**: 1項目 (33%)
- 🟡 **黄信号**: 0項目 (0%)
- 🔴 **赤信号**: 2項目 (67%)

**品質評価**: 要調査

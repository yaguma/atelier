...
メイン画面右側のサイドバーUIのインターフェースと基本構造を設計する。
...
- [x] ISidebarUIインターフェースが定義されている
- [x] サイドバーレイアウト定数が定義されている
- [x] タブ切り替え機能が設計されている
- [x] 依頼リスト・インベントリ表示領域が設計されている
...
**実装ファイル**: `src/presentation/phaser/ui/sidebar/SidebarConstants.ts`
...
### 2. ISidebarUIインターフェース 🟡

**信頼性**: 🟡 *コンポーネント設計*

**実装ファイル**: `src/presentation/phaser/ui/sidebar/ISidebarUI.ts`
...
**実装ファイル**: `src/presentation/phaser/ui/sidebar/SidebarUI.ts`
...
export class SidebarUI implements ISidebarUI {
  ...
  private createTabs(): void {
    ...
    // インベントリタブ
    this.inventoryTab = this.createTab('所持品', SidebarLayout.TAB_WIDTH, () => this.setActiveTab('inventory'));
...
**Given**: SidebarUIが初期化されている
**When**: setActiveTab('inventory')を呼ぶ
**Then**: インベントリタブがアクティブになる

### テストケース2: 初期タブ 🟡

**Given**: SidebarUIを生成する
**When**: 何もしない
**Then**: 依頼タブがアクティブになっている

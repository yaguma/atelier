# TASK-0166: EventBusテスト（32イベント）

**タスクID**: TASK-0166
**タスクタイプ**: TDD
**推定工数**: 4時間
**フェーズ**: Phase 1 - Phaser基盤・インフラ構築
**信頼性レベル**: 🟡 *妥当な推測に基づく*

## 関連文書

- **概要**: [overview.md](./overview.md)
- **コアシステム**: [core-systems.md](../../design/atelier-guild-rank-phaser/core-systems.md)

## タスク概要

定義された32種類のイベントすべてについて、発行・購読が正しく動作することをテストする。

## 依存タスク

- **前提タスク**: TASK-0165
- **後続タスク**: TASK-0181

## 完了条件

- [x] 全UIアクションイベント（21種類）のテストが通る
- [x] 全状態更新イベント（12種類）のテストが通る
- [x] ペイロード付きイベントの型安全性がテストされている
- [x] ペイロードなしイベントの動作がテストされている
- [x] テストカバレッジ80%以上

---

## 実装詳細

### 1. UIアクションイベントテスト 🟡

**信頼性**: 🟡 *イベント一覧から導出*

**テストファイル**: `tests/unit/presentation/events/UIActionEvents.test.ts`

```typescript
import { describe, it, expect, beforeEach } from 'vitest';
import { EventBus } from '../../../../src/presentation/phaser/events/EventBus';
import { UIActionEvents } from '../../../../src/presentation/phaser/events/EventTypes';

describe('UIActionEvents', () => {
  let eventBus: EventBus;

  beforeEach(() => {
    EventBus.resetInstance();
    eventBus = EventBus.getInstance();
  });

  describe('ゲーム開始イベント', () => {
    it('NEW_GAME_CLICKEDが発行できる', () => {
      const callback = vi.fn();
      eventBus.onVoid(UIActionEvents.NEW_GAME_CLICKED, callback);
      eventBus.emitVoid(UIActionEvents.NEW_GAME_CLICKED);
      expect(callback).toHaveBeenCalledTimes(1);
    });

    it('CONTINUE_CLICKEDが発行できる', () => {
      const callback = vi.fn();
      eventBus.onVoid(UIActionEvents.CONTINUE_CLICKED, callback);
      eventBus.emitVoid(UIActionEvents.CONTINUE_CLICKED);
      expect(callback).toHaveBeenCalledTimes(1);
    });
  });

  describe('依頼受注フェーズイベント', () => {
    it('QUEST_SELECTEDがペイロード付きで発行できる', () => {
      const callback = vi.fn();
      eventBus.on('ui:quest:selected', callback);
      eventBus.emit('ui:quest:selected', { questId: 'quest-001' });
      expect(callback).toHaveBeenCalledWith({ questId: 'quest-001' });
    });
    // ... 他のイベントも同様
  });

  // ... 全21イベントについてテスト
});
```

### 2. 状態更新イベントテスト 🟡

**信頼性**: 🟡 *イベント一覧から導出*

**テストファイル**: `tests/unit/presentation/events/StateUpdateEvents.test.ts`

```typescript
import { describe, it, expect, beforeEach } from 'vitest';
import { EventBus } from '../../../../src/presentation/phaser/events/EventBus';
import { StateUpdateEvents } from '../../../../src/presentation/phaser/events/EventTypes';

describe('StateUpdateEvents', () => {
  let eventBus: EventBus;

  beforeEach(() => {
    EventBus.resetInstance();
    eventBus = EventBus.getInstance();
  });

  describe('ゲーム状態イベント', () => {
    it('GAME_STATE_CHANGEDがペイロード付きで発行できる', () => {
      const callback = vi.fn();
      eventBus.on('state:game:changed', callback);
      eventBus.emit('state:game:changed', { state: 'playing' });
      expect(callback).toHaveBeenCalledWith({ state: 'playing' });
    });

    it('PHASE_CHANGEDがペイロード付きで発行できる', () => {
      const callback = vi.fn();
      eventBus.on('state:phase:changed', callback);
      eventBus.emit('state:phase:changed', {
        phase: 'gathering',
        previousPhase: 'questAccept'
      });
      expect(callback).toHaveBeenCalledWith({
        phase: 'gathering',
        previousPhase: 'questAccept'
      });
    });
  });

  describe('リソースイベント', () => {
    it('GOLD_CHANGEDが正しいペイロードで発行できる', () => {
      const callback = vi.fn();
      eventBus.on('state:gold:changed', callback);
      eventBus.emit('state:gold:changed', { gold: 1500, delta: 100 });
      expect(callback).toHaveBeenCalledWith({ gold: 1500, delta: 100 });
    });
    // ... 他のイベントも同様
  });

  // ... 全11イベントについてテスト
});
```

### 3. イベント一覧チェックテスト 🟡

**信頼性**: 🟡 *自動検証*

**テストファイル**: `tests/unit/presentation/events/EventCompleteness.test.ts`

```typescript
import { describe, it, expect } from 'vitest';
import { UIActionEvents, StateUpdateEvents } from '../../../../src/presentation/phaser/events/EventTypes';

describe('イベント完全性チェック', () => {
  it('UIアクションイベントは21種類ある', () => {
    const count = Object.keys(UIActionEvents).length;
    expect(count).toBe(21);
  });

  it('状態更新イベントは11種類ある', () => {
    const count = Object.keys(StateUpdateEvents).length;
    expect(count).toBe(11);
  });

  it('合計32イベントが定義されている', () => {
    const total = Object.keys(UIActionEvents).length + Object.keys(StateUpdateEvents).length;
    expect(total).toBe(32);
  });

  it('すべてのイベントキーがユニークである', () => {
    const allKeys = [
      ...Object.values(UIActionEvents),
      ...Object.values(StateUpdateEvents)
    ];
    const uniqueKeys = new Set(allKeys);
    expect(uniqueKeys.size).toBe(allKeys.length);
  });
});
```

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0166` - 詳細要件定義
2. `/tsumiki:tdd-testcases` - テストケース作成
3. `/tsumiki:tdd-red` - テスト実装（失敗）
4. `/tsumiki:tdd-green` - 最小実装
5. `/tsumiki:tdd-refactor` - リファクタリング
6. `/tsumiki:tdd-verify-complete` - 品質確認

---

## 注意事項

- 各テストは独立して実行可能にする
- beforeEachでEventBusをリセットする
- ペイロードの型が正しいことを確認

---

## 信頼性レベルサマリー

- **総項目数**: 3項目
- 🔵 **青信号**: 0項目 (0%)
- 🟡 **黄信号**: 3項目 (100%)
- 🔴 **赤信号**: 0項目 (0%)

**品質評価**: 中品質

# TASK-0179: DialogManager設計・実装

**タスクID**: TASK-0179
**タスクタイプ**: TDD
**推定工数**: 4時間
**フェーズ**: Phase 1 - Phaser基盤・インフラ構築
**信頼性レベル**: 🟡 *妥当な推測に基づく*

## 関連文書

- **概要**: [overview.md](./overview.md)
- **コアシステム**: [core-systems.md](../../design/atelier-guild-rank-phaser/core-systems.md)

## タスク概要

ダイアログの表示・管理を一元化するDialogManagerを設計・実装する。キュー管理や重複表示防止を行う。

## 依存タスク

- **前提タスク**: TASK-0175
- **後続タスク**: TASK-0181

## 完了条件

- [ ] DialogManagerクラスが実装されている
- [ ] ダイアログのキュー管理ができる
- [ ] 同時に1つのダイアログのみ表示される
- [ ] ダイアログが閉じたら次のダイアログが表示される
- [ ] シングルトンとしてアクセスできる

---

## 実装詳細

### 1. DialogManager実装 🟡

**信頼性**: 🟡 *標準的なマネージャーパターン*

**実装ファイル**: `src/presentation/phaser/managers/DialogManager.ts`

```typescript
import Phaser from 'phaser';
import Dialog from 'phaser3-rex-plugins/templates/ui/dialog/Dialog';
import { UIFactory } from '../ui/UIFactory';
import { DialogOptions } from '../ui/UITypes';

interface QueuedDialog {
  options: DialogOptions;
  resolve: () => void;
}

export class DialogManager {
  private static instance: DialogManager | null = null;
  private currentDialog: Dialog | null = null;
  private queue: QueuedDialog[] = [];
  private uiFactory: UIFactory | null = null;
  private scene: Phaser.Scene | null = null;

  private constructor() {}

  public static getInstance(): DialogManager {
    if (!DialogManager.instance) {
      DialogManager.instance = new DialogManager();
    }
    return DialogManager.instance;
  }

  public static resetInstance(): void {
    if (DialogManager.instance) {
      DialogManager.instance.closeAll();
      DialogManager.instance = null;
    }
  }

  public setContext(scene: Phaser.Scene, uiFactory: UIFactory): void {
    this.scene = scene;
    this.uiFactory = uiFactory;
  }

  /**
   * ダイアログを表示（Promise返却）
   */
  public async show(options: DialogOptions): Promise<void> {
    return new Promise((resolve) => {
      if (this.currentDialog) {
        // キューに追加
        this.queue.push({ options, resolve });
      } else {
        // 即時表示
        this.displayDialog(options, resolve);
      }
    });
  }

  /**
   * 確認ダイアログを表示
   */
  public async confirm(
    title: string,
    message: string,
    confirmText: string = '確認',
    cancelText: string = 'キャンセル'
  ): Promise<boolean> {
    return new Promise((resolve) => {
      this.show({
        title,
        content: message,
        buttons: [
          {
            text: cancelText,
            onClick: () => resolve(false),
          },
          {
            text: confirmText,
            onClick: () => resolve(true),
            primary: true,
          },
        ],
      });
    });
  }

  /**
   * アラートダイアログを表示
   */
  public async alert(
    title: string,
    message: string,
    buttonText: string = 'OK'
  ): Promise<void> {
    return this.show({
      title,
      content: message,
      buttons: [
        {
          text: buttonText,
          onClick: () => {},
          primary: true,
        },
      ],
    });
  }

  /**
   * 現在のダイアログを閉じる
   */
  public close(): void {
    if (this.currentDialog) {
      this.currentDialog.destroy();
      this.currentDialog = null;
      this.processQueue();
    }
  }

  /**
   * すべてのダイアログを閉じる（キューもクリア）
   */
  public closeAll(): void {
    if (this.currentDialog) {
      this.currentDialog.destroy();
      this.currentDialog = null;
    }
    this.queue.forEach(q => q.resolve());
    this.queue = [];
  }

  /**
   * ダイアログが表示中かどうか
   */
  public isShowing(): boolean {
    return this.currentDialog !== null;
  }

  /**
   * キューにダイアログがあるか
   */
  public hasQueued(): boolean {
    return this.queue.length > 0;
  }

  private displayDialog(options: DialogOptions, resolve: () => void): void {
    if (!this.uiFactory) {
      console.error('DialogManager: UIFactory not set');
      resolve();
      return;
    }

    // ボタンにresolve呼び出しを追加
    const wrappedOptions: DialogOptions = {
      ...options,
      buttons: options.buttons?.map(btn => ({
        ...btn,
        onClick: () => {
          btn.onClick();
          this.currentDialog = null;
          resolve();
          this.processQueue();
        },
      })),
    };

    this.currentDialog = this.uiFactory.createDialog(wrappedOptions);
  }

  private processQueue(): void {
    if (this.queue.length > 0) {
      const next = this.queue.shift()!;
      this.displayDialog(next.options, next.resolve);
    }
  }
}
```

### 2. DialogManagerエクスポート 🟡

**信頼性**: 🟡 *標準的なモジュール構成*

**実装ファイル**: `src/presentation/phaser/managers/index.ts`

```typescript
export { DialogManager } from './DialogManager';
export { SceneManager } from './SceneManager';
```

---

## 単体テスト要件

### テストケース1: ダイアログ表示 🟡

**Given**: DialogManagerインスタンスがある
**When**: show()を呼ぶ
**Then**: ダイアログが表示される

### テストケース2: キュー管理 🟡

**Given**: ダイアログが表示中
**When**: 別のshow()を呼ぶ
**Then**: キューに追加され、前のダイアログが閉じた後に表示される

### テストケース3: confirm 🟡

**Given**: DialogManagerインスタンスがある
**When**: confirm()を呼び確認ボタンを押す
**Then**: trueが返される

### テストケース4: closeAll 🟡

**Given**: ダイアログとキューがある
**When**: closeAll()を呼ぶ
**Then**: すべてが閉じられキューもクリアされる

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0179` - 詳細要件定義
2. `/tsumiki:tdd-testcases` - テストケース作成
3. `/tsumiki:tdd-red` - テスト実装（失敗）
4. `/tsumiki:tdd-green` - 最小実装
5. `/tsumiki:tdd-refactor` - リファクタリング
6. `/tsumiki:tdd-verify-complete` - 品質確認

---

## 注意事項

- シーン遷移時のダイアログ状態管理
- UIFactoryの参照が必要
- Promiseのリジェクトケースを検討

---

## 信頼性レベルサマリー

- **総項目数**: 2項目
- 🔵 **青信号**: 0項目 (0%)
- 🟡 **黄信号**: 2項目 (100%)
- 🔴 **赤信号**: 0項目 (0%)

**品質評価**: 中品質

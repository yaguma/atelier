# TASK-0200: MaterialViewテスト

**タスクID**: TASK-0200
**タスクタイプ**: TDD
**推定工数**: 4時間
**フェーズ**: Phase 2 - 基本シーン・共通UI実装
**信頼性レベル**: 🟡 *妥当な推測に基づく*

## 関連文書

- **概要**: [overview.md](./overview.md)
- **UI設計**: [ui-design/overview.md](../../design/atelier-guild-rank-phaser/ui-design/overview.md)

## タスク概要

MaterialView関連コンポーネントのテストを実施する。

## 依存タスク

- **前提タスク**: TASK-0199
- **後続タスク**: TASK-0201

## 完了条件

- [ ] MaterialConstantsのテストが通る
- [ ] 品質色のマッピングテストが通る
- [ ] MaterialViewの基本操作テストが通る
- [ ] テストカバレッジ80%以上

---

## 実装詳細

### 1. MaterialConstantsテスト 🟡

**信頼性**: 🟡 *定数の検証*

**テストファイル**: `tests/unit/presentation/ui/material/MaterialView.test.ts`

```typescript
import { describe, it, expect } from 'vitest';
import {
  MaterialLayout,
  MaterialQualityColors
} from '../../../../../src/presentation/phaser/ui/material/MaterialConstants';

describe('MaterialConstants', () => {
  describe('MaterialLayout', () => {
    it('アイコンサイズが定義されている', () => {
      expect(MaterialLayout.ICON_SIZE).toBeGreaterThan(0);
      expect(MaterialLayout.BADGE_SIZE).toBeGreaterThan(0);
    });

    it('コンパクト表示サイズが定義されている', () => {
      expect(MaterialLayout.COMPACT_WIDTH).toBeGreaterThan(0);
      expect(MaterialLayout.COMPACT_HEIGHT).toBeGreaterThan(0);
    });

    it('詳細表示サイズが定義されている', () => {
      expect(MaterialLayout.DETAIL_WIDTH).toBeGreaterThan(0);
      expect(MaterialLayout.DETAIL_HEIGHT).toBeGreaterThan(0);
    });

    it('詳細表示はコンパクト表示より大きい', () => {
      expect(MaterialLayout.DETAIL_WIDTH).toBeGreaterThan(MaterialLayout.COMPACT_WIDTH);
    });
  });

  describe('MaterialQualityColors', () => {
    it('品質1〜5の色が定義されている', () => {
      for (let quality = 1; quality <= 5; quality++) {
        expect(MaterialQualityColors[quality]).toBeDefined();
        expect(MaterialQualityColors[quality]).toBeTypeOf('number');
      }
    });

    it('各品質で異なる色が設定されている', () => {
      const colors = Object.values(MaterialQualityColors);
      const uniqueColors = new Set(colors);
      expect(uniqueColors.size).toBe(colors.length);
    });

    it('品質5は金色(0xffd700)である', () => {
      expect(MaterialQualityColors[5]).toBe(0xffd700);
    });
  });
});

describe('MaterialView', () => {
  // Phaserシーンのモックが必要なテスト

  describe('コンパクト表示', () => {
    it.skip('素材アイコンが表示される', () => {
      // Phaserシーンモックを使用したテスト
    });

    it.skip('個数が1より大きい場合バッジが表示される', () => {
      // 個数表示のテスト
    });

    it.skip('品質インジケーターが正しい色で表示される', () => {
      // 品質色のテスト
    });
  });

  describe('詳細表示', () => {
    it.skip('素材名が表示される', () => {
      // 名前表示のテスト
    });

    it.skip('品質テキストが表示される', () => {
      // 品質テキストのテスト
    });

    it.skip('個数が表示される', () => {
      // 個数表示のテスト
    });
  });

  describe('インタラクション', () => {
    it.skip('クリックでonClickコールバックが呼ばれる', () => {
      // クリックイベントのテスト
    });

    it.skip('ホバーでonHoverコールバックが呼ばれる', () => {
      // ホバーイベントのテスト
    });

    it.skip('setEnabledでインタラクションを無効化できる', () => {
      // 無効化のテスト
    });
  });

  describe('状態管理', () => {
    it.skip('setCountで個数を更新できる', () => {
      // 個数更新のテスト
    });

    it.skip('setSelectedで選択状態を変更できる', () => {
      // 選択状態のテスト
    });
  });
});
```

### 2. 素材カテゴリ絵文字マッピングテスト 🟡

**信頼性**: 🟡 *フォールバック表示*

```typescript
describe('素材カテゴリ絵文字', () => {
  const categoryEmojis: Record<string, string> = {
    'plant': '🌿',
    'mineral': '💎',
    'liquid': '💧',
    'powder': '✨',
    'monster': '🦴',
  };

  it('各カテゴリに対応する絵文字がある', () => {
    Object.entries(categoryEmojis).forEach(([category, emoji]) => {
      expect(emoji).toBeDefined();
      expect(emoji.length).toBeGreaterThan(0);
    });
  });

  it('未定義カテゴリはデフォルト絵文字を返す', () => {
    // 実際のMaterialView実装でテスト
    const defaultEmoji = '📦';
    expect(defaultEmoji).toBeDefined();
  });
});
```

### 3. ビジュアルテスト用シーン 🟡

**信頼性**: 🟡 *手動確認用*

**実装ファイル**: `src/presentation/phaser/scenes/MaterialViewTestScene.ts`

```typescript
import { BaseGameScene, SceneInitData } from './BaseGameScene';
import { MaterialView } from '../ui/material/MaterialView';
import { Material } from '../../../domain/material/Material';

export class MaterialViewTestScene extends BaseGameScene {
  constructor() {
    super('MaterialViewTestScene' as any);
  }

  protected onInit(data?: SceneInitData): void {}
  protected onPreload(): void {}

  protected onCreate(data?: SceneInitData): void {
    // 各品質の素材をコンパクト表示で並べる
    const materials = this.createTestMaterials();
    let x = 100;
    let y = 100;

    materials.forEach((material, index) => {
      // コンパクト表示
      new MaterialView(this, {
        material,
        x,
        y,
        mode: 'compact',
        count: index + 1,
        showQuality: true,
        interactive: true,
        onClick: (m) => console.log(`Clicked: ${m.name}`),
      });

      // 詳細表示
      new MaterialView(this, {
        material,
        x: 300,
        y,
        mode: 'detail',
        count: index + 1,
        showQuality: true,
        interactive: true,
      });

      y += 80;
    });
  }

  private createTestMaterials(): Material[] {
    // 各品質のテスト素材を生成
    return [
      // Material.create(...) でテストデータ生成
    ];
  }

  protected setupEventListeners(): void {}
}
```

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0200` - 詳細要件定義
2. `/tsumiki:tdd-testcases` - テストケース作成
3. `/tsumiki:tdd-red` - テスト実装（失敗）
4. `/tsumiki:tdd-green` - 最小実装
5. `/tsumiki:tdd-refactor` - リファクタリング
6. `/tsumiki:tdd-verify-complete` - 品質確認

---

## 注意事項

- Phaserに依存しない部分のテストを優先
- 品質色のアクセシビリティ
- ビジュアルテストの重要性

---

## 信頼性レベルサマリー

- **総項目数**: 3項目
- 🔵 **青信号**: 0項目 (0%)
- 🟡 **黄信号**: 3項目 (100%)
- 🔴 **赤信号**: 0項目 (0%)

**品質評価**: 中品質

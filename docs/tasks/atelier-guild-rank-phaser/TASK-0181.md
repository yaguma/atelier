# TASK-0181: Phase1統合テスト

**タスクID**: TASK-0181
**タスクタイプ**: TDD
**推定工数**: 4時間
**フェーズ**: Phase 1 - Phaser基盤・インフラ構築
**信頼性レベル**: 🔴 *Phaser固有の調査が必要*

## 関連文書

- **概要**: [overview.md](./overview.md)
- **設計文書**: [architecture.md](../../design/atelier-guild-rank-phaser/architecture.md)

## タスク概要

Phase 1で実装した全コンポーネント（EventBus、SceneManager、UIFactory、DialogManager、ToastManager）の統合テストを実施する。

## 依存タスク

- **前提タスク**: TASK-0166, TASK-0171, TASK-0178, TASK-0179, TASK-0180
- **後続タスク**: TASK-0182以降（Phase 2）

## 完了条件

- [x] 全コンポーネントが連携して動作する
- [x] EventBusを介したコンポーネント間通信が動作する
- [x] シーン遷移が正常に動作する
- [x] UI要素の生成と表示が動作する
- [x] 統合テストカバレッジ80%以上

---

## 実装詳細

### 1. 統合テストシナリオ 🔴

**信頼性**: 🔴 *実際の動作確認が必要*

**テストファイル**: `tests/integration/phaser/Phase1Integration.test.ts`

```typescript
import { describe, it, expect, beforeEach, afterEach } from 'vitest';
import { EventBus } from '../../../src/presentation/phaser/events/EventBus';
import { SceneManager } from '../../../src/presentation/phaser/managers/SceneManager';
import { DialogManager } from '../../../src/presentation/phaser/managers/DialogManager';
import { ToastManager } from '../../../src/presentation/phaser/managers/ToastManager';

describe('Phase 1 統合テスト', () => {
  beforeEach(() => {
    EventBus.resetInstance();
    SceneManager.resetInstance();
    DialogManager.resetInstance();
    ToastManager.resetInstance();
  });

  describe('EventBus統合', () => {
    it('全マネージャーがEventBusを共有する', () => {
      const eventBus = EventBus.getInstance();
      // 各マネージャーがEventBusを参照していることを確認
      expect(eventBus).toBeDefined();
    });

    it('イベント発行が各コンポーネントに伝播する', () => {
      const eventBus = EventBus.getInstance();
      const callback = vi.fn();

      eventBus.on('state:gold:changed', callback);
      eventBus.emit('state:gold:changed', { gold: 100, delta: 50 });

      expect(callback).toHaveBeenCalledWith({ gold: 100, delta: 50 });
    });
  });

  describe('シングルトン管理', () => {
    it('各マネージャーはシングルトンである', () => {
      const eventBus1 = EventBus.getInstance();
      const eventBus2 = EventBus.getInstance();
      expect(eventBus1).toBe(eventBus2);

      const sceneManager1 = SceneManager.getInstance();
      const sceneManager2 = SceneManager.getInstance();
      expect(sceneManager1).toBe(sceneManager2);
    });

    it('resetInstance後は新しいインスタンスが生成される', () => {
      const eventBus1 = EventBus.getInstance();
      EventBus.resetInstance();
      const eventBus2 = EventBus.getInstance();
      expect(eventBus1).not.toBe(eventBus2);
    });
  });

  describe('メモリリーク防止', () => {
    it('EventBusのclearで全リスナーが解除される', () => {
      const eventBus = EventBus.getInstance();

      eventBus.on('state:gold:changed', () => {});
      eventBus.on('state:ap:changed', () => {});
      eventBus.onVoid('ui:newGame:clicked', () => {});

      expect(eventBus.getListenerCount()).toBe(3);

      eventBus.clear();

      expect(eventBus.getListenerCount()).toBe(0);
    });
  });
});
```

### 2. UIFactory統合テスト 🔴

**信頼性**: 🔴 *Phaserのモック環境が必要*

```typescript
describe('UIFactory統合', () => {
  // Note: これらのテストはPhaserのモック環境が必要
  // 実際のテストはブラウザ環境またはheadlessモードで実行

  it.skip('ボタンクリックがEventBusにイベントを発行する', async () => {
    // 実装時に詳細化
  });

  it.skip('ダイアログのボタンが正しく動作する', async () => {
    // 実装時に詳細化
  });

  it.skip('トーストが指定時間後に消える', async () => {
    // 実装時に詳細化
  });
});
```

### 3. 動作確認用デモシーン 🔴

**信頼性**: 🔴 *手動確認用*

**実装ファイル**: `src/presentation/phaser/scenes/DemoScene.ts`

```typescript
import { BaseGameScene, SceneInitData } from './BaseGameScene';
import { SceneKeys } from '../config/SceneKeys';
import { DialogManager } from '../managers/DialogManager';
import { ToastManager } from '../managers/ToastManager';

export class DemoScene extends BaseGameScene {
  constructor() {
    super('DemoScene' as any);
  }

  protected onInit(data?: SceneInitData): void {}

  protected onPreload(): void {}

  protected onCreate(data?: SceneInitData): void {
    // タイトル
    this.uiFactory.createTitleLabel({
      x: 640, y: 50,
      text: 'Phase 1 統合デモ',
      size: 'large',
    });

    // ボタンテスト
    this.uiFactory.createButton({
      x: 200, y: 150,
      text: 'トースト（成功）',
      onClick: () => ToastManager.getInstance().success('成功しました！'),
    });

    this.uiFactory.createButton({
      x: 400, y: 150,
      text: 'トースト（エラー）',
      onClick: () => ToastManager.getInstance().error('エラーが発生しました'),
    });

    this.uiFactory.createButton({
      x: 600, y: 150,
      text: 'ダイアログ',
      onClick: () => {
        DialogManager.getInstance().confirm(
          '確認',
          'この操作を実行しますか？'
        ).then(result => {
          ToastManager.getInstance().info(result ? '実行しました' : 'キャンセルしました');
        });
      },
    });

    // EventBusテスト
    this.uiFactory.createButton({
      x: 200, y: 250,
      text: 'Gold +100',
      onClick: () => {
        this.eventBus.emit('state:gold:changed', { gold: 1100, delta: 100 });
      },
    });

    // プログレスバーテスト
    const progressBar = this.uiFactory.createProgressBar({
      x: 640, y: 350,
      width: 400,
      height: 24,
      value: 30,
      maxValue: 100,
      showText: true,
    });

    this.uiFactory.createButton({
      x: 640, y: 420,
      text: 'ゲージ +10',
      onClick: () => {
        const newValue = Math.min(100, progressBar.getValue() + 10);
        progressBar.setValue(newValue, true);
      },
    });
  }

  protected setupEventListeners(): void {
    const unsub = this.eventBus.on('state:gold:changed', (payload) => {
      ToastManager.getInstance().info(`Gold: ${payload.gold} (+${payload.delta})`);
    });
    this.subscribe(unsub);
  }
}
```

---

## テストチェックリスト

### コンポーネント単体動作

- [ ] EventBus: emit/on/off/once/clear
- [ ] SceneManager: goTo/replace/goBack/openOverlay/closeOverlay
- [ ] UIFactory: createButton/createLabel/createDialog/createProgressBar/createScrollPanel/createGridButtons
- [ ] DialogManager: show/confirm/alert/close/closeAll
- [ ] ToastManager: success/error/warning/info/clearAll

### コンポーネント連携

- [ ] EventBus → UIコンポーネント更新
- [ ] UIコンポーネント → EventBus発行
- [ ] DialogManager → UIFactory連携
- [ ] ToastManager → シーン連携

### メモリ管理

- [ ] シーン破棄時のリスナー解除
- [ ] ダイアログ閉じ時のオブジェクト破棄
- [ ] トースト消去時のオブジェクト破棄

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0181` - 詳細要件定義
2. `/tsumiki:tdd-testcases` - テストケース作成
3. `/tsumiki:tdd-red` - テスト実装（失敗）
4. `/tsumiki:tdd-green` - 最小実装
5. `/tsumiki:tdd-refactor` - リファクタリング
6. `/tsumiki:tdd-verify-complete` - 品質確認

---

## 注意事項

- Phaserのテスト環境構築が必要
- 一部テストはheadlessモードまたはブラウザで実行
- 手動確認用のデモシーンを活用

---

## 信頼性レベルサマリー

- **総項目数**: 3項目
- 🔵 **青信号**: 0項目 (0%)
- 🟡 **黄信号**: 0項目 (0%)
- 🔴 **赤信号**: 3項目 (100%)

**品質評価**: 要調査

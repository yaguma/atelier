...
ゲームオーバー条件と結果表示の統合テストを実施する。日数超過、ゴールド枯渇等のゲームオーバー判定、統計表示、リトライ機能が正しく動作することを検証する。

## 依存タスク

- **前提タスク**: TASK-0247 (GameOverScene実装), TASK-0263 (複数日進行テスト)
- **後続タスク**: TASK-0269

## 完了条件

- [x] 日数超過判定の統合テストがパスする
- [x] ゴールド枯渇判定の統合テストがパスする
- [x] ゲームオーバー統計表示の統合テストがパスする
- [x] リトライ機能の統合テストがパスする
...
**テストファイル**: `tests/integration/phaser/phase5/GameOverIntegration.test.ts`
  ...
  describe('Game Over Reasons', () => {
    it('日数超過の理由メッセージが適切', async () => {
      ...
      await vi.waitFor(() => {
        ...
        expect(call[0].reason).toMatch(/期限|30日|Sランク/);
      });
    });
    ...
    it('ゴールド枯渇の理由メッセージが適切', async () => {
      ...
      await vi.waitFor(() => {
        const call = gameOverCallback.mock.calls[0];
        expect(call[0].reason).toMatch(/所持金|ゴールド|0/);
      });
    });
  });
...
1. `/tsumiki:tdd-requirements TASK-0267` - 詳細要件定義

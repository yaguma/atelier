...
強化カード（EnhancementCard）の視覚的表現を実装する。
...
- [ ] EnhancementCardViewクラスが実装されている
- [ ] カード名が表示される
- [ ] APコストが表示される
- [ ] 効果説明が表示される
- [ ] 効果タイプに応じたアイコンが表示される
...
### 1. EnhancementCardView実装 🟡

**信頼性**: 🟡 *GatheringCardViewを参考*

**実装ファイル**: `src/presentation/phaser/ui/card/EnhancementCardView.ts`
...
export class EnhancementCardView implements ICardView {
  ...
  private getEffectIcon(): string {
    ...
    switch (effectType) {
      case 'draw': return '🃏';
      case 'ap_restore': return '⚡';
      case 'quality_up': return '⬆️';
  ...
  private getEffectDescription(): string {
    return this.card.description ?? '効果なし';
  }
}
```
...
**Given**: EnhancementCardデータがある
**When**: EnhancementCardViewを生成する
**Then**: カードが表示される

### テストケース2: 効果アイコン表示 🟡

**Given**: EnhancementCardViewがある
**When**: 効果タイプを確認する
**Then**: 対応するアイコンが表示されている

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0192` - 詳細要件定義

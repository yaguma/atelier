# TASK-0187: TitleSceneコンティニューボタン実装

**タスクID**: TASK-0187
**タスクタイプ**: TDD
**推定工数**: 4時間
**フェーズ**: Phase 2 - 基本シーン・共通UI実装
**信頼性レベル**: 🔵 *設計書に明確な記載あり*

## 関連文書

- **概要**: [overview.md](./overview.md)
- **UI設計**: [ui-design/overview.md](../../design/atelier-guild-rank-phaser/ui-design/overview.md)
- **データフロー**: [dataflow.md](../../design/atelier-guild-rank-phaser/dataflow.md)

## タスク概要

タイトル画面の「コンティニュー」ボタンを実装する。セーブデータがない場合は無効化する。

## 依存タスク

- **前提タスク**: TASK-0186
- **後続タスク**: TASK-0188

## 完了条件

- [ ] コンティニューボタンが表示される
- [ ] セーブデータがない場合は無効化される
- [ ] ボタンクリックでイベントが発行される
- [ ] クリック後にMainSceneへ遷移する

---

## 実装詳細

### 1. コンティニューボタン実装 🔵

**信頼性**: 🔵 *フローが設計書に記載*

**実装ファイル**: `src/presentation/phaser/scenes/TitleScene.ts` に追加

```typescript
import Label from 'phaser3-rex-plugins/templates/ui/label/Label';

// TitleSceneクラス内

private continueButton!: Label;
private hasSaveData = false;

protected onInit(data?: SceneInitData): void {
  // セーブデータの存在確認
  this.hasSaveData = this.checkSaveData();
}

private createMenuButtons(): void {
  // 新規ゲームボタン（TASK-0186で実装済み）
  this.newGameButton = this.uiFactory.createButton({
    x: 0,
    y: 0,
    text: '新規ゲーム',
    width: 200,
    height: 50,
    onClick: () => this.onNewGameClick(),
  });
  this.menuContainer.add(this.newGameButton);

  // コンティニューボタン
  this.continueButton = this.uiFactory.createButton({
    x: 0,
    y: 70,
    text: 'コンティニュー',
    width: 200,
    height: 50,
    onClick: () => this.onContinueClick(),
    disabled: !this.hasSaveData,
  });
  this.menuContainer.add(this.continueButton);

  // セーブデータがない場合のヒント
  if (!this.hasSaveData) {
    this.add.text(
      this.cameras.main.centerX,
      530,
      'セーブデータがありません',
      { ...TextStyles.bodySmall, color: '#888888' }
    ).setOrigin(0.5);
  }
}

private checkSaveData(): boolean {
  // LocalStorageにセーブデータがあるかチェック
  try {
    const saveData = localStorage.getItem('atelier-guild-rank-save');
    return saveData !== null;
  } catch {
    return false;
  }
}

private onContinueClick(): void {
  if (!this.hasSaveData) return;

  // SE再生
  if (this.cache.audio.exists(AudioKeys.SE_CLICK)) {
    this.sound.play(AudioKeys.SE_CLICK);
  }

  // BGM停止
  this.sound.stopAll();

  // イベント発行
  this.eventBus.emitVoid(UIActionEvents.CONTINUE_CLICKED);

  // MainSceneへ遷移（ロードフラグ付き）
  SceneManager.getInstance().goTo(SceneKeys.MAIN, {
    isNewGame: false,
    loadSave: true,
  });
}
```

### 2. ボタン状態更新メソッド 🔵

**信頼性**: 🔵 *UIFactory連携*

```typescript
// セーブデータ状態が変わった場合の更新
public updateContinueButtonState(): void {
  this.hasSaveData = this.checkSaveData();
  this.uiFactory.setButtonEnabled(this.continueButton, this.hasSaveData);
}
```

---

## 単体テスト要件

### テストケース1: ボタン表示 🔵

**Given**: TitleSceneが作成された
**When**: メニューコンテナを確認する
**Then**: コンティニューボタンが存在する

### テストケース2: セーブデータなしで無効化 🔵

**Given**: セーブデータがない
**When**: TitleSceneが作成される
**Then**: コンティニューボタンが無効状態である

### テストケース3: セーブデータありで有効化 🔵

**Given**: セーブデータがある
**When**: TitleSceneが作成される
**Then**: コンティニューボタンが有効状態である

### テストケース4: クリックイベント 🔵

**Given**: コンティニューボタンが有効
**When**: ボタンをクリックする
**Then**: CONTINUE_CLICKEDイベントが発行される

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0187` - 詳細要件定義
2. `/tsumiki:tdd-testcases` - テストケース作成
3. `/tsumiki:tdd-red` - テスト実装（失敗）
4. `/tsumiki:tdd-green` - 最小実装
5. `/tsumiki:tdd-refactor` - リファクタリング
6. `/tsumiki:tdd-verify-complete` - 品質確認

---

## 注意事項

- LocalStorageアクセスのエラーハンドリング
- 無効状態のビジュアル表現
- セーブデータ破損時の処理

---

## 信頼性レベルサマリー

- **総項目数**: 2項目
- 🔵 **青信号**: 2項目 (100%)
- 🟡 **黄信号**: 0項目 (0%)
- 🔴 **赤信号**: 0項目 (0%)

**品質評価**: 高品質

# TASK-0171: SceneManagerオーバーレイ機能実装

**タスクID**: TASK-0171
**タスクタイプ**: TDD
**推定工数**: 4時間
**フェーズ**: Phase 1 - Phaser基盤・インフラ構築
**信頼性レベル**: 🟡 *妥当な推測に基づく*

## 関連文書

- **概要**: [overview.md](./overview.md)
- **コアシステム**: [core-systems.md](../../design/atelier-guild-rank-phaser/core-systems.md)

## タスク概要

SceneManagerのオーバーレイ機能（openOverlay、closeOverlay、closeAllOverlays）を実装する。

## 依存タスク

- **前提タスク**: TASK-0170
- **後続タスク**: TASK-0181

## 完了条件

- [ ] openOverlay()メソッドが動作する
- [ ] closeOverlay()メソッドが動作する
- [ ] closeAllOverlays()メソッドが動作する
- [ ] isOverlayOpen()メソッドが動作する
- [ ] 複数オーバーレイの重ね順が管理される

---

## 実装詳細

### 1. オーバーレイ機能実装 🟡

**信頼性**: 🟡 *Phaserのlaunch/stopを使用*

**実装ファイル**: `src/presentation/phaser/managers/SceneManager.ts`に追加

```typescript
// SceneManagerクラスに追加するメソッド

async openOverlay(
  sceneKey: SceneKey,
  data?: Record<string, unknown>,
  transition: TransitionConfig = DefaultTransitions.overlay
): Promise<void> {
  if (!this.game || this.openOverlays.has(sceneKey)) return;

  const scenePlugin = this.game.scene;
  const currentScene = this.currentScene ? scenePlugin.getScene(this.currentScene) : null;

  // 現在のシーンを一時停止
  if (currentScene) {
    currentScene.scene.pause();
  }

  // オーバーレイシーンを起動
  scenePlugin.launch(sceneKey, data);
  this.openOverlays.add(sceneKey);

  // フェードイン
  if (transition.type === 'fade' && transition.duration > 0) {
    const overlayScene = scenePlugin.getScene(sceneKey);
    if (overlayScene) {
      overlayScene.cameras.main.setAlpha(0);
      overlayScene.tweens.add({
        targets: overlayScene.cameras.main,
        alpha: 1,
        duration: transition.duration,
      });
    }
  }

  this.eventBus.emit('scene:overlay:opened' as any, { sceneKey });
}

async closeOverlay(
  sceneKey: SceneKey,
  transition: TransitionConfig = DefaultTransitions.overlay
): Promise<void> {
  if (!this.game || !this.openOverlays.has(sceneKey)) return;

  const scenePlugin = this.game.scene;
  const overlayScene = scenePlugin.getScene(sceneKey);

  // フェードアウト
  if (transition.type === 'fade' && transition.duration > 0 && overlayScene) {
    await new Promise<void>(resolve => {
      overlayScene.tweens.add({
        targets: overlayScene.cameras.main,
        alpha: 0,
        duration: transition.duration,
        onComplete: () => resolve(),
      });
    });
  }

  // オーバーレイシーンを停止
  scenePlugin.stop(sceneKey);
  this.openOverlays.delete(sceneKey);

  // 他にオーバーレイがなければ、メインシーンを再開
  if (this.openOverlays.size === 0 && this.currentScene) {
    const mainScene = scenePlugin.getScene(this.currentScene);
    if (mainScene) {
      mainScene.scene.resume();
    }
  }

  this.eventBus.emit('scene:overlay:closed' as any, { sceneKey });
}

async closeAllOverlays(): Promise<void> {
  const overlays = [...this.openOverlays];
  for (const overlay of overlays) {
    await this.closeOverlay(overlay, DefaultTransitions.quick);
  }
}

isOverlayOpen(sceneKey: SceneKey): boolean {
  return this.openOverlays.has(sceneKey);
}

getOpenOverlays(): SceneKey[] {
  return [...this.openOverlays];
}
```

### 2. オーバーレイの重ね順管理 🟡

**信頼性**: 🟡 *PhaserのbringToTop使用*

```typescript
// オーバーレイを最前面に持ってくる
private bringOverlayToTop(sceneKey: SceneKey): void {
  if (!this.game) return;
  this.game.scene.bringToTop(sceneKey);
}
```

---

## 単体テスト要件

### テストケース1: オーバーレイを開く 🟡

**Given**: MainSceneがアクティブ
**When**: openOverlay('ShopScene')を呼ぶ
**Then**: ShopSceneがオーバーレイとして表示される

### テストケース2: オーバーレイを閉じる 🟡

**Given**: ShopSceneがオーバーレイで開いている
**When**: closeOverlay('ShopScene')を呼ぶ
**Then**: ShopSceneが閉じられMainSceneが再開する

### テストケース3: isOverlayOpen 🟡

**Given**: ShopSceneがオーバーレイで開いている
**When**: isOverlayOpen('ShopScene')を呼ぶ
**Then**: trueが返る

### テストケース4: closeAllOverlays 🟡

**Given**: 複数のオーバーレイが開いている
**When**: closeAllOverlays()を呼ぶ
**Then**: すべてのオーバーレイが閉じられる

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0171` - 詳細要件定義
2. `/tsumiki:tdd-testcases` - テストケース作成
3. `/tsumiki:tdd-red` - テスト実装（失敗）
4. `/tsumiki:tdd-green` - 最小実装
5. `/tsumiki:tdd-refactor` - リファクタリング
6. `/tsumiki:tdd-verify-complete` - 品質確認

---

## 注意事項

- オーバーレイ背景の半透明処理
- 複数オーバーレイの入力ブロック
- ESCキーでオーバーレイを閉じる機能を検討

---

## 信頼性レベルサマリー

- **総項目数**: 2項目
- 🔵 **青信号**: 0項目 (0%)
- 🟡 **黄信号**: 2項目 (100%)
- 🔴 **赤信号**: 0項目 (0%)

**品質評価**: 中品質

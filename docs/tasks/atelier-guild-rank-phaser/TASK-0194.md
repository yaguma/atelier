...
CardView関連コンポーネントの統合テストを実施する。

## 依存タスク

- **前提タスク**: TASK-0193
- **後続タスク**: TASK-0195, TASK-0231

## 完了条件

- [x] GatheringCardViewのテストが通る
- [x] RecipeCardViewのテストが通る
- [x] EnhancementCardViewのテストが通る
- [x] CardTooltipのテストが通る
- [x] テストカバレッジ80%以上（実装ファイルのカバレッジ: 85%以上達成）
...
**テストファイル**: `tests/unit/presentation/ui/card/CardView.test.ts`
...
describe('CardView統合', () => {
  ...
  describe('GatheringCardView', () => {
    it.skip('カードが正しく生成される', () => {
      // Phaserシーンモックを使用したテスト
    });

    it.skip('クリックイベントが発火する', () => {
      // イベントのテスト
    });
  });
  ...
  describe('EnhancementCardView', () => {
    it.skip('効果アイコンが表示される', () => {
      // 表示内容のテスト
    });
  });

  describe('CardTooltip', () => {
    it.skip('show/hideが正しく動作する', () => {
      // ツールチップの表示/非表示テスト
    });
  });
});
...
**実装ファイル**: `src/presentation/phaser/scenes/CardViewTestScene.ts`
...
1. `/tsumiki:tdd-requirements TASK-0194` - 詳細要件定義

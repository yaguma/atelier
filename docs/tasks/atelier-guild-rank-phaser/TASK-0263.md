...
- **ゲームフロー**: [core-systems.md](../../design/atelier-guild-rank-phaser/core-systems.md)

## タスク概要

複数日（複数ターン）にわたるゲーム進行が正しく動作することを検証する統合テストを実施する。日数経過、ランク進行、ゲームオーバー/クリア条件を検証する。

## 依存タスク

- **前提タスク**: TASK-0261, TASK-0262
- **後続タスク**: TASK-0266, TASK-0267

## 完了条件 ✅ **完了** (TDD開発完了 - 18テストケース全通過)

- [x] 複数日進行の統合テストがパスする（TC-01〜TC-04）
- [x] ランク進行の統合テストがパスする（TC-05〜TC-07）
- [x] 日数制限の統合テストがパスする（TC-08〜TC-10）
- [x] 経験値・ゴールド累積の検証テストがパスする（TC-05, TC-07）
- [x] 新規依頼生成の検証テストがパスする（TC-13〜TC-16）
...
**テストファイル**: `tests/integration/phaser/phase5/MultiDayProgression.test.ts`
...
describe('Multi-Day Progression Integration', () => {
  ...
  describe('Experience and Rank Progression', () => {
    ...
    it('経験値が上限に達するとランクアップ可能になる', async () => {
      // Arrange
  ...
  describe('Day Limit', () => {
    ...
    it('最大日数前にSランクに到達するとゲームクリア', async () => {
      // Arrange
...
1. `/tsumiki:tdd-requirements TASK-0263` - 詳細要件定義

# TASK-0183: BootSceneプリロード実装

**タスクID**: TASK-0183
**タスクタイプ**: TDD
**推定工数**: 4時間
**フェーズ**: Phase 2 - 基本シーン・共通UI実装
**信頼性レベル**: 🔴 *Phaser固有の調査が必要*

## 関連文書

- **概要**: [overview.md](./overview.md)
- **設計文書**: [architecture.md](../../design/atelier-guild-rank-phaser/architecture.md)

## タスク概要

BootSceneでアセットをプリロードする機能を実装する。

## 依存タスク

- **前提タスク**: TASK-0182
- **後続タスク**: TASK-0184, TASK-0256

## 完了条件

- [x] BootSceneがBaseGameSceneを継承している
- [x] preload()で全アセットが読み込まれる
- [x] 読み込み完了後にTitleSceneに遷移する
- [x] エラーハンドリングが実装されている

---

## 実装詳細

### 1. BootScene実装 🔴

**信頼性**: 🔴 *Phaserのプリロード機構を使用*

**実装ファイル**: `src/presentation/phaser/scenes/BootScene.ts`

```typescript
import { BaseGameScene, SceneInitData } from './BaseGameScene';
import { SceneKeys } from '../config/SceneKeys';
import { AllAssets, getTotalAssetCount } from '../boot/AssetList';
import { SceneManager } from '../managers/SceneManager';

export class BootScene extends BaseGameScene {
  private loadedCount = 0;
  private totalCount = 0;
  private loadingText!: Phaser.GameObjects.Text;
  private progressCallback?: (progress: number) => void;

  constructor() {
    super(SceneKeys.BOOT);
  }

  protected onInit(data?: SceneInitData): void {
    this.loadedCount = 0;
    this.totalCount = getTotalAssetCount();
  }

  protected onPreload(): void {
    // ローディングテキスト（プログレスバーはTASK-0184で実装）
    this.loadingText = this.add.text(
      this.cameras.main.centerX,
      this.cameras.main.centerY,
      'Loading...',
      { fontSize: '24px', color: '#ffffff' }
    ).setOrigin(0.5);

    // プログレスイベント
    this.load.on('progress', (value: number) => {
      this.loadingText.setText(`Loading... ${Math.round(value * 100)}%`);
      if (this.progressCallback) {
        this.progressCallback(value);
      }
    });

    // 完了イベント
    this.load.on('complete', () => {
      console.log('All assets loaded');
    });

    // エラーイベント
    this.load.on('loaderror', (file: Phaser.Loader.File) => {
      console.error('Load error:', file.key, file.url);
    });

    // アセット読み込み
    this.loadAllAssets();
  }

  protected onCreate(data?: SceneInitData): void {
    // 読み込み完了、TitleSceneへ遷移
    this.time.delayedCall(500, () => {
      SceneManager.getInstance().goTo(SceneKeys.TITLE);
    });
  }

  protected setupEventListeners(): void {
    // BootSceneではイベントリスナーは不要
  }

  /**
   * プログレスコールバックを設定（TASK-0184で使用）
   */
  public setProgressCallback(callback: (progress: number) => void): void {
    this.progressCallback = callback;
  }

  private loadAllAssets(): void {
    AllAssets.forEach(asset => {
      switch (asset.type) {
        case 'image':
          this.load.image(asset.key, asset.path);
          break;
        case 'audio':
          this.load.audio(asset.key, asset.path);
          break;
        case 'json':
          this.load.json(asset.key, asset.path);
          break;
        case 'spritesheet':
          if (asset.frameConfig) {
            this.load.spritesheet(asset.key, asset.path, asset.frameConfig);
          }
          break;
      }
    });
  }
}
```

### 2. シーン登録 🔴

**信頼性**: 🔴 *エントリーポイントで登録*

```typescript
// src/presentation/phaser/scenes/index.ts に追加
export { BootScene } from './BootScene';
```

---

## 単体テスト要件

### テストケース1: アセット読み込み開始 🔴

**Given**: BootSceneが作成される
**When**: preload()が呼ばれる
**Then**: 全アセットの読み込みが開始される

### テストケース2: 読み込み完了後の遷移 🔴

**Given**: 全アセットの読み込みが完了した
**When**: create()が呼ばれる
**Then**: TitleSceneに遷移する

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0183` - 詳細要件定義
2. `/tsumiki:tdd-testcases` - テストケース作成
3. `/tsumiki:tdd-red` - テスト実装（失敗）
4. `/tsumiki:tdd-green` - 最小実装
5. `/tsumiki:tdd-refactor` - リファクタリング
6. `/tsumiki:tdd-verify-complete` - 品質確認

---

## 注意事項

- アセットが存在しない場合のフォールバック処理
- 大量アセット読み込み時のパフォーマンス
- 読み込みエラーのログ出力

---

## 信頼性レベルサマリー

- **総項目数**: 2項目
- 🔵 **青信号**: 0項目 (0%)
- 🟡 **黄信号**: 0項目 (0%)
- 🔴 **赤信号**: 2項目 (100%)

**品質評価**: 要調査

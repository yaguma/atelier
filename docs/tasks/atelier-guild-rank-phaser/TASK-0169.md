# TASK-0169: SceneManagerインターフェース定義

**タスクID**: TASK-0169
**タスクタイプ**: TDD
**推定工数**: 4時間
**フェーズ**: Phase 1 - Phaser基盤・インフラ構築
**信頼性レベル**: 🟡 *妥当な推測に基づく*

## 関連文書

- **概要**: [overview.md](./overview.md)
- **コアシステム**: [core-systems.md](../../design/atelier-guild-rank-phaser/core-systems.md)
- **UI設計**: [ui-design/overview.md](../../design/atelier-guild-rank-phaser/ui-design/overview.md)

## タスク概要

シーン遷移を管理するSceneManagerのインターフェースを定義する。通常遷移とオーバーレイ遷移の両方をサポートする。

## 依存タスク

- **前提タスク**: TASK-0168
- **後続タスク**: TASK-0170

## 完了条件

- [ ] ISceneManagerインターフェースが定義されている
- [ ] シーン遷移メソッドが定義されている
- [ ] オーバーレイ管理メソッドが定義されている
- [ ] 遷移履歴管理メソッドが定義されている

---

## 実装詳細

### 1. シーン遷移タイプ定義 🟡

**信頼性**: 🟡 *ui-design/overview.mdを参考*

**実装ファイル**: `src/presentation/phaser/managers/SceneTransition.ts`

```typescript
import { SceneKey } from '../config/SceneKeys';

export type TransitionType = 'fade' | 'slide' | 'none';

export interface TransitionConfig {
  type: TransitionType;
  duration: number;
  direction?: 'left' | 'right' | 'up' | 'down';
}

export const DefaultTransitions: Record<string, TransitionConfig> = {
  standard: {
    type: 'fade',
    duration: 300,
  },
  quick: {
    type: 'none',
    duration: 0,
  },
  overlay: {
    type: 'fade',
    duration: 200,
  },
};

export interface SceneTransitionData {
  from: SceneKey;
  to: SceneKey;
  transition: TransitionConfig;
  data?: Record<string, unknown>;
}
```

### 2. ISceneManagerインターフェース 🟡

**信頼性**: 🟡 *標準的なシーン管理パターン*

**実装ファイル**: `src/presentation/phaser/managers/ISceneManager.ts`

```typescript
import { SceneKey } from '../config/SceneKeys';
import { TransitionConfig, SceneTransitionData } from './SceneTransition';

export interface ISceneManager {
  /**
   * 現在のシーンキーを取得
   */
  getCurrentScene(): SceneKey;

  /**
   * シーンに遷移（既存シーンは停止）
   */
  goTo(
    sceneKey: SceneKey,
    data?: Record<string, unknown>,
    transition?: TransitionConfig
  ): Promise<void>;

  /**
   * シーンを置き換え（履歴に残さない）
   */
  replace(
    sceneKey: SceneKey,
    data?: Record<string, unknown>,
    transition?: TransitionConfig
  ): Promise<void>;

  /**
   * 前のシーンに戻る
   */
  goBack(transition?: TransitionConfig): Promise<boolean>;

  /**
   * オーバーレイシーンを開く
   */
  openOverlay(
    sceneKey: SceneKey,
    data?: Record<string, unknown>,
    transition?: TransitionConfig
  ): Promise<void>;

  /**
   * オーバーレイシーンを閉じる
   */
  closeOverlay(
    sceneKey: SceneKey,
    transition?: TransitionConfig
  ): Promise<void>;

  /**
   * すべてのオーバーレイを閉じる
   */
  closeAllOverlays(): Promise<void>;

  /**
   * オーバーレイが開いているかチェック
   */
  isOverlayOpen(sceneKey: SceneKey): boolean;

  /**
   * 開いているオーバーレイ一覧を取得
   */
  getOpenOverlays(): SceneKey[];

  /**
   * シーン遷移履歴を取得
   */
  getHistory(): SceneTransitionData[];

  /**
   * 履歴をクリア
   */
  clearHistory(): void;

  /**
   * 遷移中かどうか
   */
  isTransitioning(): boolean;
}
```

### 3. シーン遷移イベント 🟡

**信頼性**: 🟡 *EventBusと連携*

**追加イベント定義**:
```typescript
// EventTypes.tsに追加
export const SceneEvents = {
  SCENE_TRANSITION_START: 'scene:transition:start',
  SCENE_TRANSITION_COMPLETE: 'scene:transition:complete',
  OVERLAY_OPENED: 'scene:overlay:opened',
  OVERLAY_CLOSED: 'scene:overlay:closed',
} as const;
```

---

## 単体テスト要件

### テストケース1: インターフェース型チェック 🟡

**Given**: ISceneManagerインターフェースが定義されている
**When**: 型チェックを実行する
**Then**: コンパイルエラーがない

### テストケース2: TransitionConfig型チェック 🟡

**Given**: TransitionConfigが定義されている
**When**: 各遷移タイプを生成する
**Then**: 型エラーがない

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0169` - 詳細要件定義
2. `/tsumiki:tdd-testcases` - テストケース作成
3. `/tsumiki:tdd-red` - テスト実装（失敗）
4. `/tsumiki:tdd-green` - 最小実装
5. `/tsumiki:tdd-refactor` - リファクタリング
6. `/tsumiki:tdd-verify-complete` - 品質確認

---

## 注意事項

- 遷移中の連続遷移要求への対応を考慮
- オーバーレイの重ね順管理
- 履歴の最大保持数を検討

---

## 信頼性レベルサマリー

- **総項目数**: 3項目
- 🔵 **青信号**: 0項目 (0%)
- 🟡 **黄信号**: 3項目 (100%)
- 🔴 **赤信号**: 0項目 (0%)

**品質評価**: 中品質

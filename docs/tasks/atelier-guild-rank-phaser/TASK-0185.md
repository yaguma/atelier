# TASK-0185: TitleSceneレイアウト設計

**タスクID**: TASK-0185
**タスクタイプ**: TDD
**推定工数**: 4時間
**フェーズ**: Phase 2 - 基本シーン・共通UI実装
**信頼性レベル**: 🔵 *設計書に明確な記載あり*

## 関連文書

- **概要**: [overview.md](./overview.md)
- **UI設計**: [ui-design/overview.md](../../design/atelier-guild-rank-phaser/ui-design/overview.md)

## タスク概要

タイトル画面のレイアウトを設計・実装する。

## 依存タスク

- **前提タスク**: TASK-0170, TASK-0173
- **後続タスク**: TASK-0186

## 完了条件

- [x] TitleSceneがBaseGameSceneを継承している
- [x] タイトルロゴが表示される
- [x] メニューボタンの配置位置が決定している
- [x] 背景が表示される
- [x] フェードイン演出がある

---

## 実装詳細

### 1. TitleSceneレイアウト 🔵

**信頼性**: 🔵 *ui-design/overview.mdに記載*

**実装ファイル**: `src/presentation/phaser/scenes/TitleScene.ts`

```typescript
import { BaseGameScene, SceneInitData } from './BaseGameScene';
import { SceneKeys } from '../config/SceneKeys';
import { ImageKeys, AudioKeys } from '../config/AssetKeys';
import { Colors } from '../config/ColorPalette';
import { TextStyles } from '../config/TextStyles';

export class TitleScene extends BaseGameScene {
  private titleText!: Phaser.GameObjects.Text;
  private subtitleText!: Phaser.GameObjects.Text;
  private menuContainer!: Phaser.GameObjects.Container;

  constructor() {
    super(SceneKeys.TITLE);
  }

  protected onInit(data?: SceneInitData): void {}

  protected onPreload(): void {
    // BootSceneでプリロード済み
  }

  protected onCreate(data?: SceneInitData): void {
    const { width, height } = this.cameras.main;
    const centerX = width / 2;

    // 背景
    this.cameras.main.setBackgroundColor(Colors.background);

    // 背景画像（存在する場合）
    if (this.textures.exists(ImageKeys.BG_TITLE)) {
      this.add.image(centerX, height / 2, ImageKeys.BG_TITLE)
        .setDisplaySize(width, height);
    }

    // タイトル
    this.titleText = this.add.text(centerX, 180, 'Atelier Guild Rank', {
      ...TextStyles.titleLarge,
      fontSize: '56px',
    }).setOrigin(0.5);

    // サブタイトル
    this.subtitleText = this.add.text(centerX, 250, '〜錬金術師ギルド物語〜', {
      ...TextStyles.titleMedium,
      fontSize: '24px',
    }).setOrigin(0.5);

    // メニューコンテナ
    this.menuContainer = this.add.container(centerX, 450);

    // ボタンは後続タスクで実装
    this.createMenuButtons();

    // バージョン表示
    this.add.text(width - 10, height - 10, 'v0.1.0', {
      ...TextStyles.bodySmall,
      fontSize: '12px',
    }).setOrigin(1, 1);

    // フェードイン
    this.cameras.main.fadeIn(500, 0, 0, 0);

    // BGM再生（存在する場合）
    if (this.cache.audio.exists(AudioKeys.BGM_TITLE)) {
      this.sound.play(AudioKeys.BGM_TITLE, { loop: true, volume: 0.5 });
    }
  }

  protected setupEventListeners(): void {
    // TitleSceneではゲーム状態のイベントは不要
  }

  private createMenuButtons(): void {
    // TASK-0186, TASK-0187で実装
  }

  // ボタンを追加するためのpublicメソッド
  public addMenuButton(button: Phaser.GameObjects.GameObject, yOffset: number): void {
    button.setPosition(0, yOffset);
    this.menuContainer.add(button);
  }
}
```

### 2. レイアウト仕様 🔵

**信頼性**: 🔵 *設計書に基づく*

```
┌─────────────────────────────────────────┐
│                                         │
│                                         │
│        Atelier Guild Rank              │ y=180
│      〜錬金術師ギルド物語〜              │ y=250
│                                         │
│                                         │
│                                         │
│           [新規ゲーム]                  │ y=420
│           [コンティニュー]              │ y=490
│                                         │
│                                         │
│                              v0.1.0     │
└─────────────────────────────────────────┘
```

---

## 単体テスト要件

### テストケース1: シーン作成 🔵

**Given**: TitleSceneが開始される
**When**: create()が呼ばれる
**Then**: タイトルテキストが表示される

### テストケース2: フェードイン 🔵

**Given**: TitleSceneが作成された
**When**: カメラを確認する
**Then**: フェードインが実行されている

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0185` - 詳細要件定義
2. `/tsumiki:tdd-testcases` - テストケース作成
3. `/tsumiki:tdd-red` - テスト実装（失敗）
4. `/tsumiki:tdd-green` - 最小実装
5. `/tsumiki:tdd-refactor` - リファクタリング
6. `/tsumiki:tdd-verify-complete` - 品質確認

---

## 注意事項

- 背景画像がない場合のフォールバック
- BGMがない場合のエラー回避
- レスポンシブ対応は将来課題

---

## 信頼性レベルサマリー

- **総項目数**: 2項目
- 🔵 **青信号**: 2項目 (100%)
- 🟡 **黄信号**: 0項目 (0%)
- 🔴 **赤信号**: 0項目 (0%)

**品質評価**: 高品質

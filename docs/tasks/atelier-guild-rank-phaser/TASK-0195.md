# TASK-0195: HandContainer設計

**タスクID**: TASK-0195
**タスクタイプ**: TDD
**推定工数**: 4時間
**フェーズ**: Phase 2 - 基本シーン・共通UI実装
**信頼性レベル**: 🟡 *妥当な推測に基づく*

## 関連文書

- **概要**: [overview.md](./overview.md)
- **UI設計**: [ui-design/overview.md](../../design/atelier-guild-rank-phaser/ui-design/overview.md)

## タスク概要

手札表示コンテナのインターフェースと基本構造を設計する。

## 依存タスク

- **前提タスク**: TASK-0178, TASK-0194
- **後続タスク**: TASK-0196

## 完了条件

- [x] IHandContainerインターフェースが定義されている
- [x] 手札レイアウト定数が定義されている
- [x] カードの並べ方（扇形/水平）が定義されている
- [x] 選択状態の管理方法が設計されている

---

## 実装詳細

### 1. 手札レイアウト定数 🟡

**信頼性**: 🟡 *ui-design/overview.mdを参考*

**実装ファイル**: `src/presentation/phaser/ui/hand/HandConstants.ts`

```typescript
export const HandLayout = {
  // 位置（画面下部中央）
  X: 640,
  Y: 650,

  // 表示設定
  MAX_VISIBLE_CARDS: 7,
  CARD_SPACING: 100,  // カード間隔（水平配置時）
  CARD_OVERLAP: 30,   // カード重なり（扇形配置時）

  // 扇形配置
  FAN_ANGLE_RANGE: 30, // 扇形の角度範囲（度）
  FAN_RADIUS: 400,     // 扇形の半径

  // アニメーション
  CARD_MOVE_DURATION: 200,
  CARD_HOVER_OFFSET: -30,
} as const;

export type HandLayoutType = 'horizontal' | 'fan';
```

### 2. IHandContainerインターフェース 🟡

**信頼性**: 🟡 *コンポーネント設計*

**実装ファイル**: `src/presentation/phaser/ui/hand/IHandContainer.ts`

```typescript
import Phaser from 'phaser';
import { Card } from '../../../../domain/card/Card';
import { ICardView } from '../card/ICardView';
import { HandLayoutType } from './HandConstants';

export interface HandContainerOptions {
  x?: number;
  y?: number;
  layoutType?: HandLayoutType;
  maxVisibleCards?: number;
  onCardSelect?: (card: Card, index: number) => void;
  onCardDeselect?: (card: Card, index: number) => void;
}

export interface IHandContainer {
  // コンテナ参照
  readonly container: Phaser.GameObjects.Container;

  // 手札管理
  setCards(cards: Card[]): void;
  addCard(card: Card, animate?: boolean): void;
  removeCard(cardOrIndex: Card | number, animate?: boolean): void;
  getCards(): Card[];
  getCardCount(): number;

  // 選択管理
  getSelectedCard(): Card | null;
  getSelectedIndex(): number;
  selectCard(cardOrIndex: Card | number): void;
  deselectCard(): void;
  setSelectable(selectable: boolean): void;

  // レイアウト
  setLayoutType(type: HandLayoutType): void;
  getLayoutType(): HandLayoutType;
  refresh(): void;

  // 表示
  setVisible(visible: boolean): void;
  setPosition(x: number, y: number): void;

  // 破棄
  destroy(): void;
}
```

### 3. カード配置計算ユーティリティ 🟡

**信頼性**: 🟡 *レイアウト計算*

**実装ファイル**: `src/presentation/phaser/ui/hand/HandLayoutUtils.ts`

```typescript
import { HandLayout, HandLayoutType } from './HandConstants';

export interface CardPosition {
  x: number;
  y: number;
  rotation: number;
  scale: number;
  depth: number;
}

export const calculateCardPositions = (
  cardCount: number,
  layoutType: HandLayoutType,
  centerX: number = HandLayout.X,
  centerY: number = HandLayout.Y
): CardPosition[] => {
  if (layoutType === 'horizontal') {
    return calculateHorizontalLayout(cardCount, centerX, centerY);
  } else {
    return calculateFanLayout(cardCount, centerX, centerY);
  }
};

const calculateHorizontalLayout = (
  cardCount: number,
  centerX: number,
  centerY: number
): CardPosition[] => {
  const positions: CardPosition[] = [];
  const totalWidth = (cardCount - 1) * HandLayout.CARD_SPACING;
  const startX = centerX - totalWidth / 2;

  for (let i = 0; i < cardCount; i++) {
    positions.push({
      x: startX + i * HandLayout.CARD_SPACING,
      y: centerY,
      rotation: 0,
      scale: 1,
      depth: i,
    });
  }

  return positions;
};

const calculateFanLayout = (
  cardCount: number,
  centerX: number,
  centerY: number
): CardPosition[] => {
  const positions: CardPosition[] = [];

  if (cardCount === 0) return positions;
  if (cardCount === 1) {
    return [{ x: centerX, y: centerY, rotation: 0, scale: 1, depth: 0 }];
  }

  const angleRange = HandLayout.FAN_ANGLE_RANGE * (Math.PI / 180);
  const startAngle = -angleRange / 2;
  const angleStep = angleRange / (cardCount - 1);
  const radius = HandLayout.FAN_RADIUS;

  for (let i = 0; i < cardCount; i++) {
    const angle = startAngle + i * angleStep;
    positions.push({
      x: centerX + Math.sin(angle) * radius,
      y: centerY - Math.cos(angle) * radius + radius,
      rotation: angle,
      scale: 1,
      depth: i,
    });
  }

  return positions;
};
```

---

## 単体テスト要件

### テストケース1: 水平レイアウト計算 🟡

**Given**: カード5枚
**When**: calculateHorizontalLayoutを呼ぶ
**Then**: 5つの位置が等間隔で返される

### テストケース2: 扇形レイアウト計算 🟡

**Given**: カード5枚
**When**: calculateFanLayoutを呼ぶ
**Then**: 5つの位置が扇形に配置される

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0195` - 詳細要件定義
2. `/tsumiki:tdd-testcases` - テストケース作成
3. `/tsumiki:tdd-red` - テスト実装（失敗）
4. `/tsumiki:tdd-green` - 最小実装
5. `/tsumiki:tdd-refactor` - リファクタリング
6. `/tsumiki:tdd-verify-complete` - 品質確認

---

## 注意事項

- カード枚数が多い場合の重なり処理
- 扇形配置の角度計算
- レスポンシブ対応は将来課題

---

## 信頼性レベルサマリー

- **総項目数**: 3項目
- 🔵 **青信号**: 0項目 (0%)
- 🟡 **黄信号**: 3項目 (100%)
- 🔴 **赤信号**: 0項目 (0%)

**品質評価**: 中品質

...
1ターン（1日）のゲームサイクルの後半部分（調合フェーズ、納品フェーズ）が正しく動作することを検証する統合テストを実施する。

## 依存タスク

- **前提タスク**: TASK-0230 (AlchemyContainerテスト), TASK-0234 (DeliveryContainerテスト)
- **後続タスク**: TASK-0263
...
describe('Turn Cycle Second Half Integration', () => {
  ...
  describe('Turn End', () => {
    ...
    it('報酬カード選択ダイアログが表示される（依頼完了時）', async () => {
      // Arrange - 依頼を完了させる
      stateManager.updateQuests({
        accepted: [{
          id: 'quest_002',

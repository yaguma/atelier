# TASK-0271: オブジェクトプール最適化

**タスクID**: TASK-0271
**タスクタイプ**: TDD
**推定工数**: 4時間
**フェーズ**: Phase 5 - 統合テスト・最適化・仕上げ
**信頼性レベル**: 🔴 *Phaser固有*

## 関連文書

- **概要**: [overview.md](./overview.md)
- **パフォーマンス計測**: TASK-0270

## タスク概要

頻繁に生成・破棄されるオブジェクト（カード、素材、エフェクト等）にオブジェクトプールを適用し、パフォーマンスを最適化する。

## 依存タスク

- **前提タスク**: TASK-0270 (パフォーマンス計測)
- **後続タスク**: TASK-0274

## 完了条件

- [ ] ObjectPoolクラスが実装されている
- [ ] カードオブジェクトにプールが適用されている
- [ ] 素材オブジェクトにプールが適用されている
- [ ] エフェクトにプールが適用されている
- [ ] パフォーマンス改善が確認されている

---

## 実装詳細

### 1. ObjectPool実装 🔴

**信頼性**: 🔴 *Phaser最適化*

**実装ファイル**: `src/presentation/phaser/utils/ObjectPool.ts`

```typescript
import Phaser from 'phaser';

export interface PoolableObject extends Phaser.GameObjects.GameObject {
  reset(): void;
  setPooled(pooled: boolean): void;
  isPooled(): boolean;
}

export interface ObjectPoolConfig<T extends PoolableObject> {
  scene: Phaser.Scene;
  factory: () => T;
  initialSize?: number;
  maxSize?: number;
  autoExpand?: boolean;
}

/**
 * 汎用オブジェクトプール
 */
export class ObjectPool<T extends PoolableObject> {
  private scene: Phaser.Scene;
  private factory: () => T;
  private pool: T[] = [];
  private active: Set<T> = new Set();
  private maxSize: number;
  private autoExpand: boolean;

  constructor(config: ObjectPoolConfig<T>) {
    this.scene = config.scene;
    this.factory = config.factory;
    this.maxSize = config.maxSize ?? 100;
    this.autoExpand = config.autoExpand ?? true;

    // 初期プール作成
    const initialSize = config.initialSize ?? 10;
    for (let i = 0; i < initialSize; i++) {
      this.createObject();
    }
  }

  private createObject(): T {
    const obj = this.factory();
    obj.setPooled(true);
    obj.setActive(false);
    obj.setVisible(false);
    this.pool.push(obj);
    return obj;
  }

  /**
   * プールからオブジェクトを取得
   */
  acquire(): T | null {
    let obj = this.pool.find(o => !this.active.has(o));

    if (!obj) {
      if (this.autoExpand && this.pool.length < this.maxSize) {
        obj = this.createObject();
      } else {
        console.warn('ObjectPool exhausted');
        return null;
      }
    }

    obj.setActive(true);
    obj.setVisible(true);
    obj.setPooled(false);
    this.active.add(obj);

    return obj;
  }

  /**
   * オブジェクトをプールに返却
   */
  release(obj: T): void {
    if (!this.active.has(obj)) {
      console.warn('Trying to release object not in active set');
      return;
    }

    obj.reset();
    obj.setActive(false);
    obj.setVisible(false);
    obj.setPooled(true);
    this.active.delete(obj);
  }

  /**
   * 全アクティブオブジェクトを返却
   */
  releaseAll(): void {
    this.active.forEach(obj => {
      obj.reset();
      obj.setActive(false);
      obj.setVisible(false);
      obj.setPooled(true);
    });
    this.active.clear();
  }

  /**
   * プールを破棄
   */
  destroy(): void {
    this.pool.forEach(obj => obj.destroy());
    this.pool = [];
    this.active.clear();
  }

  /**
   * 統計情報
   */
  getStats(): { total: number; active: number; available: number } {
    return {
      total: this.pool.length,
      active: this.active.size,
      available: this.pool.length - this.active.size,
    };
  }
}
```

### 2. PoolableCardView実装 🔴

**信頼性**: 🔴 *カードプール*

**実装ファイル**: `src/presentation/phaser/ui/cards/PoolableCardView.ts`

```typescript
import Phaser from 'phaser';
import { PoolableObject } from '../../utils/ObjectPool';
import { CardViewOptions } from './CardView';

/**
 * プール対応カードビュー
 */
export class PoolableCardView extends Phaser.GameObjects.Container implements PoolableObject {
  private _isPooled: boolean = false;
  private cardData: any = null;
  private background: Phaser.GameObjects.Graphics;
  private nameText: Phaser.GameObjects.Text;
  private descText: Phaser.GameObjects.Text;

  constructor(scene: Phaser.Scene, x: number = 0, y: number = 0) {
    super(scene, x, y);

    // 基本要素を事前作成
    this.background = scene.add.graphics();
    this.add(this.background);

    this.nameText = scene.add.text(0, -40, '', {
      fontSize: '14px',
      color: '#ffffff',
    }).setOrigin(0.5);
    this.add(this.nameText);

    this.descText = scene.add.text(0, 20, '', {
      fontSize: '10px',
      color: '#cccccc',
      wordWrap: { width: 80 },
    }).setOrigin(0.5);
    this.add(this.descText);

    this.setSize(100, 140);
    scene.add.existing(this);
  }

  /**
   * カードデータを設定
   */
  setCardData(data: any): void {
    this.cardData = data;

    // 背景描画
    this.background.clear();
    this.background.fillStyle(this.getCardColor(data.type), 1);
    this.background.fillRoundedRect(-50, -70, 100, 140, 8);
    this.background.lineStyle(2, 0xffffff, 0.5);
    this.background.strokeRoundedRect(-50, -70, 100, 140, 8);

    // テキスト更新
    this.nameText.setText(data.name || '');
    this.descText.setText(data.description || '');
  }

  private getCardColor(type: string): number {
    switch (type) {
      case 'gathering': return 0x4a7c59;
      case 'recipe': return 0x7c4a5c;
      case 'enhancement': return 0x5c5a7c;
      default: return 0x555555;
    }
  }

  /**
   * リセット（プール返却時）
   */
  reset(): void {
    this.cardData = null;
    this.setPosition(0, 0);
    this.setScale(1);
    this.setAlpha(1);
    this.setAngle(0);

    this.background.clear();
    this.nameText.setText('');
    this.descText.setText('');

    this.removeInteractive();
    this.removeAllListeners();
  }

  setPooled(pooled: boolean): void {
    this._isPooled = pooled;
  }

  isPooled(): boolean {
    return this._isPooled;
  }

  getCardData(): any {
    return this.cardData;
  }
}
```

### 3. CardPoolManager実装 🔴

**信頼性**: 🔴 *プール管理*

**実装ファイル**: `src/presentation/phaser/managers/CardPoolManager.ts`

```typescript
import Phaser from 'phaser';
import { ObjectPool } from '../utils/ObjectPool';
import { PoolableCardView } from '../ui/cards/PoolableCardView';

/**
 * カードプール管理
 */
export class CardPoolManager {
  private pool: ObjectPool<PoolableCardView>;
  private scene: Phaser.Scene;

  constructor(scene: Phaser.Scene) {
    this.scene = scene;

    this.pool = new ObjectPool<PoolableCardView>({
      scene,
      factory: () => new PoolableCardView(scene),
      initialSize: 20, // 初期20枚
      maxSize: 50, // 最大50枚
      autoExpand: true,
    });
  }

  /**
   * カードビューを取得
   */
  acquireCard(cardData: any, x: number, y: number): PoolableCardView | null {
    const card = this.pool.acquire();

    if (card) {
      card.setPosition(x, y);
      card.setCardData(cardData);
    }

    return card;
  }

  /**
   * カードビューを返却
   */
  releaseCard(card: PoolableCardView): void {
    this.pool.release(card);
  }

  /**
   * 複数カードを返却
   */
  releaseCards(cards: PoolableCardView[]): void {
    cards.forEach(card => this.pool.release(card));
  }

  /**
   * 全カードを返却
   */
  releaseAll(): void {
    this.pool.releaseAll();
  }

  /**
   * 統計取得
   */
  getStats(): { total: number; active: number; available: number } {
    return this.pool.getStats();
  }

  /**
   * 破棄
   */
  destroy(): void {
    this.pool.destroy();
  }
}
```

### 4. EffectPool実装 🔴

**信頼性**: 🔴 *エフェクトプール*

```typescript
// src/presentation/phaser/effects/PoolableEffect.ts

import Phaser from 'phaser';
import { PoolableObject } from '../utils/ObjectPool';

export class PoolableParticle extends Phaser.GameObjects.Particles.ParticleEmitter implements PoolableObject {
  private _isPooled: boolean = false;

  reset(): void {
    this.stop();
    this.setPosition(0, 0);
  }

  setPooled(pooled: boolean): void {
    this._isPooled = pooled;
  }

  isPooled(): boolean {
    return this._isPooled;
  }
}

export class EffectPoolManager {
  private sparklePool: ObjectPool<PoolableParticle>;

  constructor(scene: Phaser.Scene) {
    // パーティクルプール
    // 実際の実装はPhaserバージョンに依存
  }

  playSparkle(x: number, y: number): void {
    // プールからエミッターを取得して再生
  }

  destroy(): void {
    this.sparklePool?.destroy();
  }
}
```

---

## 単体テスト要件

### テストケース1: プール取得・返却 🔴

**Given**: ObjectPoolが初期化されている
**When**: acquire → release を実行
**Then**: オブジェクトが再利用される

### テストケース2: プール拡張 🔴

**Given**: 初期サイズ10のプール
**When**: 15個取得
**Then**: プールが自動拡張される

### テストケース3: 最大サイズ制限 🔴

**Given**: maxSize=20のプール
**When**: 21個取得
**Then**: nullが返される

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0271` - 詳細要件定義
2. `/tsumiki:tdd-testcases` - テストケース作成
3. `/tsumiki:tdd-red` - テスト実装（失敗）
4. `/tsumiki:tdd-green` - 最小実装
5. `/tsumiki:tdd-refactor` - リファクタリング
6. `/tsumiki:tdd-verify-complete` - 品質確認

---

## パフォーマンス目標

| 項目 | 最適化前 | 目標 |
|------|---------|------|
| カード生成時間 | 計測値 | -50% |
| GC発生頻度 | 計測値 | -30% |
| FPS安定性 | 計測値 | +10% |

---

## 注意事項

- リセット処理の完全性
- 循環参照の防止
- メモリリーク防止

---

## 信頼性レベルサマリー

- **総項目数**: 4項目
- 🔵 **青信号**: 0項目 (0%)
- 🟡 **黄信号**: 0項目 (0%)
- 🔴 **赤信号**: 4項目 (100%)

**品質評価**: 低品質（実装時に調整要・Phaser固有）

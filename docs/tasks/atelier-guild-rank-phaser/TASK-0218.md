# TASK-0218: QuestAcceptContainer受注操作実装
...
QuestAcceptContainerの依頼受注操作を実装する。受注確認、キャンセル、複数受注の制限を実装する。
...
private showAcceptConfirmDialog(quest: Quest): void {
  ...
  const rewardText = this.scene.add.text(
    this.width / 2,
    panelY + 85,
    `報酬: ${quest.reward.gold}G / ${quest.reward.exp}EXP`,
...
private showMaxQuestsWarning(): void {
  ...
  this.maxQuestsWarning = this.scene.add.text(
    400,
    460,
    `受注上限（${this.maxAcceptedQuests}件）に達しています`,
...
private async playAcceptAnimation(quest: Quest): Promise<void> {
  return new Promise((resolve) => {
    ...
    const cardText = this.scene.add.text(0, 0, '📋', { fontSize: '24px' }).setOrigin(0.5);
...
```typescript
// QuestAcceptContainerのsetupEventListenersに追加
...
**Given**: 確認ダイアログで「受注する」をクリック
**When**: 受注処理が実行される
**Then**: quest:acceptイベントが発火する

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0218` - 詳細要件定義

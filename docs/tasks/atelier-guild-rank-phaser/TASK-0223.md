...
GatheringContainerの素材提示機能を実装する。採取地カードに基づいて採取可能な素材を生成し、確率に応じて提示する。
...
**実装ファイル**: `src/presentation/phaser/ui/phase/GatheringMaterialGenerator.ts`
...
**実装ファイル**: `src/presentation/phaser/ui/phase/GatheringMaterialPresenter.ts`
...
export class GatheringMaterialPresenter {
  ...
  private async presentRareMaterial(option: MaterialOption, index: number): Promise<void> {
    ...
    return new Promise((resolve) => {
      ...
      // レア表示テキスト
      const rareText = this.scene.add.text(x, y - 30, '★ RARE ★', {
        fontSize: '10px',
        color: '#ffd700',
        fontStyle: 'bold',
      }).setOrigin(0.5);
...
### 3. GatheringContainerに統合 🔵

**信頼性**: 🔵 *統合*

```typescript
// GatheringContainerに追加
...
**Given**: 採取地カードがある
**When**: generateMaterialOptions(card)を呼ぶ
**Then**: 確率に応じた素材リストが返る

### テストケース2: レア判定 🔵

**Given**: 確率が0.2の素材
**When**: isRareMaterial(0.2)を呼ぶ
**Then**: trueが返る

### テストケース3: 最低保証 🔵

**Given**: 全素材が確率判定で落ちた場合
**When**: generateMaterialOptionsを呼ぶ
**Then**: 最低1つは素材が返る
...
1. `/tsumiki:tdd-requirements TASK-0223` - 詳細要件定義

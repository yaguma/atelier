...
**信頼性**: 🔵 *TASK-0201の設計に基づく*

**実装ファイル**: `src/presentation/phaser/ui/header/HeaderUI.ts`
...
export class HeaderUI implements IHeaderUI {
  ...
  private createExpGauge(): void {
    ...
    // 経験値ラベル
    const expLabel = this.scene.add.text(gaugeX, gaugeY - 20, '経験値', {
      ...TextStyles.bodySmall,
      fontSize: '11px',
    });
  ...
  // updateDay, updateGold, updateAPは次のTASK-0203で実装
  ...
  private createGoldDisplay(): void {
    const goldIcon = this.scene.add.text(HeaderLayout.GOLD_X - 25, HeaderLayout.HEIGHT / 2, '💰', {
      fontSize: '20px',
    }).setOrigin(0.5);
...
**Given**: HeaderUIが初期化されている
**When**: updateRankを呼ぶ
**Then**: ランクテキストと色が更新される

### テストケース2: 経験値ゲージ更新 🔵

**Given**: HeaderUIが初期化されている
**When**: updateExp(50, 100)を呼ぶ
**Then**: ゲージが50%塗られ、テキストが"50/100"になる

### テストケース3: 経験値獲得アニメーション 🔵

**Given**: HeaderUIが表示されている
**When**: animateExpGain(100)を呼ぶ
**Then**: "+100 EXP"がフロートして消える

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0202` - 詳細要件定義

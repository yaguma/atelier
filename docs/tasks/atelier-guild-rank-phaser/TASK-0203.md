# TASK-0203: HeaderUI日数・ゴールド・AP実装
...
**実装ファイル**: `src/presentation/phaser/ui/header/HeaderUI.ts` (追加)
...
private createAPGauge(): void {
  ...
  // APアイコン
  const apIcon = this.scene.add.text(gaugeX - 25, gaugeY, '⚡', {
    fontSize: '20px',
  }).setOrigin(0.5);
...
updateAP(current: number, max: number): void {
  ...
  // APの残量に応じた色
  let fillColor = 0x00aaff; // 通常: 青
  if (ratio <= 0.25) {
    fillColor = 0xff4444; // 危険: 赤
  } else if (ratio <= 0.5) {
    fillColor = 0xffaa00; // 注意: オレンジ
  }
...
**When**: updateDay(5, 30)を呼ぶ
**Then**: "Day 5/30"が表示される

### テストケース2: 残り日数警告 🔵

**Given**: HeaderUIが初期化されている
**When**: updateDay(26, 30)を呼ぶ
**Then**: テキストが警告色になる

### テストケース3: ゴールド変化アニメーション 🔵

**Given**: HeaderUIが表示されている
**When**: animateGoldChange(500)を呼ぶ
**Then**: "+500"がフロートして消える

### テストケース4: AP残量色 🔵

**Given**: HeaderUIが初期化されている
**When**: updateAP(2, 10)を呼ぶ
**Then**: ゲージが赤色で表示される
...
1. `/tsumiki:tdd-requirements TASK-0203` - 詳細要件定義

...
素材アイテムの視覚的表現コンポーネントを設計・実装する。素材アイコン、名前、品質、個数を表示する。

## 依存タスク

- **前提タスク**: TASK-0162, TASK-0163
- **後続タスク**: TASK-0200

## 完了条件

- [ ] IMaterialViewインターフェースが定義されている
- [ ] MaterialViewクラスが実装されている
- [ ] 素材名が表示される
- [ ] 品質が色分けで表示される
- [ ] 個数が表示される
- [ ] クリック/ホバーのインタラクションが動作する

---

## 実装詳細

### 1. MaterialView定数・インターフェース 🟡

**信頼性**: 🟡 *素材表示の標準化*

**実装ファイル**: `src/presentation/phaser/ui/material/MaterialConstants.ts`
...
**実装ファイル**: `src/presentation/phaser/ui/material/IMaterialView.ts`
...
**実装ファイル**: `src/presentation/phaser/ui/material/MaterialView.ts`
...
**Given**: 素材データがある
**When**: MaterialViewをcompactモードで生成する
**Then**: アイコンと個数が表示される

### テストケース2: 詳細表示 🟡

**Given**: 素材データがある
**When**: MaterialViewをdetailモードで生成する
**Then**: 名前、品質、個数が表示される

### テストケース3: 品質色分け 🟡

**Given**: 品質5の素材がある
**When**: MaterialViewを生成する
**Then**: 金色のインジケーターが表示される

...
全シーン間の遷移が正しく動作することを検証する統合テストを実施する。正常遷移、エッジケース、エラー復旧を含む包括的なテストを行う。

## 依存タスク

- **前提タスク**: TASK-0188, TASK-0238, TASK-0243, TASK-0246, TASK-0249
- **後続タスク**: TASK-0261

## 完了条件

- [ ] Boot → Title遷移テストがパスする
- [ ] Title → Main遷移テストがパスする
- [ ] Main → Shop → Main遷移テストがパスする
- [ ] Main → RankUp → Main遷移テストがパスする
- [ ] Main → GameOver遷移テストがパスする
- [ ] Main → GameClear遷移テストがパスする
- [ ] GameOver/GameClear → Title遷移テストがパスする
- [ ] 異常系テスト（二重遷移防止等）がパスする
...
**テストファイル**: `tests/integration/phaser/phase5/SceneTransitionIntegration.test.ts`
...
describe('Scene Transition Integration', () => {
  ...
  describe('Main to SubScenes', () => {
    ...
    it('Main → RankUp → Mainの往復遷移', async () => {
      // Act - 昇格試験へ
  ...
  describe('Game End Transitions', () => {
    ...
    it('ゲームオーバー時にGameOverSceneへ遷移する', async () => {
      // Act
    ...
    it('ゲームクリア時にGameClearSceneへ遷移する', async () => {
      // Act
...
1. `/tsumiki:tdd-requirements TASK-0260` - 詳細要件定義

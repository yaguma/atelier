...
採取フェーズコンテナの基本設計を行う。素材選択画面のレイアウトとインターフェースを定義する。

## 依存タスク

- **前提タスク**: TASK-0213, TASK-0220, TASK-0221
- **後続タスク**: TASK-0223

## 完了条件

- [x] IGatheringContainerインターフェースが定義されている
- [x] GatheringContainerOptionsが定義されている
- [x] 基本レイアウト（採取地カード、素材選択、APコスト）が配置されている
- [x] BasePhaseContainerを継承している

---

## 実装詳細

### 1. GatheringContainerレイアウト定数 🔵

**信頼性**: 🔵 *UI設計書*

**実装ファイル**: `src/presentation/phaser/ui/phase/GatheringContainerConstants.ts`
...
### 2. IGatheringContainerインターフェース 🔵

**信頼性**: 🔵 *フェーズコンテナ設計*

**実装ファイル**: `src/presentation/phaser/ui/phase/IGatheringContainer.ts`
...
### 3. GatheringContainer基本実装 🔵

**信頼性**: 🔵 *採取フェーズコンテナ*

**実装ファイル**: `src/presentation/phaser/ui/phase/GatheringContainer.ts`
...
**Given**: GatheringContainerを初期化
**When**: オプションを渡してインスタンス化
**Then**: コンテナが作成される

### テストケース2: 採取地カード設定 🔵

**Given**: GatheringContainerがある
**When**: setGatheringCard(card)を呼ぶ
**Then**: カードが表示される

### テストケース3: 素材選択肢設定 🔵

**Given**: GatheringContainerがある
**When**: setMaterialOptions([...])を呼ぶ
**Then**: 素材選択グリッドが表示される
...
1. `/tsumiki:tdd-requirements TASK-0222` - 詳細要件定義
...
- BasePhaseContainerの継承活用
- MaterialOptionViewとGatheringCostViewの連携
- APコストのリアルタイム計算

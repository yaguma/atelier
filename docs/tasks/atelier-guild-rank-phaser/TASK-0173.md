# TASK-0173: UIFactoryボタン生成実装

**タスクID**: TASK-0173
**タスクタイプ**: TDD
**推定工数**: 4時間
**フェーズ**: Phase 1 - Phaser基盤・インフラ構築
**信頼性レベル**: 🟡 *妥当な推測に基づく*

## 関連文書

- **概要**: [overview.md](./overview.md)
- **コアシステム**: [core-systems.md](../../design/atelier-guild-rank-phaser/core-systems.md)

## タスク概要

UIFactoryのcreateButton()メソッドを実装し、ゲーム内で使用するボタンを生成できるようにする。

## 依存タスク

- **前提タスク**: TASK-0172
- **後続タスク**: TASK-0174, TASK-0185, TASK-0201, TASK-0209, TASK-0210, TASK-0221, TASK-0247, TASK-0248

## 完了条件

- [ ] createButton()メソッドが実装されている
- [ ] ボタンのクリックイベントが動作する
- [ ] ホバー状態が視覚的に表示される
- [ ] 無効状態が実装されている
- [ ] カスタムスタイルが適用できる

---

## 実装詳細

### 1. createButton実装 🟡

**信頼性**: 🟡 *rexUI.Buttonsを使用*

**実装ファイル**: `src/presentation/phaser/ui/UIFactory.ts`に追加

```typescript
import Buttons from 'phaser3-rex-plugins/templates/ui/buttons/Buttons';
import Label from 'phaser3-rex-plugins/templates/ui/label/Label';

createButton(options: ButtonOptions): Label {
  const {
    x, y, text, width, height,
    style, textStyle, onClick, disabled
  } = options;

  const bgColor = style?.backgroundColor ?? Colors.primary;
  const hoverColor = Colors.primaryHover;
  const disabledColor = Colors.secondary;

  // 背景
  const background = this.rexUI.add.roundRectangle(
    0, 0,
    width ?? 150,
    height ?? 44,
    style?.cornerRadius ?? 8,
    disabled ? disabledColor : bgColor
  );

  // ラベル（ボタンとして使用）
  const button = this.rexUI.add.label({
    x, y,
    width: width ?? 150,
    height: height ?? 44,
    background,
    text: this.scene.add.text(0, 0, text, textStyle ?? TextStyles.button),
    space: {
      left: style?.padding ?? 20,
      right: style?.padding ?? 20,
      top: 10,
      bottom: 10,
    },
    align: 'center',
  }).layout();

  // インタラクション
  if (!disabled) {
    button.setInteractive({ useHandCursor: true });

    button.on('pointerover', () => {
      (background as any).setFillStyle(hoverColor);
    });

    button.on('pointerout', () => {
      (background as any).setFillStyle(bgColor);
    });

    button.on('pointerdown', () => {
      (background as any).setFillStyle(Colors.primaryHover);
      this.scene.tweens.add({
        targets: button,
        scaleX: 0.95,
        scaleY: 0.95,
        duration: 50,
      });
    });

    button.on('pointerup', () => {
      (background as any).setFillStyle(hoverColor);
      this.scene.tweens.add({
        targets: button,
        scaleX: 1,
        scaleY: 1,
        duration: 50,
        onComplete: () => {
          if (onClick) onClick();
        }
      });
    });
  }

  return button;
}

// ボタンの有効/無効を切り替える
setButtonEnabled(button: Label, enabled: boolean): void {
  const background = (button as any).getElement('background');
  if (enabled) {
    button.setInteractive({ useHandCursor: true });
    background.setFillStyle(Colors.primary);
  } else {
    button.disableInteractive();
    background.setFillStyle(Colors.secondary);
  }
}
```

### 2. プライマリ/セカンダリボタン 🟡

**信頼性**: 🟡 *デザインシステムに基づく*

```typescript
createPrimaryButton(options: Omit<ButtonOptions, 'style'>): Label {
  return this.createButton({
    ...options,
    style: {
      backgroundColor: Colors.primary,
      cornerRadius: 8,
    }
  });
}

createSecondaryButton(options: Omit<ButtonOptions, 'style'>): Label {
  return this.createButton({
    ...options,
    style: {
      backgroundColor: Colors.secondary,
      cornerRadius: 8,
    }
  });
}

createDangerButton(options: Omit<ButtonOptions, 'style'>): Label {
  return this.createButton({
    ...options,
    style: {
      backgroundColor: Colors.danger,
      cornerRadius: 8,
    }
  });
}
```

---

## 単体テスト要件

### テストケース1: ボタン生成 🟡

**Given**: UIFactoryインスタンスがある
**When**: createButton()を呼ぶ
**Then**: Labelオブジェクトが返される

### テストケース2: クリックイベント 🟡

**Given**: ボタンにonClickが設定されている
**When**: ボタンをクリックする
**Then**: onClickコールバックが呼ばれる

### テストケース3: 無効状態 🟡

**Given**: disabled: trueでボタンを生成する
**When**: ボタンをクリックする
**Then**: onClickは呼ばれない

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0173` - 詳細要件定義
2. `/tsumiki:tdd-testcases` - テストケース作成
3. `/tsumiki:tdd-red` - テスト実装（失敗）
4. `/tsumiki:tdd-green` - 最小実装
5. `/tsumiki:tdd-refactor` - リファクタリング
6. `/tsumiki:tdd-verify-complete` - 品質確認

---

## 注意事項

- rexUIのLabelコンポーネントAPIを確認
- アニメーションは控えめに
- アクセシビリティ（フォーカス状態）を検討

---

## 信頼性レベルサマリー

- **総項目数**: 2項目
- 🔵 **青信号**: 0項目 (0%)
- 🟡 **黄信号**: 2項目 (100%)
- 🔴 **赤信号**: 0項目 (0%)

**品質評価**: 中品質

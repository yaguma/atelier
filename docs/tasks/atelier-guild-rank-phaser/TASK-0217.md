# TASK-0217: QuestAcceptContainer依頼表示実装
...
QuestAcceptContainerの依頼リスト表示機能を完成させる。スクロール、フィルター、ソート機能を実装する。
...
private createSortButtons(): void {
  ...
  const sortOptions: Array<{ label: string; value: SortType }> = [
    { label: '期限順', value: 'deadline' },
    { label: '報酬順', value: 'reward' },
...
private createFilterButtons(): void {
  ...
  // フィルターアイコン
  const filterIcon = this.scene.add.text(0, 5, '🔍', { fontSize: '14px' });
  this.filterButtons.add(filterIcon);

  // 難易度ドロップダウン（簡易版：ボタンで切り替え）
  const difficulties = [null, 'easy', 'normal', 'hard', 'expert'];
  const labels = ['全て', '簡単', '普通', '難', '超難'];

  // 現在のフィルター表示
  const currentFilterText = this.scene.add.text(25, 5, '全て', {
    fontSize: '11px',
    color: '#ffffff',
  });
...
**Given**: 期限がバラバラの依頼リスト
**When**: setSortType('deadline')を呼ぶ
**Then**: 期限が近い順に並ぶ

### テストケース2: ソート - 報酬順 🔵

**Given**: 報酬がバラバラの依頼リスト
**When**: setSortType('reward')を呼ぶ
**Then**: 報酬が高い順に並ぶ

### テストケース3: フィルター 🔵

**Given**: 各難易度の依頼がある
**When**: difficultyFilter = 'hard'を設定
**Then**: 難しい依頼のみ表示される

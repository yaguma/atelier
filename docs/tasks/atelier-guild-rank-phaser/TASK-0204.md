...
- [x] HeaderConstantsのテストが通る
- [x] HeaderUtilsのテストが通る
- [x] HeaderUIの基本操作テストが通る
- [x] テストカバレッジ80%以上
...
### 1. HeaderConstantsテスト 🔵

**信頼性**: 🔵 *定数の検証*

**テストファイル**: `tests/unit/presentation/ui/header/HeaderUI.test.ts`
...
describe('HeaderConstants', () => {
  ...
  describe('HeaderColors', () => {
    ...
    it('高ランクほど目立つ色（主観的だが金色は最高ランク）', () => {
      expect(HeaderColors.RANK_S).toBe(0xffd700); // 金色
    });
  });
});
...
describe('HeaderUI', () => {
  ...
  describe('表示項目', () => {
    ...
    it.skip('メニューボタンが表示される', () => {
      // メニューボタンのテスト
    });
  });

  describe('更新機能', () => {
    it.skip('updateAllで全項目が更新される', () => {
      // 一括更新のテスト
    });

    it.skip('updateRankでランク色が変わる', () => {
      // ランク更新のテスト
    });

    it.skip('updateExpでゲージ幅が変わる', () => {
      // 経験値更新のテスト
    });
  });

  describe('アニメーション', () => {
    it.skip('animateExpGainでフロートテキストが表示される', () => {
      // 経験値獲得アニメーションのテスト
    });

    it.skip('animateGoldChangeでフロートテキストが表示される', () => {
      // ゴールド変化アニメーションのテスト
    });

    it.skip('animateAPChangeでフロートテキストが表示される', () => {
      // AP変化アニメーションのテスト
    });
  });

  describe('警告表示', () => {
    it.skip('残り日数が少ないと警告色になる', () => {
      // 日数警告のテスト
    });

    it.skip('APが少ないとゲージ色が変わる', () => {
      // AP警告のテスト
    });

    it.skip('APが少ないとテキストが点滅する', () => {
      // AP点滅のテスト
    });
  });
});
...
**実装ファイル**: `src/presentation/phaser/scenes/HeaderUITestScene.ts`
...
1. `/tsumiki:tdd-requirements TASK-0204` - 詳細要件定義

...
サイドバーの依頼リスト表示機能を実装する。受注中の依頼を一覧表示し、選択・ハイライト機能を提供する。
...
**実装ファイル**: `src/presentation/phaser/ui/sidebar/QuestListItem.ts`
...
export class QuestListItem {
  ...
  private getStatusColor(): number {
    ...
    if (this.quest.isComplete) {
      return 0x00ff00; // 完了可能: 緑
    }
    if (this.isUrgent()) {
      return 0xff4444; // 緊急: 赤
    }
    return 0x0088ff; // 通常: 青
  }
...
**実装ファイル**: `src/presentation/phaser/ui/sidebar/SidebarUI.ts` (追加)
...
**信頼性**: 🟡 *rexUIのScrollablePanel*

```typescript
// SidebarUIに追加（rexUIが必要）

private createQuestsScrollablePanel(): void {
  const scene = this.scene as any; // rexUI拡張を使用
...
**Given**: 受注中依頼が3件ある
**When**: setQuestsを呼ぶ
**Then**: 3件の依頼アイテムが表示される

### テストケース2: 依頼選択 🟡

**Given**: 依頼リストが表示されている
**When**: 依頼をクリックする
**Then**: クリックした依頼がハイライトされる

### テストケース3: 緊急依頼の表示 🟡

**Given**: 残り3日の依頼がある
**When**: リストに表示する
**Then**: 赤いステータスインジケーターが表示される

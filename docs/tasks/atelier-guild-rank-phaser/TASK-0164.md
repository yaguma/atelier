...
Phaser UIとApplication層の疎結合通信を実現するEventBusのインターフェースを定義する。
...
- [ ] IEventBusインターフェースが定義されている
- [ ] 32種類のイベント型が定義されている
- [ ] イベントペイロード型が定義されている
- [ ] 型安全なイベント発行・購読の仕組みがある
...
**信頼性**: 🟡 *core-systems.mdの32イベントを参考*

**実装ファイル**: `src/presentation/phaser/events/EventTypes.ts`
...
**実装ファイル**: `src/presentation/phaser/events/EventPayloads.ts`
...
**実装ファイル**: `src/presentation/phaser/events/IEventBus.ts`
...
**Given**: IEventBusインターフェースが定義されている
**When**: 型チェックを実行する
**Then**: コンパイルエラーがない

### テストケース2: イベントキー網羅 🟡

**Given**: EventTypesが定義されている
**When**: 全イベントキーを列挙する
**Then**: 32種類のイベントが存在する
...
- 型安全性を最優先
- イベント名は一貫した命名規則（namespace:entity:action）
- ペイロードはイミュータブルに

...
EventBusで定義されている全32種類のイベントが正しく発火・受信されることを検証する統合テストを実施する。

## 依存タスク

- **前提タスク**: TASK-0238 (MainScene EventBus統合), TASK-0251 (EventBus-UseCase連携実装)
- **後続タスク**: TASK-0270

## 完了条件

- [ ] UI→Appイベント（12種類）の統合テストがパスする
- [ ] App→UIイベント（20種類）の統合テストがパスする
- [ ] イベントペイロードの型安全性テストがパスする
- [ ] イベント順序の検証テストがパスする
...
**テストファイル**: `tests/integration/phaser/phase5/EventBusIntegration.test.ts`
...
1. `/tsumiki:tdd-requirements TASK-0269` - 詳細要件定義

...
ゲームクリア条件と結果表示の統合テストを実施する。Sランク到達時のクリア判定、統計表示、タイトルへの復帰が正しく動作することを検証する。

## 依存タスク

- **前提タスク**: TASK-0248 (GameClearScene実装), TASK-0263 (複数日進行テスト)
- **後続タスク**: TASK-0269

## 完了条件

- [x] Sランク到達判定の統合テストがパスする
- [x] クリア統計表示の統合テストがパスする
- [x] タイトル復帰の統合テストがパスする
- [x] クリアデータ保存の統合テストがパスする
...
**テストファイル**: `tests/integration/phaser/phase5/GameClearIntegration.test.ts`
  ...
  describe('Clear Score Calculation', () => {
    ...
    it('完了依頼数が多いほど高スコア', async () => {
      ...
      await vi.waitFor(() => {
        ...
        // 依頼ボーナス
        expect(stats.questBonus).toBe(20 * 50); // 20 * 50点
      });
    });
...
1. `/tsumiki:tdd-requirements TASK-0266` - 詳細要件定義

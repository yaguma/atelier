# TASK-0224: GatheringContainer選択操作実装
...
GatheringContainerの素材選択・確定操作を実装する。複数選択、選択上限、AP確認、確定処理を実装する。
...
- [x] 選択上限に達すると追加選択できない
- [x] AP不足時に確定できない
- [x] 確定時に採取結果アニメーションが再生される
- [x] EventBusにイベントが発火する
...
**Given**: maxSelections=3で2つ選択済み
**When**: 3つ目を選択
**Then**: 選択でき、上限メッセージが表示される

### テストケース2: AP不足 🔵

**Given**: currentAP=2, 選択中のコスト=5
**When**: 確定ボタンを押す
**Then**: 確定されない
...
**Given**: 素材が選択されAP十分
**When**: confirmGathering()を呼ぶ
**Then**: gathering:confirmイベントが発火し結果が渡される

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0224` - 詳細要件定義

...
複数の小さなテクスチャを1つのアトラスにまとめ、描画バッチを減らしてパフォーマンスを向上させる。
...
assets/
├── atlas/
│   ├── cards.json       # カードアトラス定義
│   ├── cards.png        # カードアトラス画像
│   ├── materials.json   # 素材アトラス定義
│   ├── materials.png    # 素材アトラス画像
│   ├── ui.json          # UIアトラス定義
│   └── ui.png           # UIアトラス画像
```
...
**実装ファイル**: `scripts/generate-atlas.ts`

```typescript
/**
 * テクスチャアトラス生成スクリプト
 *
 * 使用: npx ts-node scripts/generate-atlas.ts
 * 依存: free-tex-packer-core
 */
...
**実装ファイル**: `src/presentation/phaser/loaders/AtlasLoader.ts`
...
**実装ファイル**: `src/presentation/phaser/factories/AtlasSpriteFactory.ts`
...
### 5. BootSceneアトラス読み込み更新 🔴

**信頼性**: 🔴 *既存コード修正*

**修正ファイル**: `src/presentation/phaser/scenes/BootScene.ts`

```typescript
// BootScene.preload() に追加
...
1. `/direct-setup` - アトラス生成・ローダー実装
2. `/direct-verify` - 動作確認・パフォーマンス比較

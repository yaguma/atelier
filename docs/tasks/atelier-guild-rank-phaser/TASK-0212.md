# TASK-0212: BasePhaseContainer設計

**タスクID**: TASK-0212
**タスクタイプ**: TDD
**推定工数**: 4時間
**フェーズ**: Phase 3 - フェーズコンテナ・メイン画面実装
**信頼性レベル**: 🟡 *妥当な推測に基づく*

## 関連文書

- **概要**: [overview.md](./overview.md)
- **UI設計**: [ui-design/overview.md](../../design/atelier-guild-rank-phaser/ui-design/overview.md)
- **アーキテクチャ**: [architecture.md](../../design/atelier-guild-rank-phaser/architecture.md)

## タスク概要

各ゲームフェーズ（依頼受注、採取、調合、納品）の基底コンテナクラスを設計する。共通のライフサイクルとイベント処理を定義する。

## 依存タスク

- **前提タスク**: TASK-0168
- **後続タスク**: TASK-0213

## 完了条件

- [ ] IPhaseContainerインターフェースが定義されている
- [ ] 共通ライフサイクル（enter/exit/update）が設計されている
- [ ] EventBusとの連携方法が設計されている
- [ ] 子クラスでのオーバーライドポイントが明確

---

## 実装詳細

### 1. IPhaseContainerインターフェース 🟡

**信頼性**: 🟡 *コンポーネント設計*

**実装ファイル**: `src/presentation/phaser/ui/phase/IPhaseContainer.ts`

```typescript
import Phaser from 'phaser';
import { GamePhase } from '../../../../domain/common/GamePhase';
import { EventBus } from '../../core/EventBus';

export interface PhaseContainerConfig {
  scene: Phaser.Scene;
  eventBus: EventBus;
  x?: number;
  y?: number;
  width?: number;
  height?: number;
}

export interface IPhaseContainer {
  // 識別
  readonly phase: GamePhase;
  readonly container: Phaser.GameObjects.Container;

  // ライフサイクル
  enter(): Promise<void>;
  exit(): Promise<void>;
  update(delta: number): void;

  // 表示制御
  setVisible(visible: boolean): void;
  setEnabled(enabled: boolean): void;

  // アクション
  canComplete(): boolean;
  complete(): void;
  cancel(): void;

  // 破棄
  destroy(): void;
}
```

### 2. PhaseContainerEvents 🟡

**信頼性**: 🟡 *イベント定義*

**実装ファイル**: `src/presentation/phaser/ui/phase/PhaseContainerEvents.ts`

```typescript
import { GamePhase } from '../../../../domain/common/GamePhase';

export interface PhaseContainerEvents {
  // フェーズ完了
  'phase:complete': { phase: GamePhase; result?: any };

  // フェーズキャンセル
  'phase:cancel': { phase: GamePhase };

  // フェーズ内アクション
  'phase:action': { phase: GamePhase; action: string; data?: any };

  // エラー
  'phase:error': { phase: GamePhase; error: Error };
}
```

### 3. BasePhaseContainer基底クラス設計 🟡

**信頼性**: 🟡 *抽象クラス*

**実装ファイル**: `src/presentation/phaser/ui/phase/BasePhaseContainer.ts`

```typescript
import Phaser from 'phaser';
import { GamePhase } from '../../../../domain/common/GamePhase';
import { IPhaseContainer, PhaseContainerConfig } from './IPhaseContainer';
import { EventBus } from '../../core/EventBus';
import { Colors } from '../../config/ColorPalette';

export abstract class BasePhaseContainer implements IPhaseContainer {
  public abstract readonly phase: GamePhase;
  public readonly container: Phaser.GameObjects.Container;

  protected scene: Phaser.Scene;
  protected eventBus: EventBus;
  protected width: number;
  protected height: number;
  protected enabled: boolean = true;
  protected isActive: boolean = false;

  constructor(config: PhaseContainerConfig) {
    this.scene = config.scene;
    this.eventBus = config.eventBus;
    this.width = config.width ?? 800;
    this.height = config.height ?? 500;

    const x = config.x ?? 200;
    const y = config.y ?? 150;

    this.container = this.scene.add.container(x, y);
    this.container.setDepth(200);
    this.container.setVisible(false);

    // 基本構造を作成
    this.createBackground();
    this.createContent();
    this.setupEventListeners();
  }

  // --- ライフサイクル ---

  async enter(): Promise<void> {
    this.isActive = true;
    this.container.setVisible(true);
    this.container.setAlpha(0);

    // フェードインアニメーション
    await this.animateIn();

    // サブクラスの初期化処理
    await this.onEnter();

    // フェーズ開始イベント
    this.eventBus.emit('phase:action', {
      phase: this.phase,
      action: 'enter',
    });
  }

  async exit(): Promise<void> {
    // サブクラスの終了処理
    await this.onExit();

    // フェードアウトアニメーション
    await this.animateOut();

    this.container.setVisible(false);
    this.isActive = false;
  }

  update(delta: number): void {
    if (!this.isActive || !this.enabled) return;
    this.onUpdate(delta);
  }

  // --- 表示制御 ---

  setVisible(visible: boolean): void {
    this.container.setVisible(visible);
  }

  setEnabled(enabled: boolean): void {
    this.enabled = enabled;
    this.container.setAlpha(enabled ? 1 : 0.5);
    this.onEnabledChange(enabled);
  }

  // --- アクション ---

  abstract canComplete(): boolean;

  complete(): void {
    if (!this.canComplete()) return;

    const result = this.getCompletionResult();
    this.eventBus.emit('phase:complete', {
      phase: this.phase,
      result,
    });
  }

  cancel(): void {
    this.eventBus.emit('phase:cancel', {
      phase: this.phase,
    });
  }

  // --- 破棄 ---

  destroy(): void {
    this.removeEventListeners();
    this.container.destroy();
  }

  // --- 抽象メソッド（サブクラスで実装） ---

  protected abstract createContent(): void;
  protected abstract onEnter(): Promise<void>;
  protected abstract onExit(): Promise<void>;
  protected abstract onUpdate(delta: number): void;
  protected abstract getCompletionResult(): any;

  // --- オプショナルオーバーライド ---

  protected onEnabledChange(enabled: boolean): void {}
  protected setupEventListeners(): void {}
  protected removeEventListeners(): void {}

  // --- ヘルパーメソッド ---

  private createBackground(): void {
    const bg = this.scene.add.graphics();
    bg.fillStyle(Colors.panelBackground, 0.95);
    bg.fillRoundedRect(0, 0, this.width, this.height, 12);
    bg.lineStyle(2, Colors.panelBorder);
    bg.strokeRoundedRect(0, 0, this.width, this.height, 12);
    this.container.add(bg);
  }

  private async animateIn(): Promise<void> {
    return new Promise((resolve) => {
      this.scene.tweens.add({
        targets: this.container,
        alpha: 1,
        y: this.container.y - 20,
        duration: 300,
        ease: 'Power2.easeOut',
        onComplete: () => resolve(),
      });
    });
  }

  private async animateOut(): Promise<void> {
    return new Promise((resolve) => {
      this.scene.tweens.add({
        targets: this.container,
        alpha: 0,
        y: this.container.y + 20,
        duration: 200,
        ease: 'Power2.easeIn',
        onComplete: () => resolve(),
      });
    });
  }

  // --- EventBusヘルパー ---

  protected emitAction(action: string, data?: any): void {
    this.eventBus.emit('phase:action', {
      phase: this.phase,
      action,
      data,
    });
  }

  protected emitError(error: Error): void {
    this.eventBus.emit('phase:error', {
      phase: this.phase,
      error,
    });
  }
}
```

---

## 単体テスト要件

### テストケース1: ライフサイクル 🟡

**Given**: BasePhaseContainerの具象クラスがある
**When**: enter()を呼ぶ
**Then**: onEnterが呼ばれ、コンテナが表示される

### テストケース2: 完了処理 🟡

**Given**: canCompleteがtrueを返す
**When**: complete()を呼ぶ
**Then**: phase:completeイベントが発火する

### テストケース3: キャンセル処理 🟡

**Given**: フェーズがアクティブ
**When**: cancel()を呼ぶ
**Then**: phase:cancelイベントが発火する

---

## 実装手順

1. `/tsumiki:tdd-requirements TASK-0212` - 詳細要件定義
2. `/tsumiki:tdd-testcases` - テストケース作成
3. `/tsumiki:tdd-red` - テスト実装（失敗）
4. `/tsumiki:tdd-green` - 最小実装
5. `/tsumiki:tdd-refactor` - リファクタリング
6. `/tsumiki:tdd-verify-complete` - 品質確認

---

## 注意事項

- 非同期ライフサイクルの適切な管理
- EventBusとの疎結合
- サブクラスでの拡張性

---

## 信頼性レベルサマリー

- **総項目数**: 3項目
- 🔵 **青信号**: 0項目 (0%)
- 🟡 **黄信号**: 3項目 (100%)
- 🔴 **赤信号**: 0項目 (0%)

**品質評価**: 中品質
